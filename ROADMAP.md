...
> publish` maintainer utility. Design lock: [PROP-002](vibevm/vibespecs/modules/vibe-registry/PROP-002-decentralized-registry.xml).
>
...
> [PROP-003](vibevm/vibespecs/modules/vibe-resolver/PROP-003-dep-evolution.xml)
...
[PROP-001](vibevm/vibespecs/modules/vibe-registry/PROP-001-git-backend.xml);
...
**Scope.** Full design lock in [PROP-002](vibevm/vibespecs/modules/vibe-registry/PROP-002-decentralized-registry.xml) (supersedes PROP-001 §2.3 / §2.4 / §2.6). Phase A of that PROP is this slice's shippable surface:
...
**Thesis.** Bring vibevm in line with cargo `[workspace]` / Maven multi-module: a project decomposes into modules, each published independently — or not at all — with the whole structure declared in one manifest. Design lock: [PROP-007](vibevm/vibespecs/modules/vibe-workspace/PROP-007-workspace.xml).
...
lock: [PROP-009](vibevm/vibespecs/modules/vibe-workspace/PROP-009-loading-model.xml).
...
[PROP-012](vibevm/vibespecs/modules/vibe-workspace/PROP-012-managed-redirect-block.xml)
...
**Thesis.** Replace the flat `<kind>:<name>` namespace with reverse-FQDN `group` qualification (Maven `groupId` shape), keeping short names as CLI sugar. Design lock: [PROP-008](vibevm/vibespecs/modules/vibe-registry/PROP-008-qualified-naming.xml).
...
**Order.** Needs the short-name-resolution machinery from [PROP-005](vibevm/vibespecs/modules/vibe-index/PROP-005-package-index.xml) (the index), which is implemented. Sequence: M1.17 → M1.18 → PROP-005 → M1.19.
...
[PROP-010](vibevm/vibespecs/modules/vibe-registry/PROP-010-local-package-cache.xml).
...
[PROP-011](vibevm/vibespecs/modules/vibe-workspace/PROP-011-incremental-install.xml).
...
`mcp:org.vibevm.ai-native/rust-ai-native-mcp` (18 tools) and
`mcp:org.vibevm.ai-native/typescript-ai-native-mcp` (17 tools), both live-chained
...
Source: [PROP-003 §2.5.0](vibevm/vibespecs/modules/vibe-resolver/PROP-003-dep-evolution.xml#delivery-modes), [PROP-004 §5.4](legacy-spec/research/PROP-004-tessl-comparative-research.md#three-modes).
...
- Acceptance: a `delivery=lazy-push` subskill installed against a Claude Code session never appears in `vibevm/vibespecs/...` but is observably present in the agent's context when the trigger description matches.
...
Source: [PROP-004 §5.12](legacy-spec/research/PROP-004-tessl-comparative-research.md#search), [PROP-005](vibevm/vibespecs/modules/vibe-index/PROP-005-package-index.xml).
...
The index that makes this tractable is [PROP-005](vibevm/vibespecs/modules/vibe-index/PROP-005-package-index.xml), implemented alongside — the `vibe-index` server + CLI in `crates/vibe-index/`, and the consumer-side `IndexClient` fast path in `vibe-registry`.
...
- **vibevm registry explorer.** A browsable visualisation over the
  ...
  [PROP-008 §2.9](vibevm/vibespecs/modules/vibe-registry/PROP-008-qualified-naming.xml).
...
- **Per milestone:** walk the acceptance checklist in §16 of the
  ...
  release (`v0.1.0-m0`, `v0.1.0-m1`, etc.) and update `vibevm/vibespecs/WAL.xml`
  to reflect the new "Current phase."
- **Per session:** read `CLAUDE.md`, then `vibevm/vibespecs/WAL.xml`, then the
