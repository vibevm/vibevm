...
замер: корпус **11 167 / 209 / 44 — 97.8 %** из 11 420; реестр **102
обязательства / 209 дрейфов — 28 отложено решениями владельца, 74 открыто;
owed 41, ВСЕ на sync-маршруте владельца**. Boss-owed — ноль по всем маршрутам.
Ситтинг 2026-08-02 (после session-end, ЧЕТЫРЕ обмена): стройки вместо
смягчения (B-033…B-035), сверка тройки построена (шаг 0c), F-285 снят (D29),
и **BUILD-FIRST PIVOT** — правила дисциплины не выключаются за
неиспользование: F-154 → стройки B-036/B-037/B-038/B-040, TS-близнецы
починены вердикт-сначала (D30, split F-355; дефект генератора — B-043),
R-001-пара F-161 → B-039, F-167 применён (D31) + B-042, F-181 → семья
B-005, B-041 — карта развития инструментария (прямой запрос владельца);
бэклог стоит на B-043.
...
1. **Ситтинг 2026-08-02 осушен целиком (четыре обмена; ledger
   `#rulings-2026-08-02-2`):** F-185 / F-218 / F-132 / F-217 / F-154 /
   F-355 / F-181 отложены на записанные-или-построенные работы, F-167
   применён (D31), F-285 снят (D29), сверка тройки построена (шаг 0c).
   **Стоячее правило четвёртого обмена — BUILD-FIRST:** для механизмов
   дисциплины «аннотировать отсутствие» умерло; аннотация легитимна только
   как интерим, называющий записанную стройку; ослаблять правило за
   неиспользование запрещено словом владельца. На владельце: **правило
   свипа B-027** (ждёт «да, свипуй»), **две ре-презентации F-161**
   (противоречие первоисточника 74.8 % vs 75.3/70.2 в core-ai-native —
   семья F-125; однострочная починка битой ссылки §6→§4), **семьи
   F-215/F-281** (complete-цель без замера ×3 стека; no-zombie-тест ×6
   копий). **Карта B-041 — одобрена и интегрирована** (пятый обмен):
   живёт в корне как `TOOLING-MAP.md` рядом с бэклогом (раздел `#map` в
   BACKLOG.md — дайджест волн; корневой ROADMAP.md — продуктовые вехи,
   другой документ), рамка владельца записана в `##frame-line` дословно:
   волны исполняются фазами кампании (E после гейта D), недостающее
   откладывается, карта работ не стартует. Дальше — остатки sync-очереди
   по registry. Формат предъявлений: суть по-человечески →
