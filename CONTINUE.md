...
**Транспорт субагентов E/T (директива владельца 2026-08-03):** переключатель
`campaigns/packages-2026-09/SUBAGENT-MODE.toml` (сейчас `mode = "claudez"`) —
воркеры = Claude Code на GLM-5.2 через машинные запускаторы
`C:\Users\olegc\opt\bin\{claudez,claudez2}` (у каждого СВОЙ каталог состояния,
`-c` продолжает свой тред — проверено матрицей ALPHA/BRAVO); две параллельные
линии в worktree для изолированных правок, один поток для конфликтоопасных;
effort=max вшит. Вся механика: `SUBAGENT-LAUNCHERS.md` рядом с переключателем.
Босс перечитывает переключатель перед каждым fan-out; ревью/вердикты/коммиты
не делегируются ни в одном режиме. **Наблюдаемость (тот же день, проверено
вживую):** воркеры пишут stream-json-лог (событие на строку, с timestamp)
СРАЗУ в архив `C:\Users\olegc\git\v\cache\agents\{sorted/<task-id>,unsorted}/`;
пакет обязывает heartbeat-«echo PROGRESS: …»/«TASK-DONE»; босс опрашивает
~раз в 30 с (первичный сигнал — рост лога: воркер пропускает heartbeat'ы,
измерено); финализация — meta.md с вердиктом ревью; приёмка по артефактам,
не по финальной строке. Каждый пакет обязывает закрывающий
`WORKER-REPORT-<task-id>.md` по вшитому шаблону (список файлов, приёмка
по пунктам с file:line, дословный self-verify, обязательные отклонения) —
карта ревью босса; конфликтов между воркерами нет (отдельные worktree +
task-id в имени; в архив — со штампом).
