...
1. **Ситтинг 2026-08-02 осушен целиком (шесть обменов; ledger
   ...
   мёртв). Восьмой обмен: направление архитектуры
   записано — B-047 (норма поверхностей: логика в разделяемом крейте,
   CLI+MCP — тонкие поверхности; аудит «где прибито»; стеки уже держат,
   первая дыра хоста — MCP-половина B-018) и B-046 (мультиязычная
   композиция: autodiscovery по рельсам lockfile/[[mcp_server]]/[[binary]],
   закон автономности дословно, три варианта формы — развилка 11 карты);
   F-210 применён (D38) и резолвнут — якорь одного-клиента несёт полную
   историю с `@impl/plan (→ B-046)`, голдены — честную аннотацию. Дальше —
   остатки sync-очереди (14 открытых строк, owed 27 — числа командами). Формат предъявлений: суть по-человечески →
