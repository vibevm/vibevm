...
каждой посадке. **S4 (доки) и луп B-035 №2 — СДЕЛАНЫ** (гайды описывают
построенное + цитируют принцип паритета; `harvest/e12-b035-parity-pass.md` —
таблица №2, строки 1/13 паритет достигнут). **НЕ закрыто (до конца батча 2):**
пересуд семьи F-185 (реестр) + backlog-строки диспозиций. **Зеркала на 20+
коммитов позади** (роллаут — на явном wind-down).

## ПЕРВОЕ ДЕЛО НОВОЙ СЕССИИ — закрыть батч 2 (мандат Б/В/Г стоит, паузы нет)

S4 и луп сделаны. Осталось закрыть батч 2, затем батчи 3/4:

1. **Пересуд семьи F-185** (B-033 докладывает семью). F-185 — drift
   «`seam-error-cites-req` is not a rule» (`OBLIGATIONS.md:47`) — теперь ОНО
   правило `go-seam-error-cites-req`, drift закрыт. Рецепт: `vibe progress
   mirror --campaign <zone>` → `python
   campaigns/packages-2026-09/tasks/merge-verdicts.py --force` → `vibe progress
   seal <явные PATH'ы>` (mirror ПЕРЕД merge; merge+seal НЕ сцеплять; seal
   рефьюзит несуждённые маркеры). Реестр сожмётся на закрытые drift'ы.
2. **Backlog-строки** B-033/B-030/B-049 — пометить построенным в диспозициях
   (+ `##B035-LOOP-2` строка), как батч 1 помечал свои.
3. **§7 LOG** — запись батча 2 УЖЕ добавлена; после F-185 обнови числа.
4. **Батч 3** (B-036+B-037+B-038, развилка №1 computed-names С B-038 — стоп по
   одной) → **батч 4** (B-025+B-026) → выход **M-PARITY**.
5. **Волны В/Г** (см. WAL #next).

_(Старый детальный список S4-правок жил в `scratchpad/packet-E12-S4-DOCS.md`;
S4 уже посажен — см. `git log` `docs(packages)` и таблицу №2.)_

### Луп-заметка (сделано)
`harvest/e12-b035-parity-pass.md`: строка 1 (seam-error ×3) и строка 13
(floor-disable ×3) — паритет достигнут; строка 7 (conformance: Go построен
