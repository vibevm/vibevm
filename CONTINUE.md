...
замер: корпус **11 167 / 209 / 44 — 97.8 %** из 11 420; реестр **101
обязательство / 209 дрейфов — 25 отложено решениями владельца, 76 открыто;
owed 53, ВСЕ на sync-маршруте владельца**. Boss-owed — ноль по всем маршрутам.
Ситтинг 2026-08-02 (после session-end, три обмена): предъявления F-185 /
семья дупликаций / остаток F-132 отсужены «стройками вместо смягчения» —
B-033/B-034/B-035 зафайлены, B-029 обогащён, B-011 поднят до Самого
Высокого Приоритета; тройка CLAUDE/AGENTS/GEMINI получила алгоритмическую
сверку (шаг 0c self-check), F-217 отложен, F-285 снят (D29, resolved);
бэклог стоит на B-035.
...
1. **Ситтинг 2026-08-02 осушен целиком (три обмена; ledger
   `#rulings-2026-08-02-2`):** F-185 / F-218 / F-132 / F-217 отложены на
   записанные-или-построенные работы, F-285 снят (D29), сверка тройки
   CLAUDE/AGENTS/GEMINI построена шагом 0c `tools/self-check.sh`. На
   владельце: **правило свипа B-027** («не планируется → @spec/done,
   запланировано записью → @impl/plan» — ждёт «да, свипуй»), затем
   **остатки sync-очереди по registry** (35 обязательств на sync-маршруте;
   крупные — по `drift-registry.py`). Формат предъявлений, обязательный
   вперёд (уточнение 2026-08-02 поверх формата (ii)): сначала суть
