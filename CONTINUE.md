...
Долг ревью ЗАКРЫТ (138 строк прочитаны поштучно). Батч W1 ЗАКРЫТ: 407 якорей,
368 / 32 / 7 — 90.4 %, из них 26 self-referential. Кластер ai-native пересчитан в
91.6 % (2 470 / 207 / 20). Открыты находки F-124…F-128.

БАТЧ W2 СОБРАН, НО НЕ СУЖДЁН. Четыре таблицы уже на диске и в коммитах:
  ev-W2a.json  111 строк — wal: README, boot, WAL-PROTOCOL, SKILL
  ev-W2b.json  149 — wal: cold-resume, morning-routine, session-end-hook
  ev-W2c.json  179 — two-process-model, все пять файлов
  ev-W2d.json  253 — sync-from-code (5 файлов) + wal-specspaces (3)
Итого 692 якоря, 2 404 ссылки, 3 неразрешённые и ни одной выдумки. Ничего
заказывать заново не нужно — начинай с чтения.

Перед началом прочитай, в этом порядке:
  1. CONTINUE.md — целиком: §w2-in-flight (что таблицы уже установили),
     §recipe (цикл, которым закрыт W1), §standard (стандарт вердикта),
     §w1-reading (что уже прочитано)
  ...
  5. spec/terraforms/PACKAGES-ACTUALIZATION-CAMPAIGN-v0.1.md §3.1, §3.2, §5
     и §9 LOG с конца — там записи этой фазы

ПЕРВОЕ ДЕЙСТВИЕ: прогони verify-evidence.py по всем четырём таблицам W2 ДО
чтения, потом show-rows.py --brief построчно и выноси вердикт по каждой строке.
Начни с ev-W2a (111) — она самая чистая; затем W2b, W2c, W2d. Каждый файл
мержится и запечатывается отдельно, коммит на батч.
...
self-referential: `python campaigns/packages-2026-09/tasks/summary.py --batch W2`.
...
поведением хоста (§3.1 источник 2), и хост — живой потребитель. Правило-факт
дрейфует при СИСТЕМАТИЧЕСКОМ несоблюдении (двузначная доля окна), а не при
единичных исключениях, которые запись называет поимённо.
...
работающий воркер, и НЕ читай таблицу, пока её воркер может дописывать: обе ошибки
совершены в прошлой сессии и обе записаны.
