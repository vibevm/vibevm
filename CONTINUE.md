...
каждой посадке. **БАТЧ 2 ЗАКРЫТ:** S4-доки, луп B-035 №2
(`harvest/e12-b035-parity-pass.md` — строки 1/13 паритет достигнут), пересуд
семьи F-185 (`drift → confirmed`, реестр **87/176, 98.1 %**) и backlog-строки —
всё сделано. **Остаток батча 2 — только гигиена реестра:** ре-судить +
ре-запечатать S4-правленные гайды + манифест (семантически stale seal).
**Зеркала на ~28 коммитов позади** (роллаут — на явном wind-down).

## ПЕРВОЕ ДЕЛО НОВОЙ СЕССИИ — батч 3 (мандат Б/В/Г стоит, паузы нет)

Батч 2 закрыт. Дальше:

0. **(Гигиена, оппортунистически)** ре-seal S4-гайдов + манифеста: verdict-батч
   (правленые/новые якоря → `confirmed`) → `merge-verdicts.py <batch> --force`
   → `vibe progress seal <пути>` (образец —
   `tasks/evidence/batch-E12-F185-rejudge.json`). Присоединяется к пяти
   незапечатанным observed-файлам (PROP-035/029 + дизайн-доки).
1. **Батч 3** (карта §4): цензус (образец E8/E11) → босс-дизайн → claudez.
   B-036 (позиция инвариант-комментов) + B-037 (кастомные REQ-линты) + B-038
   (pending-карты). **Развилка №1 карты (computed-names) приходит С B-038 —
   НАСТОЯЩИЙ стоп владельцу** (дерево-вопрос, AskUserQuestion с рекомендацией).
2. **Батч 4** (B-025 → F-146; B-026 → F-206) → выход **M-PARITY**.
3. **Волны В/Г** (см. WAL #next).
