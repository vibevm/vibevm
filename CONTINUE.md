...
1. **Следующие предъявления sync-очереди** (формат-ruling (ii): по документу,
   ...
   2026-08-01): **F-185** (conform-frontend-go: три якоря, включая «conform.toml
   gains a Go section»; слово `gated_crates` уже поправлено рулингом F-166 —
   остаётся структурная ложь секции), **семья дупликаций F-217/F-218/F-285**
   (d7b, одно измерение «59» на троих), **остаток F-132 — ОДИН якорь
   маршрута (b)** (`RUST-PRINCIPLE-GENERATOR-INPUT-IS-TAGGED`; прежняя
   строка «девятка ждёт текстов» была устаревшей — партия 1a, коммит
   `eccb1499`/D13, применила и пере-судила девятку + RUNTIME-TRANSPORT ещё
   2026-08-01; сверено с кэшем 2026-08-02). Дальше — остатки sync-очереди
   по registry.
