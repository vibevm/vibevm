...
use ts_extract_bridge::{FileRecord, RawFact, RawMarker};
...
pub enum TcgBridgeError {
    #[error(
        "node is not spawnable: {source} \
         (violates spec://typescript-ai-native/mechanisms/TCG-PROTOCOL#errors; \
    ...
    #[error(
        "the project's `typescript` install is unresolvable: {detail} \
         (violates spec://typescript-ai-native/mechanisms/TCG-ORACLE#compiler; \
    ...
    #[error(
        "the oracle process died mid-session: {detail} \
         ...
         `tcg-typescript validate ...` to see the child's stderr)"
    )]
    OracleCrashed { detail: String },

    #[error(
        "protocol violation: {detail} \
         (violates spec://typescript-ai-native/mechanisms/TCG-PROTOCOL#framing; \
...
/// `ts_extract_bridge::conform_facts` lowers oracle answers exactly like
