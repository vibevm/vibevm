...
fn the_walk_reads_xml_under_the_same_address() {
    ...
    std::fs::create_dir_all(dir.path().join("spec/test")).unwrap();
    std::fs::write(dir.path().join("spec/test/DOC.xml"), XML_FORM).unwrap();
    ...
    assert_eq!(units[0].file, "spec/test/DOC.xml");
}
...
fn a_pair_in_one_directory_is_loud_and_both_halves_skipped() {
    ...
    std::fs::write(dir.path().join("spec/A.md"), "## A {#a}\nmd body\n").unwrap();
    std::fs::write(
        dir.path().join("spec/A.xml"),
        "<spec xmlns=\"https://vibevm.org/spec/1\"><section id=\"a\" title=\"A\"><p>xml body</p></section></spec>",
    )
    .unwrap();
    std::fs::write(dir.path().join("spec/B.md"), "## B {#b}\nbody\n").unwrap();
    let (units, warnings) = crate::mdspec::scan_spec_tree(dir.path(), &Config::default());
    ...
    assert!(
        warnings[0].message.contains("spec/A.md"),
        "{}",
        warnings[0].message
    );
    assert!(
        warnings[0].message.contains("spec/A.xml"),
        "{}",
        warnings[0].message
    );
    ...
    std::fs::create_dir_all(dir.path().join("spec/deep")).unwrap();
    std::fs::write(
        dir.path().join("spec/deep/A.xml"),
        "<spec xmlns=\"https://vibevm.org/spec/1\"/>",
    )
    .unwrap();
    let (units, warnings) = crate::mdspec::scan_spec_tree(dir.path(), &Config::default());
