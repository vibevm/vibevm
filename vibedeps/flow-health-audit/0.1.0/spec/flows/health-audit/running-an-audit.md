...
@fact:scope-of-this-document **Scope of this document.** This is the operational procedure for one
audit run, start to finish, plus the exact shape of an `AUDIT.md`
section and a worked example on an invented generic project. @status:impl/done

@fact:companion-document-pointers The
categories you walk in step 2: [`audit-checklist.md`](audit-checklist.md);
the rationale: [`HEALTH-AUDIT-PROTOCOL.md`](HEALTH-AUDIT-PROTOCOL.md). @status:impl/done

@fact:A-RUN-MUST-FINISH-THE-INVENTORY A run must finish the **inventory**. @status:impl/done

@fact:A-RUN-NEED-NOT-FINISH-EVERY-FIX It need not finish every *fix* —
fixing is the work the inventory schedules. @status:impl/done

## The run, step by step {#steps}

1. @fact:STEP-OPEN-A-DATED-SECTION **Open a dated section** in `AUDIT.md` at the repository root. If
   the file does not exist yet, create it with a one-line header and
   start the first section. The section heading is the run date. @status:impl/done
...
3. @fact:STEP-RECORD-EVERY-FINDING-AS-YOU-GO **Record every finding** as you go — ID, category, locator,
   severity, disposition. Do not batch this to the end; a finding
   un-recorded at the moment of discovery is a finding lost. @status:impl/done

4. @fact:STEP-CARRY-FORWARD-AND-RE-JUDGE **Carry forward** every still-`open` finding, and every `filed` one
...
5. @fact:STEP-FIX-THE-CHEAP-FINDINGS-AND-FILE-THE-REST **Fix the cheap findings in-run.** Small P1/P2 findings get fixed on
   ...
   becomes `filed` with a pointer to where it was filed. @status:impl/done

6. @fact:STEP-RECONCILE-THE-CHECKPOINT **Reconcile the checkpoint.** Bring the living checkpoint's "known
   ...
   `AUDIT.md`; the checkpoint only points at the active subset. @status:impl/done
...
@fact:AUDIT-MD-IS-APPEND-ONLY `AUDIT.md` is append-only: each run **prepends or appends a new dated
section**, never rewrites an old one. @status:impl/done

@fact:PAST-SECTIONS-ARE-FROZEN-HISTORY Past sections are frozen history
— the trend lives in their accumulation. @status:impl/done

@fact:one-section-looks-like-this One section looks like this: @status:impl/done
...
@fact:THE-DISP-COLUMN-RECORDS-WHERE-THE-FINDING-WENT The `Disp`
...
`open` findings get a one-line note below the table. @status:impl/done

## A worked run — "Acme API" {#worked-example}

@fact:take-an-invented-project Take an invented project: a REST service, `pytest` + `ruff` + a GitHub
Actions gate. @status:impl/done

@fact:a-milestone-closed-and-the-owner-triggers-an-audit A milestone just closed; the owner triggers an audit.
The run, narrated: @status:impl/done

- @fact:EXAMPLE-A-TEST-INTEGRITY **A (test integrity).** The coverage tool shows the payment-retry
  ...
  out stale and green-if-enabled (A2, P3, *fixed* by deleting them). @status:impl/done

- @fact:EXAMPLE-B-ROT-OUTSIDE-THE-GATE **B (rot outside the gate).** `scripts/migrate/` has its own tests
  ...
  by adding it to the workflow). @status:impl/done

- @fact:EXAMPLE-C-DRIFT **C (drift).** `API.md` still documents a `--legacy` flag removed two
  milestones back (C1/C2, P2, → *fixed*). The `CONTINUE.md` checkpoint
  claims a phase in progress that shipped (C3, reconciled in step 6). @status:impl/done

- @fact:EXAMPLE-D-DEBT **D (debt).** `git blame` dates a `FIXME: temporary` in `billing.py`
  ...
  *accepted* with the reason and a revisit trigger (D3, P3). The
  ...
  escalated (D4, P1, *open* → escalated). @status:impl/done

@fact:EXAMPLE-THE-RUN-CLOSES-WITH-THE-SECTION-AND-SEPARATE-COMMITS The run closes: `AUDIT.md` gets the section above, the three in-run
...
as `docs(audit): 2026-07-07 health audit`. @status:impl/done

@fact:EXAMPLE-THE-NEXT-RUN-OPENS-BY-RE-READING-THESE-ROWS The next run will open by
re-reading this section's `open` and `filed` rows. @status:impl/done
...
- @fact:DOES-NOT-OWE-EVERY-FIX **Does not owe:** every fix. A P1 that needs a design pass is *filed*,
  not force-fixed inside the audit; the inventory's job is to schedule
  it, not to complete it. @status:impl/done

## Summary {#summary}

- @fact:SUM-THE-SEVEN-STEPS Seven steps: open a dated section → walk the checklist breadth-first
  → record each finding → carry forward prior unresolved findings → fix
  the cheap ones and file the rest → reconcile the checkpoint → commit. @status:impl/done
- @fact:SUM-AUDIT-MD-IS-APPEND-ONLY `AUDIT.md` is append-only: new dated section each run, old sections
  frozen. One table row per finding, five fields, disposition records
  where it went. @status:impl/done
- @fact:SUM-FIX-CHEAP-FILE-THE-REST-COMMIT-SEPARATELY Fix cheap findings in-run as separate commits; file the rest; the
  audit section is its own commit. @status:impl/done
- @fact:SUM-THE-RUN-MUST-FINISH-THE-INVENTORY The run must finish the inventory, not every fix. @status:impl/done
