...
mod tests {
    ...
    /// A `spec/` tree with one kept doc beside one the project excludes: the
    /// kind of split a `spec_roots` glob cannot name on its own ("everything
    /// under `spec/`, except the rewritten-every-session checkpoint"). Two
    ...
    fn an_excluded_file_yields_no_units_but_its_neighbour_does() {
        ...
        let cfg = Config {
            spec_exclude: vec!["spec/gen/WAL.md".into()],
            ..Config::default()
        };
    ...
    fn a_stale_exclude_pattern_names_itself_and_removes_nothing() {
        let dir = spec_exclude_tree();
        let cfg = Config {
            spec_exclude: vec!["spec/gen/RETIRED.md".into()],
            ..Config::default()
        };
        let (units, warnings) = scan_spec_tree(dir.path(), &cfg);
        ...
        assert!(
            stale[0].message.contains("spec/gen/RETIRED.md"),
            "{}",
            stale[0].message
        );
    }
    ...
    fn an_invalid_exclude_glob_is_a_warning_naming_the_pattern() {
        let dir = spec_exclude_tree();
        let cfg = Config {
            spec_exclude: vec!["spec/a**/x.md".into()],
            ..Config::default()
        };
        let (units, warnings) = scan_spec_tree(dir.path(), &cfg);
        ...
        assert!(
            bad[0].message.contains("spec/a**/x.md"),
            "{}",
            bad[0].message
        );
