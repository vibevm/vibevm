...
##core-cards-are-the-rust-pilot-reference-set (The core `cards/` are the Rust pilot's reference set; a future
symmetry pass may unify both languages' Band-3 in the core — see the
guide's tooling note.) @spec/done

##standing-rules-lead Standing rules at the surface level: @impl/done

- ##RULE-ORDINARY-IDIOMATIC-TYPESCRIPT-SURFACE Ordinary idiomatic TypeScript at the token level — no invented dialect,
  ...
  verification (`tsc --noEmit -p <cell>` + `vitest run <cell>`, < ~60s). @impl/done
- ##RULE-COMPILER-IS-A-CONFIGURABLE-VERIFIER The compiler is a configurable verifier: the project `tsconfig` is a
  ...
  `deviates` + reason. @impl/done
- ##RULE-CELLS Cells: one cell = one file-set with a single public entry (`index.ts`
  ...
  (module-level mutable singletons, `declare global`) is forbidden. @impl/done
- ##RULE-UNSAFE-SET-IS-FORBIDDEN-IN-DOMAIN-CODE Types are erased and can be lied to. The `unsafe` set —
  ...
  (never `@ts-ignore`). Each deviation carries a recorded reason. @impl/done
- ##RULE-BRANDING-RECOVERS-NOMINAL-TYPING Structural typing is recovered to nominal by branding: a meaning-bearing
  primitive crossing a seam is a branded type, so the wrong same-shaped
  value fails `tsc`. @impl/done
- ##RULE-TYPED-SEAM-FAILURES Failure on a seam is a typed value (`Result<T, E>` / discriminated
  union), never an untyped `throw`; the `E` union cites `spec://` REQs and
  its exhaustive `switch` is compiler-checked. @impl/done
- ##RULE-EXTERNAL-DATA-ENTERS-AS-UNKNOWN Untyped external data (network, JSON, `process.env`, user input) enters
  as `unknown` and is narrowed only through a single-source schema
  (Zod/Valibot) that is both the static type and the runtime check. @impl/done
- ##RULE-EXAMPLE-PER-SEAM Every public seam carries one type-checked example (Twoslash) of
  ...
  requires a differential oracle (`fast-check`). @impl/done
- ##RULE-UNIFORMITY-IS-LOAD-BEARING Uniformity is load-bearing: one idiom per operation; exceptions are
  marked, or they propagate as false training signal. @impl/done

##shipped-toolchain-lead The shipped toolchain (this stack materialises it; no dev tree needed): @impl/done

- ##TOOLCHAIN-TYPESCRIPT-AI-NATIVE-UMBRELLA `typescript-ai-native` — `init` (bootstrap policies + registries),
  ...
  over node's TAP) / `tripwire` / `trace` / `fast-loop` / `codemod`; @impl/done
- ##TOOLCHAIN-NARROW-ENGINES plus
  the narrow `typescript-ai-native-conform` and `typescript-ai-native-specmap` engines, @impl/done
- ##TOOLCHAIN-AGENTIC-TYPE-ORACLE and
  ...
  conform rules as the gate — GUIDE §14, §15 move 5). @impl/done

##STRUCTURAL-GATE-PARSES-THROUGH-THE-PROJECT-TYPESCRIPT The
structural gate parses through the PROJECT's own `typescript` install
(node ≥ 22.6; the same devDependency the tsc floor step needs). @impl/done

##RUN-VIBE-NATIVELY-FROM-PATH-OR-IN-PLACE Run
...
##PROCEDURES-AS-AGENT-SKILLS Procedures as agent skills: `/typescript-ai-native-sweep` (recurring),
`/typescript-ai-native-terraform` (brownfield adoption) — `vibe skill install`
projects them. @impl/done
