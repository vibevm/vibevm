...
When implemented, `typescript-ai-native-conform-frontend` ships **in this package** — `stack:org.vibevm/typescript-ai-native-lang` — as the package's own code-root: the TypeScript mirror of how `rust-ai-native-lang` now carries `crates/rust-ai-native-conform-frontend`. Installing the stack would then yield a working `.ts` structural gate, not a description of one (the PROP-024 promise, applied to TypeScript). The shippable tree excludes build output (`node_modules/`, `dist/`, build caches) the same way it excludes `target/` for Rust (PROP-024 §2.2). Its binary is **`typescript-ai-native-conform`** — language-suffixed like the Rust stack's `rust-ai-native-conform`, so several discipline checkers can share a `PATH` without shadowing one another.
