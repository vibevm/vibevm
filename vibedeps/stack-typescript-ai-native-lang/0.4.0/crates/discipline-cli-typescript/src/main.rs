//! `discipline-typescript` — the umbrella discipline tool for
...
enum Cmd {
    ...
    /// The ts-tsc structural gate (conform-typescript check/freeze).
    Conform {
        #[command(subcommand)]
        cmd: ConformCmd,
    },
    ...
    TestGate {
        ...
        #[arg(long, default_value = discipline_cli_typescript::DEFAULT_TESTS_BASELINE)]
        baseline: String,
    },
    ...
    Tripwire {
        ...
        #[arg(long, default_value = discipline_cli_typescript::DEFAULT_DEBT_REGISTRY)]
        debt: String,
    },
    ...
    Health {
        ...
        #[arg(long, default_value = discipline_cli_typescript::DEFAULT_HEALTH_OUT)]
        out: String,
    },
...
enum ConformCmd {
    ...
    Check {
        #[arg(long, default_value = conform_cli_typescript::DEFAULT_TS_BASELINE)]
        baseline: String,
...
fn main() -> Result<()> {
    ...
    match cli.command {
        Cmd::Init { namespace, force } => discipline_cli_typescript::run_init(
            &root,
            &discipline_cli_typescript::InitOptions { namespace, force },
        ),
        Cmd::Floor { keep_going, quiet } => discipline_cli_typescript::run_floor(
            &root,
            &discipline_cli_typescript::FloorOptions { keep_going, quiet },
        ),
        Cmd::Conform {
            cmd: ConformCmd::Check { baseline, scope },
        } => conform_cli_typescript::run_check(&root, &baseline, scope.as_deref()),
        Cmd::Conform {
            cmd: ConformCmd::Freeze { baseline },
        } => conform_cli_typescript::run_freeze(&root, &baseline),
        Cmd::Specmap { check, gate } => {
            if gate {
                specmap_cli_typescript::run_gate(&root)
            } else {
                specmap_cli_typescript::run_specmap_typescript(&root, check)
            }
        }
        Cmd::Trace {
            cmd:
        ...
        } => discipline_cli_typescript::run_trace_explain(&root, &target, json, prose),
        Cmd::TestGate { baseline } => discipline_cli_typescript::run_test_gate(&root, &baseline),
        Cmd::Tripwire { base, debt } => {
            discipline_cli_typescript::run_tripwire(&root, base.as_deref(), &debt)
        }
        Cmd::Health { out } => discipline_cli_typescript::run_health(&root, &out),
        Cmd::FastLoop {
            cell,
            budget,
            enforce_budget,
        } => {
            discipline_cli_typescript::run_fast_loop(&root, cell.as_deref(), budget, enforce_budget)
        }
        Cmd::Codemod {
            cmd: CodemodCmd::AddCell { cell, spec_uri },
        } => discipline_cli_typescript::run_codemod_add_cell(&root, &cell, &spec_uri),
    }
}
