...
Для этого нужна адресная схема. Каноническая грамматика живёт в одном месте — секции `{#uri-scheme}` протокола адресуемых спек (`spec://org.vibevm.world/addressable-specs/flows/addressable-specs/ADDRESSABLE-SPECS-PROTOCOL#uri-scheme`) — и здесь только цитируется, не пересказывается:
```
spec://<group>/<name>[@<version>]/<doc-path>#<section>[.<sub>…][~r<N>]
```
Версия — опциональная фича: без неё адрес резолвится в самую свежую установленную версию пакета.
