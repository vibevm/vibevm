...
//! `tcg-rust` (TCG-PROTOCOL-RUST §2–§3): policy loading, the gate's
...
specmark::scope!("spec://rust-ai-native/mechanisms/TCG-PROTOCOL-RUST-v0.1#enrichment");
...
use specmark::spec;
use tcg_oracle_bridge_rust::oracle::Completion;
use tcg_oracle_bridge_rust::position::OuterPosition;
use tcg_oracle_bridge_rust::{Diagnostic, RustOracle, ValidateOutcome};

pub mod bench;
pub mod serve;

/// The ratchet baseline's conventional filename — the same file
/// `discipline-rust init` writes and `conform-rust check` reads
...
#[spec(implements = "spec://rust-ai-native/mechanisms/TCG-PROTOCOL-RUST-v0.1#enrichment")]
pub struct Policy {
    pub root: PathBuf,
    pub config: Config,
...
///
/// ```
/// let (name, module) = tcg_cli_rust::derive_crate_module(
///     &["crates/*".to_string()],
...
pub fn enrich_validate(
    ...
    let owned = conform_cli_rust::build_rules(&policy.config);
...
///
/// ```
/// let hits = tcg_cli_rust::detect_newtypes(
...
/// ```
#[spec(implements = "spec://rust-ai-native/mechanisms/TCG-PROTOCOL-RUST-v0.1#ops")]
...
pub fn spawn_oracle(
    root: &Path,
) -> Result<RustOracle<tcg_oracle_bridge_rust::client::ChildTransport>> {
