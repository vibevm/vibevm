//! bin `tcg-rust` — the agentic type oracle's CLI face
...
fn main() -> Result<()> {
    ...
    let code = match cli.cmd {
        Cmd::Serve { root } => tcg_cli_rust::serve::run_serve(&resolve_root(&root))?,
        Cmd::Validate {
            file,
            content_from,
            json,
        ...
        Cmd::Scope {
            ...
            let pos = position
                ...
                .map(parse_position)
                .transpose()?
                .unwrap_or(tcg_oracle_bridge_rust::position::OuterPosition {
                    line: 1,
                    character: 0,
                });
            let mut oracle = spawn_oracle(&root)?;
        ...
        Cmd::Bench {
            ...
            report,
            root,
        } => tcg_cli_rust::bench::run_bench(&corpus, &report, &resolve_root(&root))?,
    };
