//! `discipline-rust init` — bootstrap a project's discipline surface: the
...
pub fn run_init(root: &Path, opts: &InitOptions) -> Result<()> {
    ...
    conform.push_str(&format!(
        "# conform.toml — this project's AI-Native discipline policy\n\
         # (ENGINE-CONFORM; generated by `discipline-rust init`, then owned by you).\n\
         #\n\
    ...
    let mut specmap = format!(
        "# specmap.toml — this project's traceability policy\n\
         # (PROP-014; generated by `discipline-rust init`, then owned by you).\n\
         \n\
    ...
    if externals.is_empty() {
        specmap.push_str(
            "\n# No installed packages with spec trees were found under vibedeps/.\n\
             # After `vibe install`, re-run `discipline-rust init --force` (or add\n\
    ...
    eprintln!(
        "discipline-rust init: namespace `{namespace}`, {} layout",
        if is_workspace {
            "workspace"
        } else {
            "single-crate"
