//! `rust-ai-native` — the umbrella AI-Native discipline tool (PROP-024
...
enum Command {
    ...
    /// The conform gate (ENGINE-CONFORM) — delegate of `rust-ai-native-conform`.
    Conform {
        #[command(subcommand)]
        cmd: ConformCmd,
    },
    /// Build (or check) the specmap index — delegate of `rust-ai-native-specmap`.
    Specmap {
...
enum LedgerCmd {
    ...
    Render {
        ...
        #[arg(long, default_value = rust_ai_native_cli::DEFAULT_INTENT_REGISTRY)]
        intent: String,
    },
}
...
fn main() -> Result<()> {
    ...
    let root = cli.path;
    match cli.command {
        Command::Init { namespace, force } => rust_ai_native_cli::run_init(
            &root,
            &rust_ai_native_cli::InitOptions { namespace, force },
        ),
        Command::Floor {
            keep_going,
            quiet,
        ...
        Command::Conform { cmd } => match cmd {
            ...
            ConformCmd::Freeze { baseline } => rust_ai_native_conform::run_freeze(&root, &baseline),
        },
        Command::Specmap { check, gate } => {
            if gate {
                rust_ai_native_specmap::run_gate(&root)
            } else {
        ...
        Command::Trace { cmd } => match cmd {
            TraceCmd::Explain {
                ...
                json,
                prose,
            } => rust_ai_native_cli::run_trace_explain(&root, &target, json, prose),
        },
        Command::TestGate { baseline } => rust_ai_native_cli::run_test_gate(&root, &baseline),
        Command::Tripwire { base, debt } => {
            rust_ai_native_cli::run_tripwire(&root, base.as_deref(), &debt)
        }
        Command::Health { out } => rust_ai_native_cli::run_health(&root, &out, &[]),
        Command::FastLoop {
            cell,
            budget,
            enforce_budget,
        } => rust_ai_native_cli::run_fast_loop(&root, cell.as_deref(), budget, enforce_budget),
        ...
        Command::Ledger { cmd } => match cmd {
            LedgerCmd::Render {
                ...
                debt,
                intent,
            } => rust_ai_native_cli::run_ledger_render(&root, &debt, &intent, check),
        },
    }
}
