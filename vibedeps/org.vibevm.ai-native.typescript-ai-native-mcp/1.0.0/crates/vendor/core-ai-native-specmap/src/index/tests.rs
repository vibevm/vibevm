...
fn overloaded_item_fires_inclusively_and_is_silent_below() {
    ...
    let root = tmp.path();
    std::fs::create_dir_all(root.join("spec")).unwrap();
    std::fs::write(root.join("spec/T.md"), spec_doc_with_units(4)).unwrap();
    let src = root.join("crates/x/src");
    std::fs::create_dir_all(&src).unwrap();
    std::fs::write(
        src.join("lib.rs"),
...
fn two_verbs_into_one_target_count_as_one_connection() {
    ...
    let root = tmp.path();
    std::fs::create_dir_all(root.join("spec")).unwrap();
    std::fs::write(root.join("spec/T.md"), spec_doc_with_units(1)).unwrap();
    let src = root.join("crates/x/src");
    std::fs::create_dir_all(&src).unwrap();
...
fn overloaded_item_disabled_at_zero() {
    ...
    let root = tmp.path();
    std::fs::create_dir_all(root.join("spec")).unwrap();
    std::fs::write(root.join("spec/T.md"), spec_doc_with_units(5)).unwrap();
    let src = root.join("crates/x/src");
    std::fs::create_dir_all(&src).unwrap();
    std::fs::write(
        src.join("lib.rs"),
...
fn overloaded_warnings_are_deterministically_ordered() {
    ...
    let root = tmp.path();
    std::fs::create_dir_all(root.join("spec")).unwrap();
    std::fs::write(root.join("spec/T.md"), spec_doc_with_units(6)).unwrap();
    let src = root.join("crates/x/src");
    std::fs::create_dir_all(&src).unwrap();
    std::fs::write(
        src.join("lib.rs"),
