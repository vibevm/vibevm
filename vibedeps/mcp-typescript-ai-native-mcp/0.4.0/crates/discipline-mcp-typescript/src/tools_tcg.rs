...
//! Enrichment goes through `tcg_cli_typescript::enrich_validate` — the
...
specmark::scope!("spec://typescript-ai-native-mcp/tools/discipline-mcp-typescript#tcg-tools");
...
use tcg_cli_typescript::{ORACLE_TIMEOUT, Policy, enrich_validate, parse_position};
use tcg_oracle_bridge::{OracleTransport, Position, SystemOracle, TcgBridgeError};
...
fn tcg_bench(session: &mut TcgSession, args: &Value) -> ToolOutput {
    ...
    match mcp_core::capture(|| {
        tcg_cli_typescript::bench::run_bench(&root, &root.join(corpus), &root.join(report))
    }) {
    ...
    vec![
        t(
            ...
            "Check a file (or a hypothetical overlay) BEFORE writing it: the \
             ...
             diagnostic OR a non-baselined finding. = `tcg-typescript validate`.",
        ...
        t(
            ...
            "Type-valid continuations at a position, with the unsafe-set flag on \
             banned forms. = `tcg-typescript complete`.",
            json!({
        ...
        t(
            ...
            "The differential corpus harness: oracle-vs-tsc agreement + latency \
             report. = `tcg-typescript bench`. Heavy — expect minutes.",
            json!({
