...
//! rule assembly (through `typescript_ai_native_conform::build_rules` — the
...
impl Policy {
    ...
    pub fn load(root: &Path) -> Result<Self> {
        ...
        let baseline_path = root.join(typescript_ai_native_conform::DEFAULT_TS_BASELINE);
...
pub fn enrich_validate(policy: &Policy, file: &str, raw: ValidateResult) -> EnrichedValidate {
    ...
    let owned = typescript_ai_native_conform::build_rules(&policy.config);
...
fn spawn_ready(root: &Path, policy: &Policy) -> Result<SystemOracle> {
    ...
    eprintln!(
        "typescript-ai-native-tcg: oracle up — typescript {}, {} root file(s), config {}",
...
pub fn run_validate(
    ...
    if json {
        ...
        println!(
            "typescript-ai-native-tcg validate: {} diagnostic(s) ({errors} error(s)), \
...
mod tests {
    use super::*;
    use typescript_ai_native_extract_bridge::{RawFact, RawMarker};
    use typescript_ai_native_tcg_bridge::Diagnostic;
