...
///
/// ```
/// use core_ai_native_conform::rules::FlagSites;
/// use core_ai_native_conform::Rule;
///
/// let rule = FlagSites {
            ...
            for f in &sf.facts {
                    ...
                    && cells.contains(type_name)
                {
                    out.push(Finding {
                        ...
                        line: *line,
                        message: req_message(
                            "discipline://rust-ai-native-lang/guide#registry-and-flags",
                            &format!(
                ...
                for (_t, other_file, other_crate) in &cells {
                    ...
                    if same_crate || cross_crate {
                        out.push(Finding {
                            ...
                            line: *line,
                            message: req_message(
                                "discipline://rust-ai-native-lang/guide#cells",
...
///
/// ```
/// use core_ai_native_conform::rules::CellHasOracle;
    ...
    fn check(&self, facts: &[SourceFacts]) -> Vec<Finding> {
        ...
        for (type_name, file, crate_name) in cell_types(facts) {
            ...
            out.push(Finding {
                ...
                line,
                message: req_message(
                    "discipline://rust-ai-native-lang/cards/scaffold-d-differential-oracle#ops",
                    &format!(
