...
@fact:PACKAGE-INSTALLS-A-CONFLICT-RESOLUTION-DISCIPLINE A `flow` package that installs a conflict-resolution discipline for
projects where two writers — a human and a coding agent — edit one
file set. @status:impl/done

@fact:CONTRADICTIONS-ARE-NORMAL-COOPERATION-NOT-AN-ERROR The two *will* write contradictory things; that is normal
cooperation, not an error. @status:spec/done

@fact:WHAT-THE-DISCIPLINE-FORBIDS-IS-RESOLVING-A-CONTRADICTION-SILENTLY What the discipline forbids is resolving a
...
invented semantics. @status:impl/done

@fact:the-core-law-fits-on-one-line The core law fits on one line: @status:impl/done

```
Human  >  Spec  >  Tests  >  Code  >  WAL
```

@fact:HUMAN-CHANGES-THE-SPEC-CODE-CONFORMS-AND-STATE-FILES-ARE-LAST The human may change the spec; code must conform to the spec; tests
are the spec in executable form; volatile state files are records,
dead last. @status:impl/done

@fact:RECENCY-DECIDES-NOTHING Recency decides nothing. @status:impl/done
...
- @fact:COMPOSES-TWO-PROCESS-MODEL `flow:two-process-model` — the hierarchy is that model's law #1;
  this package is its enforcement arm. @status:impl/done
- @fact:COMPOSES-SYNC-FROM-CODE `flow:sync-from-code` — the sanctioned *loud* path for code-first
  changes. This protocol is what makes the silent path forbidden;
  sync-from-code is where legitimate code-first reality goes. @status:impl/done
- @fact:COMPOSES-WAL `flow:wal` — the WAL sits at the hierarchy's tail as a volatile
  record; the stale-WAL recovery drill lives in this package's
  failure-modes document. @status:impl/done
- @fact:COMPOSES-ADDRESSABLE-SPECS `flow:addressable-specs` — a REVIEW marker and a correction both
  cite the violated anchor; addressability is what makes a dispute
  cost twenty tokens instead of two hundred. @status:spec/done
- @fact:COMPOSES-DECISION-RECORDS `flow:decision-records` — a resolved conflict often ends as a
...
@fact:practice-extracted-from-the-book The practice is extracted from *AI-native development*, chapters 1–2:
...
modes come from chapter 2. @status:spec/done

@fact:CHAPTERS-SHIP-IN-RUSSIAN-INSIDE-REDBOOK Both chapters ship in Russian inside
