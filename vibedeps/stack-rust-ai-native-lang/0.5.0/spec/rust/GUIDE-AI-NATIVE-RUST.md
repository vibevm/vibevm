...
- **The language-suffix rule (owner policy, 2026-07-07).** Every Rust artifact that has an analog in another language ends in `-rust` — crate and module names included; executable names and externally visible libraries especially (`discipline-cli-rust` beside `discipline-cli-typescript`; bin `conform-rust` beside `conform-typescript`; brief `vibe-tcg-rust.md` beside `vibe-tcg-ts.md`). Artifacts with no cross-language analog keep their plain names (`env-audit`); language-NEUTRAL artifacts (the vendored engine crates, vibevm's generic `vibe-tcg` family crate) are outside the rule by definition. The point is the same as the uniformity rule above: a name is a token program, and a cross-language pair that differs only sometimes is false signal.
...
The tcg line has two briefs here. **Shipped:** `rust/tools/vibe-agentic-tcg-rust.md` — the agentic type oracle (`tcg-rust` over the consumer's rust-analyzer; validate/scope/complete/type on in-memory overlays, discipline-enriched by the same conform engine as the gate; the four `tcg_*` MCP tools answer `language: "rust"`). **Very-far-future:** `rust/tools/vibe-tcg-rust.md` — token-level masking to rust-analyzer-validated, discipline-conformant continuations; waits on an inference substrate. The oracle is the generation-time complement to the post-generation `cargo check` loop (Class E) — consultation today, masking maybe-someday; the floor stays the truth either way.

## 13. Wiring the gates in a consumer project

The stack ships everything below; nothing requires the discipline's dev tree.

1. **Install the toolchain.** `vibe install` materialises this package into `vibedeps/`. Then either put the umbrella binary on PATH once — `cargo install --path vibedeps/<stack-slot>/crates/discipline-cli-rust` — or run it in place: `cargo run --manifest-path vibedeps/<stack-slot>/Cargo.toml -p discipline-cli-rust --bin discipline-rust -- <args>`. Add `vibedeps/**/target/` to `.gitignore`.
2. **Bootstrap.** `discipline-rust init` writes `conform.toml` (topology-detected roots; every crate exempt-with-a-reason — the pre-adoption posture), `specmap.toml` (your `namespace` + `[[external_specs]]` discovered from the installed packages, so citations of `spec://core-ai-native/…` resolve), and the `discipline/registry/` files. Idempotent; `--force` to regenerate. Run it after your workspace skeleton exists (topology is detected at init time); re-run with `--force` if the layout changes later.
