//! `discipline-mcp-rust` — the AI-Native Rust discipline served over
...
//! commands (thin adapters over the SAME lib fns the `discipline-rust`
//! / `conform-rust` / `specmap-rust` CLIs call, each wrapped in the
...
specmark::scope!("spec://discipline-rust/tools/discipline-mcp-rust#root");
...
pub const SERVER_NAME: &str = "discipline-rust";

/// Assemble the full tool set for a project root: 13 discipline + 5
/// tcg = 18 tools, the whole `discipline-rust` + `tcg-rust` command
/// surface.
///
/// ```
/// let set = discipline_mcp_rust::tool_set(std::path::Path::new("."));
