...
specmark::scope!("spec://discipline-rust/tools/discipline-mcp-rust#discipline-tools");
...
pub(crate) fn language_mismatch(args: &Value) -> Option<ToolOutput> {
    ...
    Some(ToolOutput::failed(format!(
        "this server serves language `rust`; asked for `{asked}` — mount that \
         language's own discipline server (mcp:org.vibevm/discipline-{asked}) \
    ...
    vec![
        t(
            ...
            "Bootstrap the discipline surface (conform.toml, specmap.toml, registries, \
             external specs). = `discipline-rust init`. Never overwrites without `force`.",
            properties! {
            ...
            |root, args| {
                discipline_cli_rust::run_init(
                    root,
                    &discipline_cli_rust::InitOptions {
                        namespace: args
                            .get("namespace")
        ...
        t(
            ...
            "The portable verification floor: fmt → test → clippy → conform → specmap → \
             test-gate, one report, one verdict. = `discipline-rust floor`. Runs the \
        ...
        t(
            ...
            "The conform gate: extract facts, run the Class-F/G rules, fail on any new \
             finding past the frozen ratchet. = `conform-rust check` / `discipline-rust \
             conform check`.",
            properties! {
        ...
        t(
            ...
            "Rewrite the conform ratchet to the current finding set (a NEW rule landing, \
             or a re-freeze after the set shrank). = `conform-rust freeze`.",
            properties! {
                "baseline": {"type": "string", "description": "ratchet file, root-relative"},
            },
            |root, args| {
                conform_cli_rust::run_freeze(
                    root,
                    str_arg(args, "baseline", "conform-baseline.json"),
                )
            },
        ...
        t(
            ...
            "Rebuild the traceability index in memory and byte-compare against the \
             committed specmap.json; the orphan ratchet blocks. = `specmap-rust --check`.",
            properties! {},
            |root, _| specmap_cli_rust::run_specmap(root, true),
        ),
        t(
            "specmap_write",
            "Regenerate and write specmap.json (drift report to the log); the orphan \
             ratchet reports non-blocking. = `specmap-rust`.",
            properties! {},
            |root, _| specmap_cli_rust::run_specmap(root, false),
        ),
        t(
            "trace_explain",
            "Explain one symbol or spec unit through the index: what implements, \
             verifies, documents it. = `discipline-rust trace <target>`.",
            properties! {
            ...
            |root, args| {
                ...
                discipline_cli_rust::run_trace_explain(
                    root,
                    target,
                    bool_arg(args, "json"),
                    bool_arg(args, "prose"),
        ...
        t(
            ...
            "The xfail-strict test gate over nextest results vs the tests baseline. \
             = `discipline-rust test-gate`. Runs the whole test suite — expect minutes.",
            properties! {
                "baseline": {"type": "string", "description": "root-relative baseline (default discipline/registry/tests-baseline.json)"},
            },
            |root, args| {
                discipline_cli_rust::run_test_gate(
                    root,
                    str_arg(
                        args,
                        "baseline",
                        discipline_cli_rust::DEFAULT_TESTS_BASELINE,
                    ),
                )
            },
        ),
        ...
        t(
            ...
            "The sweep's fact collector: one JSON snapshot of gating, budgets, danger \
             bands. = `discipline-rust health`.",
            properties! {
                "out": {"type": "string", "description": "root-relative output (default discipline/health/latest.json)"},
            },
            |root, args| {
                discipline_cli_rust::run_health(
                    root,
                    str_arg(args, "out", discipline_cli_rust::DEFAULT_HEALTH_OUT),
                    &[],
                )
            },
        ),
        t(
            ...
            "Per-cell first-signal budget: build+test each cell in isolation. \
             = `discipline-rust fast-loop`. Expensive.",
            properties! {
        ...
        t(
            ...
            "Scaffold one cell (module + registration + scope! tag) with rollback on \
             failure. = `discipline-rust codemod add-cell`.",
            properties! {
        ...
        t(
            ...
            "Render the debt/intent registries into the committed DEBT.md / INTENT.md \
             views. = `discipline-rust ledger render [--check]`.",
            properties! {
                "check": {"type": "boolean", "description": "verify the rendered views are current instead of writing"},
            },
            |root, args| {
                discipline_cli_rust::run_ledger_render(
                    root,
                    discipline_cli_rust::DEFAULT_DEBT_REGISTRY,
                    discipline_cli_rust::DEFAULT_INTENT_REGISTRY,
                    bool_arg(args, "check"),
                )
            },
        ),
...
mod tests {
    ...
    fn language_mismatch_refuses_with_the_recipe() {
        ...
        assert!(out.is_error);
        assert!(
            out.report.contains("discipline-typescript"),
            "{}",
            out.report
        );
