...
(installed; authored in `vibevm/vibepacks/org.vibevm.world/wal-specspaces/`).
The scoped grammar and target resolution come from its boot snippet, compiled
into the static lane `vibevm/vibespecs/boot/STATIC.xml`, read at boot; this file is the registry.
...
(this repository's own `vibevm/vibespecs/WAL.xml` + `CONTINUE.md`), never a
