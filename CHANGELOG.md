...
- **Index-backed short-name resolution.** `vibe install wal` — a bare, unqualified pkgref — is resolved to the qualified `org.vibevm/wal` at the CLI input boundary, once, before the depsolver runs; manifests and the lockfile only ever store the qualified form. The lockfile is consulted first (a locked package of that name wins outright), then registry candidates are enumerated — a local-directory registry by directory scan, a multi-registry resolver by one `by-name/<name>.json` GET per registry.
