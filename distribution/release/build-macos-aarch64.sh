...
usage() {
    printf '%s\n' \
        'usage: build-macos-aarch64.sh [--upload] [--checks] [--tests] [--self-check]'
}
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        -h|--help) usage; exit 0 ;;
        *) printf 'build-macos-aarch64.sh: unknown argument: %s\n' "$1" >&2; usage >&2; exit 2 ;;
    esac
    shift
done
...
if [ "$detected_os" != Darwin ] || { [ "$detected_arch" != arm64 ] && [ "$detected_arch" != aarch64 ]; }; then
    printf 'build-macos-aarch64.sh: requires macOS ARM64; detected OS=%s architecture=%s\n' \
        "$detected_os" "$detected_arch" >&2
    exit 2
fi
if ! command -v cargo >/dev/null 2>&1; then
    printf '%s\n' \
        'build-macos-aarch64.sh: cargo was not found on PATH; install the repository Rust toolchain first.' >&2
    exit 127
fi
...
if [ ! -x "$xtask_binary" ]; then
    printf 'build-macos-aarch64.sh: compiled xtask was not found at %s\n' "$xtask_binary" >&2
    exit 1
fi

set -- dist build --target aarch64-apple-darwin
...
exec "$xtask_binary" dist upload-built --target aarch64-apple-darwin
