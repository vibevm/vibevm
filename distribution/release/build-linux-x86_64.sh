...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        -h|--help) usage; exit 0 ;;
        *) printf 'build-linux-x86_64.sh: unknown argument: %s\n' "$1" >&2; usage >&2; exit 2 ;;
    esac
    shift
done
...
if [ "$detected_os" != Linux ] || { [ "$detected_arch" != x86_64 ] && [ "$detected_arch" != amd64 ]; }; then
    printf 'build-linux-x86_64.sh: requires Linux x86_64; detected OS=%s architecture=%s\n' \
        "$detected_os" "$detected_arch" >&2
    exit 2
fi
if ! command -v cargo >/dev/null 2>&1; then
    printf '%s\n' \
        'build-linux-x86_64.sh: cargo was not found on PATH; install the repository Rust toolchain first.' >&2
    exit 127
fi
...
if [ ! -x "$xtask_binary" ]; then
    printf 'build-linux-x86_64.sh: compiled xtask was not found at %s\n' "$xtask_binary" >&2
    exit 1
fi
