...
The independent documentation is now integrated from the documentation branch under `vibevm/vibepacks/org.vibevm.core/vibevm-docs/v1.0.0` and its Russian adaptation. `NEXT-PREVIEW-DOCS-INPUT` still binds its exact source/revision and allowed scope before dependent work; repository integration alone does not accept that campaign gate or start execution. Pending input binding blocks only its dependents. Update documentation by comparing the bound baseline, current authored documentation and accepted product changes; include new topics and preserve concurrent edits. Future checker stages distinguish intake, edited documentation and verified claims so an early success cannot imply later proof.
