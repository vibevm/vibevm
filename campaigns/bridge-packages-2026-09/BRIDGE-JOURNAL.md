...
- Finish the technical bridge substrate before treating package population as
  the main activity.
- The first adaptation examples are GitHub Spec Kit and Matt Pocock Skills.
...
- `org.vibevm.bridges/matt-pocock-skills` and its former public repository
  `https://github.com/vibespecs/org.vibevm.bridges.matt-pocock-skills` are
  replaced by `com.mattpocock/skills` and
  `https://github.com/vibespecs/com.mattpocock.skills`.
- Their canonical sources are now independent local Git repositories at
  ...
  `C:\Users\olegc\git\v\packages\com.mattpocock.skills`, not directories in
...
- The old `org.vibevm.bridges/spec-kit` and
  `org.vibevm.bridges/matt-pocock-skills` coordinates are absent. The corrected
  `org.speckit/speckit` and `com.mattpocock/skills` coordinates are present.
- Both packages record `Oleg Chirukhin` as the package author. Their upstream
  authors remain separate: `GitHub, Inc.` for Spec Kit and `Matt Pocock` for
