...
- R5.4-EPOCH-LOWER: registry `ab0ab90f`, workspace `d2b06561`, map
  `55595f1f` add origin-token opaque row indices and one neutral runtime per
  node/global package unit. One registry owns exact compile/native row subsets,
  plan, mechanisms and routes; borrowed overlap is pointer-identical, selected
  member Project/World is common and exact, and complete runtimes lower before
  boot publication. Boot, verify and analyzer consume retained plans; prepared
  regeneration returns the runtimes while legacy wrappers explicitly discard.
  Reviews rejected ABA/silent index loss, missing all-family/spec pins, weak
  owner-seat/routes/exact-once evidence, circular Ready/Fresh parity and an
  unwind-unsafe observer; all were corrected before final PASS. Gates pass
