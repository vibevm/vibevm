...
**Corpus:** `vibevm/vibepacks/org.vibevm.ai-native/typescript-ai-native-lang/v0.6.0/`.
...
@fact:B6-SAME-SKELETON `go-ai-native-lang` v0.1.0 — batch B5, 665 units, landed in
...
@fact:B6-DOCUMENT-MARKERS Document markers, measured off the marked go stack rather
...
@fact:B6-F092-IS-EXPECTED Both `SKILL.md` files open with YAML frontmatter. The
...
@fact:B6-F092-EXPECTED-RESIDUE **Expected end state for this batch: exactly one
...
@fact:B6-NO-IN-FILE-DODGE **Do not invent an in-file workaround.** A previous worker
...
@fact:B6-EIGHT-COMPACT-CARDS Eight cards (`a`, `b`, `c`, `e`, `f`, `g`, `h`, `i`)
...
@fact:B6-BAND-THREE-IS-CODE Band 3 is a fenced `card-ops` block in the eight compact
...
@fact:B6-EXPECT-VERSION-CLAIMS The guide states tool versions, ecosystem facts and
...
@fact:B6-EXPECT-HOST-ONLY-CITATION A document in this package cites a path that only
