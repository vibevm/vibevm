...
##B15-SEVEN-SIBLINGS B8 through B14 are landed `world` batches of this shape. Read the nearest
sibling's corresponding file before marking yours.

##B15-UMBRELLA-PRECEDENT **`git-practices` is a family aggregator and there is exactly one marked
...
##B15-COMPOSITION-STAGES **Ruling 37 governs the member entries**: a Composition entry takes the
...
##B15-COMPOSITION **Measured composition: 46 cells, 120 items, 134 paragraphs, 237
...
##B15-COUNTER-CLAUSES **The counting rule gained two clauses after B14 and both bite here.**
...
##B15-PREDICTION **Predicted: 419–438 units** (`1.07–1.15 × 237 + 120 + 46`). Six realised
...
##B15-CELL-DENSITY **One file is 67 % table cells** —
...
- ##B15-EXPECT-RESIDUAL **Residual: ZERO.** No `SKILL.md`, no frontmatter and no nested fence in
  scope — the two classes that produced residuals in this campaign are both absent.
- ##B15-EXPECT-RESIDUAL-FILES **No file may carry a residual.**
- ##B15-EXPECT-CORPUS-TOTAL **Corpus-wide unmarked afterwards: 232** — it stands at 532 and this
  batch owes all 300. That leaves exactly B16.

## This batch is F-097's epicentre — fourteen dead names, three of them titles {#f097}

##B15-F097-FOURTEEN-SITES **Three of the six packages here ARE the renamed ones**, and they still
...
##B15-F097-FOUR-BROKEN-COMMANDS **Four of those are command lines a consumer cannot run** — three
...
⚠️ ##B15-PATHS-ARE-NOT-NAMES **A path is not a name, and this batch is where that distinction
...
##B15-STILL-DO-NOT-FIX **Mark them, do not fix them, do not re-file.** Four names are dead
...
##B15-F103-ALL-FIVE **Four of the six packages keep their boot snippet at `boot/`, not
...
##B15-F103-FILED **Already filed and widened — do not re-file.** Ruling 7 is about the
...
##B15-KNOWN-F108 **F-108** — its `vibe.toml` declares **four** members and, three lines
...
closure pulls four.
##B15-KNOWN-F109 **F-109** — the same manifest cites `neworder2/memory/BACKLOG.md`, a path
that exists in this repository and in no consumer's.

##B15-KNOWN-DO-NOT-REFILE **Both are filed. Mark the README as it stands and do not re-file
...
##B15-ANCHORS-OWED Ruling 5 makes missing heading `{#anchor}`s part of what a batch owes, and
...
##B15-ANCHOR-SPACE Watch `##FACT-ID-GRAMMAR`: heading anchors and fact ids share **one
...
##B15-EXPECT-THE-FAMILY-CROSS-REFERENCES These four git packages describe each other constantly — the format
...
##B15-EXPECT-PROSE-HEAVY-PROTOCOLS `ATOMIC-COMMITS-PROTOCOL.md` and `splitting-large-changes.md` carry 46
...
##B15-EXPECT-TWO-SEGMENT-COLONS **Ruling 45's two-segment colon is still undecided and rulings 3, 20, 35,
