...
- **2026-08-29 (действует): двойная поверхность — нативные Opus-субагенты
  ...
  фоне утренней директивы того же дня «порождай субагентов Opus для
  реализации задач, но ограниченные (не читать весь boot lane)». Итого:
  нативные `opus5`-субагенты (глубокие/архитектурно-тяжёлые атомы) И лейн
  `claudez` до **5** параллельных воркеров (хорошо специфицированные,
  проверяемые пакеты; worktree-на-воркера). `claudez2` остаётся запрещённой;
  `codexrunner` сегодня не назван — вне действующего порядка до нового
  слова владельца.
- 2026-08-20 (снято 2026-08-29): `codexrunner` — приоритетный лейн;
  `claudez` — второй; `claudez2` — запрещён. Тогда же: модель
