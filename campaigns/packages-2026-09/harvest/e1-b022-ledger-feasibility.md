...
| M-D | release slice: exported, signed, shipped | yes | L, cross-crate | **B-015, parked by owner's word**; the spec itself forbids unsigned exposure (`LEDGER-INTENT-v0.1.xml:87`), so no unsigned interim exists | **no work until the owner's B-015 notice**; annotation names B-015 explicitly |
