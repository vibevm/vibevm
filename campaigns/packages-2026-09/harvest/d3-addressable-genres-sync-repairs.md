...
**Anchors:** 0 edited of 4 — `authoring-rules.md#ROW-BUDGET-BOOT-FILE`,
...
565 words  ~753 tok  packages/org.vibevm.world/secrets-hygiene/v0.1.0/spec/boot/57-flow-secrets-hygiene.md
506 words  ~674 tok  packages/org.vibevm.world/wal-specspaces/v0.1.0/spec/boot/11-flow-wal-specspaces.md
499 words  ~665 tok  packages/org.vibevm.world/decision-records/v0.1.0/spec/boot/25-flow-decision-records.md

$ w=$(wc -w < packages/org.vibevm.world/addressable-specs/v0.1.0/spec/boot/15-flow-addressable-specs.md); echo "$w words ~$((w*4/3)) tok"
360 words ~480 tok

$ lane=$(cat spec/boot/STATIC.md spec/boot/00-core.md spec/boot/90-user.md spec/boot/INDEX.md | wc -w)
...
(`spec/boot/STATIC.md` + `00-core.md` + `90-user.md` + `INDEX.md`), `spec/WAL.md`,
...
the boot lane (~32× over), `spec/WAL.md` and the nine oversized specs are over
...
collection — here at `authoring-rules.md#SUM-THE-SIZE-BUDGETS` and again at
`two-process-model/…/files-as-ipc.md#ROW-PLANE-CONTROL` (F-347) — with nothing
...
`spec-tree-layout.md#HARNESS-ENTRY-FILES-ARE-THIN-REDIRECTS`
**Files touched:** none
**Re-verification:**
...
`spec/boot/STATIC.md`, not a single file.
...
tree at `spec-tree-layout.md:19` names the same file the rule points at, and
...
```console
$ grep -h "^#" spec/boot/00-core.md spec/boot/90-user.md spec/boot/INDEX.md | grep -c "{#"
0
$ grep -c "{#" packages/org.vibevm.world/addressable-specs/v0.1.0/spec/boot/15-flow-addressable-specs.md
7
```

**Who falsifies it:** the host — its own `spec/boot/` and `spec/WAL.md` headings
are unanchored; the package's boot snippet stating the rule is fully anchored.
**Perimeter searched:** `spec/boot/00-core.md`, `spec/boot/90-user.md`,
...
anchor the 23 headings in `spec/boot/` and the 8 in `spec/WAL.md`.
...
**Outcome:** OUT-OF-ROUTE
**Anchors:** 0 edited of 1 — `spec-tree-layout.md#ROW-HOME-SPEC-PROTOCOL`
**Files touched:** none
**Re-verification:**
...
flows under `spec/flows/<flow>/`, compiled into `spec/boot/STATIC.md`.
...
`spec-tree-layout.md:21`, so the package does not contradict itself. The
...
`SPEC-GENRES-PROTOCOL.md#GENRE-TYPING-REMOVES-ALL-THREE`,
`SPEC-GENRES-PROTOCOL.md#PLACE-DO-NOT-BLEND`
**Files touched:** none
**Re-verification:**

```console
$ grep -c "MUST" spec/design/structural-loader.md
2
$ grep -n "idx-structural-loader" spec/design/README.md
50:- ##idx-structural-loader [Structural loader](structural-loader.md) — provisional loader instructions held for PROP-035; not yet wired into any live boot. @spec/hold
```
...
Host obligation: `spec/design/structural-loader.md` is half requirement and half
...
**Outcome:** OUT-OF-ROUTE
**Anchors:** 0 edited of 2 —
`design-docs.md#RECORD-EACH-FORK-AS-A-FIXED-SKELETON`,
`design-docs.md#SUM-RECORD-FORKS-AS-A-SKELETON`
**Files touched:** none
**Re-verification:**

```console
$ grep -rno "Consequence" spec/design/
spec/design/loading-and-boot-model.md:196:Consequence
$ grep -o "Chosen:" spec/design/workspace-and-qualified-naming.md | wc -l
13
```
...
`spec/design/loading-and-boot-model.md` and
`spec/design/workspace-and-qualified-naming.md` carry no Rejected or Consequence
lines.
...
**Outcome:** OUT-OF-ROUTE
**Anchors:** 0 edited of 2 —
`when-to-write-what.md#MOST-DECISIONS-PRODUCE-BOTH`,
`when-to-write-what.md#SUM-A-DECISION-PRODUCES-BOTH`
**Files touched:** none
**Re-verification:**
...
SPEC-GENRES-PROTOCOL.md
design-docs.md
when-to-write-what.md
...
**Outcome:** OUT-OF-ROUTE
**Anchors:** 0 edited of 1 —
`SPEC-GENRES-PROTOCOL.md#AN-UNLINKED-DESIGN-DOC-IS-A-DEFECT`
**Files touched:** none
**Re-verification:**

```console
$ grep -rn "structural-loader" spec/terraforms/SPEC-ACTUALIZATION-CAMPAIGN-v0.1.md BACKLOG.md
spec/terraforms/SPEC-ACTUALIZATION-CAMPAIGN-v0.1.md:770:  design↔PROP backlinks hold 4/4 (structural-loader parked by its own
$ grep -c "PROP-035" spec/design/structural-loader.md
3
$ grep -c "structural-loader" spec/modules/vibe-workspace/PROP-035-spec-compiler.md
0
```

**Who falsifies it:** the host — its record classifies the instance as parked,
where the flow classifies it as a defect.
**Perimeter searched:** `spec/terraforms/SPEC-ACTUALIZATION-CAMPAIGN-v0.1.md`
...
the doc is unlinked in both directions. It is not. `PROP-035-spec-compiler.md`
**exists** at `spec/modules/vibe-workspace/`, 37 KB, and
`spec/design/structural-loader.md` names it three times — its own status line
says «DESIGN — provisional (PROP-035 §13)». What is missing is only the return
leg: PROP-035's `##related` header at :8 links
`spec/design/loading-and-boot-model.md` and never mentions `structural-loader`.
...
`SPEC-GENRES-PROTOCOL.md#A-ONE-DIRECTIONAL-LINK-IS-A-LATENT-BREAK` (`confirmed`
in Phase C on this same file). Host obligation: add the return leg to
`spec/modules/vibe-workspace/PROP-035-spec-compiler.md`'s `##related` line — one
...
**Outcome:** OUT-OF-ROUTE
**Anchors:** 0 edited of 2 —
`SYNC-PROTOCOL.md#PART-THE-REVISIT-TRIGGER`,
`SYNC-PROTOCOL.md#SUM-OUTPUT-IS-VALUE-REASON-AND-REVISIT-TRIGGER`
**Files touched:** none
**Re-verification:**
...
the rule the sibling `review-workflow.md#NO-TRIGGER-MEANS-NO-AUDIT-PATH` exists
...
`when-to-apply.md#BOUNDARY-FLOW-ATOMIC-COMMITS`; **not edited**
`when-to-apply.md#BOUNDARY-VIBE-BUILD`
**Files touched:**
`packages/org.vibevm.world/sync-from-code/v0.1.0/spec/flows/sync-from-code/when-to-apply.md`
**Re-verification:**

```console
$ grep -n "CONVENTIONAL-COMMITS-IS-THE-FORMAT-THIS-FLOW-IS-THE-ATOMICITY" packages/org.vibevm.world/git-atomic-commits/v0.1.0/spec/boot/30-flow-atomic-commits.md
...
$ diff <(git show HEAD:packages/org.vibevm.world/sync-from-code/v0.1.0/spec/flows/sync-from-code/when-to-apply.md | grep -o '##[A-Za-z0-9-]*' | sort) <(grep -o '##[A-Za-z0-9-]*' packages/org.vibevm.world/sync-from-code/v0.1.0/spec/flows/sync-from-code/when-to-apply.md | sort) && echo IDENTICAL
IDENTICAL
```
...
`git-atomic-commits` states at its own `30-flow-atomic-commits.md:23-26` that
...
(`SYNC-PROTOCOL.md:171`), and `docs(spec): sync timeout into PROP-003
§verification.timeout` (`review-workflow.md:119`). I did not repair it, for two
reasons. The registry's own falsifiers for this row (`SYNC-PROTOCOL.md:165`,
...
`git-conventional-commits/…/conventional-commits.md:169` carries the identical
...
`git-conventional-commits/…/conventional-commits.md:169`) — a `duplication`
...
**Outcome:** OUT-OF-ROUTE
**Anchors:** 0 edited of 1 —
`review-workflow.md#APPROVAL-STEP-COMMITS-WITH-CONVENTIONAL-COMMITS`
**Files touched:** none
**Re-verification:**
...
`cognitive-load-split.md#TEXT-THAT-WORKS-FOR-THE-AI-WORKS-FOR-THE-OTHER-TWO`
**Files touched:**
`packages/org.vibevm.world/two-process-model/v0.1.0/spec/flows/two-process-model/cognitive-load-split.md`
**Re-verification:**

```console
$ grep -n "THE-REPORT-IS-FOR-THE-HUMAN\|they-serve-different-readers" packages/org.vibevm.world/wal/v0.2.0/spec/flows/wal/session-end-hook.md
...
$ grep -n "READ-THE-END-OF-SESSION-REPORT-EVERY-TIME" packages/org.vibevm.world/two-process-model/v0.1.0/spec/flows/two-process-model/files-as-ipc.md
67:##READ-THE-END-OF-SESSION-REPORT-EVERY-TIME It is structured input for the next decision

$ diff <(git show HEAD:packages/org.vibevm.world/two-process-model/v0.1.0/spec/flows/two-process-model/cognitive-load-split.md | grep -o '##[A-Za-z0-9-]*' | sort) <(grep -o '##[A-Za-z0-9-]*' packages/org.vibevm.world/two-process-model/v0.1.0/spec/flows/two-process-model/cognitive-load-split.md | sort) && echo IDENTICAL
IDENTICAL
```

**Who falsifies it:** the document itself — a sibling package in `packages/`
says the opposite in its own words, and this package's own `files-as-ipc.md`
...
believe it: `wal/…/session-end-hook.md:140,145` specifies a human-scan report
*and* a machine-read WAL for the same state and says «They serve different
readers», and this package's own `files-as-ipc.md:60,64-68` puts the checkpoint
...
**Outcome:** OUT-OF-ROUTE
**Anchors:** 0 edited of 1 — `files-as-ipc.md#ROW-PLANE-CONTROL`
**Files touched:** none
**Re-verification:**

```console
$ wc -w spec/boot/STATIC.md spec/boot/00-core.md spec/boot/90-user.md spec/WAL.md
  9860 spec/boot/STATIC.md
   827 spec/boot/00-core.md
  1282 spec/boot/90-user.md
  3296 spec/WAL.md
 15265 total
```
...
`spec/boot/INDEX.md` to the four files above), and `spec/WAL.md` is ~4 395
...
`addressable-specs/…/authoring-rules.md#SUM-THE-SIZE-BUDGETS` author the same
...
**Files touched:**
`packages/org.vibevm.world/wal-specspaces/v0.1.0/spec/boot/11-flow-wal-specspaces.md`
**Re-verification:**

```console
$ grep -n "^- ##FIELD-" packages/org.vibevm.world/wal-specspaces/v0.1.0/spec/flows/wal-specspaces/SPECSPACES-PROTOCOL.md
...
$ sed -n '55,57p' packages/org.vibevm.world/wal-specspaces/v0.1.0/spec/flows/wal-specspaces/SPECSPACES-PROTOCOL.md
...
$ diff <(git show HEAD:packages/org.vibevm.world/wal-specspaces/v0.1.0/spec/boot/11-flow-wal-specspaces.md | grep -o '##[A-Za-z0-9-]*' | sort) <(grep -o '##[A-Za-z0-9-]*' packages/org.vibevm.world/wal-specspaces/v0.1.0/spec/boot/11-flow-wal-specspaces.md | sort) && echo IDENTICAL
IDENTICAL
```
...
**Outcome:** OUT-OF-ROUTE
**Anchors:** 0 edited of 1 — `SPECSPACES-PROTOCOL.md#SUM-THE-REGISTRY`
**Files touched:** none
**Re-verification:**
