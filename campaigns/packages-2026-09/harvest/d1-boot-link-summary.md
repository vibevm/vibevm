...
- **Counts in `vibevm/vibespecs/boot/STATIC.xml`:** 69 `](../flows/` links · 41 `spec/flows/`
...
- **Sharpest fact:** the link is *correct in the installed slot* for 22 of 27
  ...
  Concatenation into `vibevm/vibespecs/boot/STATIC.xml` is what severs it. The other 5
...
- **What binds a decision:** PROP-009 §2.1 already obliges the packages —
  *"a package's internal cross-references must become package-relative or
  `spec://` URIs"* (`vibevm/vibespecs/modules/vibe-workspace/PROP-009-loading-model.xml:41`),
