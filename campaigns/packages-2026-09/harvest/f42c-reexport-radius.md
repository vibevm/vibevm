...
> **ПОПРАВКА 2026-08-17 — препятствий не четыре, а ПЯТЬ, и пятое главнее всех
> перечисленных.** Этот замер шёл по типам и не спросил, СКОЛЬКО РАЗ каждый из
> них эмитирован. Ответ: словник подставляется в каждую схему, которая его
> тянет, поэтому `VersionEntry` существует в трёх сгенерированных модулях,
> `PackageKind` — в семи, `NamingConvention` — в двух; 102 объявления против 58
> имён. Реэкспорт невозможен не «пока не решены A–D», а **ни при каком выборе
> модуля-источника**, и лечится это только дедупликацией выхода (шаг Ф4.2c-3a).
> Разбор — [`f42c3-generated-duplication.md`](f42c3-generated-duplication.md);
> измеренный компилятором радиус — [`f42c3-blast-radius.md`](f42c3-blast-radius.md).
> Урок, который этот файл оплатил собой: **чтение здесь систематически
> недосчитывает**, и там, где ответ обязан быть полным, спрашивают компилятор.
...
> **ПОПРАВКА 2026-08-17 — числа этого раздела УСТАРЕЛИ от нашей же посадки, и
> следующий читатель не должен по ним строить.** Замер снят на `9c27a1ff`, до
> шага Ф4.2c-1; тот шаг завёл безусловный трейт-этаж, и сгенерированная сторона
> теперь выводит `Debug, Clone, PartialEq, Eq, Serialize, Deserialize` — **74
> строки полного этажа** (перемерено 2026-08-17:
> `grep -rho "^#\[derive([^]]*)\]" crates/vibe-wire/src/generated/ | sort | uniq -c`
> даёт 74 полных, 11 бессерде-этажей у открытых словарей, 2 реестровых).
> Значит **`Debug`, `Clone`, `PartialEq`, `Eq` НЕ теряются**, и препятствие (A)
> закрыто наполовину: теряются ровно `Copy`, `PartialOrd`, `Ord`, `Hash`,
> `Default`, `ValueEnum`.
>
> И то, что этот раздел оценивал ЧТЕНИЕМ, измерено компилятором в
> [`f42c3-blast-radius.md`](f42c3-blast-radius.md): 48 сайтов `::default()` в
> крейте (15 на теряющих `Default`), **19** сайтов ломаются от потери `Copy` —
> тогда как греп по сигнатурам видел три; а `Ord`/`Hash` не нужны **НИ ОДНОМУ**
> сайту, хотя `scanner/manifest.rs:14` называет `Ord` причиной, по которой
> словарь вообще продублирован.
>
> Главного эта находка тоже не увидела: сгенерированный тип существует в
> НЕСКОЛЬКИХ модулях сразу (словник подставляется в каждую втянувшую схему), и
> потому реэкспорт невозможен ни при каком выборе модуля-источника — разбор в
> [`f42c3-generated-duplication.md`](f42c3-generated-duplication.md). Числа
> главнее текста; где строка здесь и вывод команды разошлись, верна команда.
