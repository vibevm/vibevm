...
- **Звуковое правило класса A.** Идентификаторы события существуют только в
  ...
  (`spec/boot/STATIC.md`, qualify-таблица коротких имён), а формы
...
| C10 | **ПОДТВЕРЖДЕНО** | `spec/modules/vibe-index/PROP-005-package-index.md:971` (внутри §2.18, открывающегося :957 `### 2.18 Channels — author-named version pointers {#channels}`) — «`Renamed`, `Notice` and `ForceReplaced` stand in the same place for the same reason.» — про отказ проектора из-за непостроенного носителя. |
...
| `spec/modules/vibe-index/PROP-005-package-index.md` | 587, 589–593 | спек-проза (блок решений о коллапсе) | правки не нужны (текст постановки, не описание текущего словаря) | это сами факты-решения, executer сворачивает их исполнением; :593 — обязанность того же коммита |
| `spec/modules/vibe-index/PROP-005-package-index.md` | 971 | спек-проза (предложение §2.18 об отказе) | переписать | обязано тем же коммитом (PROP-005:593): перестаёт быть верным в обеих половинах |
| `spec/modules/vibe-index/PROP-005-package-index.md` | 237 | спек-проза (рабочий пример надгробия — переименование) | правки не нужно | пример — модель нового факта (reason + superseded_by), решение на него опирается (PROP-005:590) |
...
| `spec/WAL.md` | 50, 87 | состояние работ (WAL) | правки не нужно | живой журнал сессии |
...
1. `spec/common/PROP-044-change-native-formats.md:146` — «(3) the **registry facts journal** — append-only records of what sources cannot carry: publication, yank, rename, removal, ownership, security notice;»
