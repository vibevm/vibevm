...
Спек-кандидат №1 (`spec/common/PROP-044-change-native-formats.md`) прочитан полностью,
...
- **394: Р1** — `spec/common/PROP-044-change-native-formats.md`, секция `{#laws}`,
...
- Grep `поимённо|литерал|одиннадцат` по `spec/` → 2 попадания, оба нерелевантны
  (`spec/research/schema-evolution-2026-08/01-measure-our-wire-format-claudez.md:57` —
  про rename-варианты `NamingConvention`; `12-HANDOFF.md:221` — про пустые файлы
