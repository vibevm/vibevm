...
- **`two-process-model` is the corpus's only WORDS-DIFFER case in this batch, and
  its reason is not the one this capture first recorded.** The join reports three
  words present in the package and absent from the host — `architecture`,
  `consequences`, `never` — and reading them as missing *rule* words is wrong. They
  are the `{#…}` suffixes on three headings:
  ...
  **Every rule's prose is word-identical; what the host is missing is the ability to
  cite three of its four sections.** The cause is staleness, traced by `git log`:
  `54499a6c` (2026-07-27) added all three anchors, the `vibedeps/` slot was last
  written by `1836a42b` (2026-07-14) and `STATIC.md` regenerated by `1018d971`
  (2026-07-26) — both before. Cross-checked against the alternative explanation, that
  the boot compiler strips anchors: it does not, the same lane's addressable-specs
  section still carries its `{#single-source}` and `{#placement}` anchors, and this
  snippet's own `{#root}` survives.

  *This correction is the third time this session a reason in the record had to be
  fixed rather than a verdict. The first framing would have sent the next reader
