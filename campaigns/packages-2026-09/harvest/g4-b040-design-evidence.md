# G4 — B-040 design evidence: `vibevm/vibespecs/design/typed-seams.xml` against the built tree
...
**Perimeter read.** Read in full: `vibevm/vibespecs/design/typed-seams.xml` (90 lines), the census
...
`vibevm/vibespecs/boot/90-user.xml` (SCOPE-DISCIPLINE), `spec/modules/vibe-registry/PROP-002-…md`,
...
- **evidence.** `BACKLOG.md:1038` (`### B-040 — рефакторинг-обзор собственных
  ...
  the companion statement itself at `vibevm/vibespecs/design/typed-seams.xml:5`; GUIDE
  section `- ##ln **B — Typed builders / typestate** (`scaffold-b-typed-builders`)`
  in `vibevm/vibepacks/org.vibevm.ai-native/rust-ai-native-lang/v0.7.0/vibevm/vibespecs/rust/GUIDE-AI-NATIVE-RUST.xml`.
- **match.** SUPPORTS
...
- **evidence.** The fact itself at `vibevm/vibespecs/design/typed-seams.xml:11`; the
...
- **evidence.** The fact at `vibevm/vibespecs/design/typed-seams.xml:13`; the four sites
...
- **claim.** The rule is not stylistic — `##SCOPE-DISCIPLINE` and PROP-002
  ...
  its first real run.
- **evidence.** `vibevm/vibespecs/boot/90-user.xml:28` (`##SCOPE-DISCIPLINE` … "Never
...
- **evidence.** The decision is recorded in §7 of the design
  (`vibevm/vibespecs/design/typed-seams.xml:71-81`); the GUIDE scaffold-B section is
...
- **evidence.** `sync-engines.toml`: every `[[sync]]` `source_root` is under
  `vibevm/vibepacks/org.vibevm.ai-native/`, and every `crates` entry is a
...
- **note.** SUPPORTS (point C): the conclusion follows because every
  ...
  `vibevm/vibepacks/org.vibevm.ai-native/` (the neutral engines
