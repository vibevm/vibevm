...
**Files touched:** `vibevm/vibepacks/org.vibevm.world/wal/v0.2.0/vibevm/vibespecs/flows/wal/WAL-PROTOCOL.xml`
...
```
$ wc -lwc vibevm/vibespecs/WAL.xml
  344  3296 22006 vibevm/vibespecs/WAL.xml          (verdict measured 299 / 2 914 / 18 972)

$ sed -n '1,4p' vibevm/vibespecs/WAL.xml
...
$ for c in $(git log -n 14 --format=%h -- vibevm/vibespecs/WAL.xml); do git show "$c:vibevm/vibespecs/WAL.xml" | sed -n '3p'; done
...
$ grep -n '^## ' vibevm/vibespecs/WAL.xml
...
$ sed -n '80,182p' vibevm/vibespecs/WAL.xml | grep -cE '^- '                      → 28
$ sed -n '80,182p' vibevm/vibespecs/WAL.xml | grep -E '^- ' | grep -cE 'spec://|#[0-9]+'  → 0
...
**New obligations noticed:** the host's `vibevm/vibespecs/WAL.xml:341` now contains prose
...
$ git log --since=2026-06-01 --date=short --format=%ad -- vibevm/vibespecs/WAL.xml | sort -u | wc -l → 29
...
```
$ for the 18 most recent commits touching vibevm/vibespecs/WAL.xml, compare line 3 pairwise
...
```
$ grep -n 'Read `vibevm/vibespecs/WAL.xml` \*\*before\*\*' vibevm/vibespecs/boot/STATIC.xml   → 1382
$ wc -l vibevm/vibespecs/boot/STATIC.xml                                          → 1584
```
...
**Files touched:** `vibevm/vibepacks/org.vibevm.world/wal/v0.2.0/README.md`
...
```
$ ls vibevm/vibepacks/org.vibevm.ai-native/core-ai-native/
v0.7.0
v0.8.0

$ ls -la vibevm/vibepacks/org.vibevm.ai-native/core-ai-native/v0.8.0/vibevm/vibespecs/06-WAL-CONVENTION.xml
...
```
$ ls vibevm/vibepacks/org.vibevm.world/redbook/v0.2.0/vibevm/vibespecs/book/ru/
...
They are: `ROADMAP.md` and `vibevm/vibespecs/modules/**` using `flow:wal` as the worked
package-manager *example* / e2e fixture (`vibevm/vibespecs/common/PROP-000.xml:92-94,105,173`,
`spec/modules/vibe-registry/PROP-002…:69,199,219`,
`spec/modules/vibe-index/PROP-005…:19,308`,
`vibevm/vibespecs/design/workspace-and-qualified-naming.xml:62,76,81,82`); `vibe.lock:62`
and `vibevm/vibespecs/boot/STATIC.xml:1369` recording that it is *installed and compiled in*;
...
- **`PACKAGE-IS-THE-CANONICAL-HOME-OF-THE-WAL-CONVENTION` did not move, and is
  ...
  and booted** (`vibe.lock:62`; `vibevm/vibespecs/boot/STATIC.xml:1369`). Making that
...
`vibevm/vibespecs/boot/STATIC.xml:1369` compiles this package's snippet, and `core-ai-native`
...
`vibevm/vibespecs/boot/` in filename order, then `vibevm/vibespecs/WAL.xml`, then any relevant PROP/FEAT
documents under `vibevm/vibespecs/common/` and `vibevm/vibespecs/modules/` for the task at hand. Only
after that, start work.

$ grep -n 'Run the full boot sequence' CLAUDE.md
205:1. Run the full boot sequence (this file → `vibevm/vibespecs/boot/INDEX.md` and its files →
    `vibevm/vibespecs/WAL.xml`), read `CONTINUE.md`, and verify repository state empirically…
```
...
path, and it does reverse the order: boot sequence ending at `vibevm/vibespecs/WAL.xml`,
...
```
$ grep -rn 'CONTINUE' CLAUDE.md vibevm/vibespecs/boot/*.md | grep -v STATIC.md
...
$ grep -c 'CONTINUE' vibevm/vibespecs/boot/STATIC.xml   → 4   (this package's own snippet)
```
...
`CONTINUE.md` and `vibevm/vibespecs/WAL.xml`»* — same order, no rival list. The disagreement
is host-side only.
...
:47  ## 2. Rewrite `vibevm/vibespecs/WAL.xml`
...
:90  2. STEP-REWRITE-THE-WAL               Rewrite `vibevm/vibespecs/WAL.xml`
...
*«confirm a good stopping state, rewrite `vibevm/vibespecs/WAL.xml`, overwrite
...
$ sed -n '3p' CLAUDE.md
Every session in this repository begins by reading this file, then every file in
`vibevm/vibespecs/boot/` in filename order, then `vibevm/vibespecs/WAL.xml`, …

$ sed -n '9,13p' vibevm/vibespecs/boot/00-core.xml
##boot-sequence-lead Every session starts here. In order:
1. ##BOOT-STEP-BOOT-DIR Read this file and the rest of `vibevm/vibespecs/boot/` end to end …
2. ##BOOT-STEP-WAL Read `vibevm/vibespecs/WAL.xml` — current project state (checkpoint, not log).
...
looser reading — *before doing any work*, which is how `vibevm/vibespecs/boot/00-core.xml`'s
...
**Files touched:**
`vibevm/vibepacks/org.vibevm.world/health-audit/v0.1.0/vibevm/vibespecs/flows/health-audit/HEALTH-AUDIT-PROTOCOL.xml`
...
```
$ sed -n '20,22p' vibevm/vibespecs/common/PROP-013-periodic-health-audit.xml
...
$ grep -rn 'debt.json\|DEBT.md' vibevm/vibepacks/org.vibevm.world/health-audit/v0.1.0/
  → zero occurrences
```
...
**Files touched:**
`vibevm/vibepacks/org.vibevm.world/health-audit/v0.1.0/vibevm/vibespecs/flows/health-audit/audit-checklist.xml`

**Re-verification.** Which of the three runs applied which growth rule:

```
$ grep -n 'HIST-CATEGORY-E' vibevm/vibespecs/common/PROP-013-periodic-health-audit.xml
...
`vibevm/vibespecs/common/PROP-013-periodic-health-audit.xml:63` (`##CADENCE-FLOOR`, *«a vibevm
...
`./manual-tests`, `./vibevm/vibespecs/manual-tests`, and
`./vibevm/vibepacks/org.vibevm.fractality/fractality/v0.1.0/vibevm/vibespecs/manual-tests`. The other
...
**two** — `manual-tests/` and `vibevm/vibespecs/manual-tests/` — and only the second is the
...
| `MANUAL-TESTS-LIVE-IN-A-DEDICATED-DIRECTORY-AT-THE-REPOSITORY-ROOT` | one directory, at the root | a second home at `vibevm/vibespecs/manual-tests/` |
...
— `vibevm/vibespecs/common/PROP-000.xml:187` (`##MT-LOCATION`, the root directory and the
...
test»* — a one-row host fix. (b) `vibevm/vibespecs/common/PROP-000.xml` `##MT-LOCATION`
records only the root home, so the eleven tests' second home at
`vibevm/vibespecs/manual-tests/` sits outside the decision that governs the tier. Both
...
$ grep -nE '^#{2,3} ' vibevm/vibespecs/manual-tests/MT-02-vibe-tree-tui.xml vibevm/vibespecs/manual-tests/MT-03-vibe-prefs-tui.xml
...
```
$ grep -n 'MT-ISOLATION' vibevm/vibespecs/common/PROP-000.xml
...
`vibevm/vibespecs/common/PROP-000.xml` `##MT-ISOLATION` and
...
```
$ for f in manual-tests/*.md vibevm/vibespecs/manual-tests/*.md; do echo "$f $(grep -c 'Expected' "$f")"; done
...
vibevm/vibespecs/manual-tests/MT-01-vibe-tree.xml             10
vibevm/vibespecs/manual-tests/MT-02-vibe-tree-tui.xml          0
vibevm/vibespecs/manual-tests/MT-03-vibe-prefs-tui.xml         0
```
...
$ grep -rniE 'manual[- ]test|MT-0[123]|M2\.10' \
    ...
    discipline/ vibevm/vibespecs/common/ vibevm/vibespecs/modules/ vibevm/vibespecs/design/ \
...
3. **The convention being specified** — `vibevm/vibespecs/common/PROP-000.xml:187-202`.
...
`vibevm/vibespecs/common/PROP-000.xml:202` `##MT-WAL-NAMES` records that *«MT-02 and MT-03
...
```
vibevm/vibepacks/org.vibevm.world/wal/v0.2.0/README.md                                   +4 -3
vibevm/vibepacks/org.vibevm.world/wal/v0.2.0/vibevm/vibespecs/flows/wal/WAL-PROTOCOL.xml              +2 -2
vibevm/vibepacks/org.vibevm.world/health-audit/v0.1.0/vibevm/vibespecs/flows/health-audit/HEALTH-AUDIT-PROTOCOL.xml  +5 -3
vibevm/vibepacks/org.vibevm.world/health-audit/v0.1.0/vibevm/vibespecs/flows/health-audit/audit-checklist.xml        +1 -1
```
...
`vibevm/vibepacks/org.vibevm.world/campaign-plans/` and
...
4. **F-352 turns on a reading, not a measurement.** Strictly, *«read the WAL
   ...
   loosely — the reading `vibevm/vibespecs/boot/00-core.xml`'s own step 4 uses — the host
