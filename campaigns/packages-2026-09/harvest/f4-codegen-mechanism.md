...
> **ПОПРАВКА 2026-08-17 — «расхождение версий» ЛОЖНО, и следующий читатель не
> должен по нему строить.** Пиненный бинарь отвечает `jtd-codegen 0.4.1`, и ОН
> ЖЕ пишет в шапку `// Code generated by jtd-codegen for Rust v0.2.1` —
> проверено прямо, прогоном этого бинаря по пробной схеме. Строка шапки
> называет версию **Rust-таргета**, а не инструмента, поэтому две величины
> сравнивались как одна. Расхождения нет; зелёный `check-codegen` утверждает
> сильнее — закоммиченные артефакты побайтово равны сегодняшней эмиссии
> пиненного бинаря. Отсутствие гейта на версию при этом верно и НАМЕРЕННО:
> дом пина запрещает пересказывать число в дереве потребителя и прямо
> объявляет, что CI не принуждает конкретную сборку генератора. Что из этого
> пункта осталось работой и сделано — двупричинный отказ `check-codegen`
> (коммит `e8d8238f`).
