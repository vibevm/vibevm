...
4. **Пин генератора не сведён с артефактами — видно в дереве.**
   *(**ПОПРАВКА 2026-08-17: утверждение ЛОЖНО.** Пиненный бинарь отвечает
   `jtd-codegen 0.4.1` и ОН ЖЕ пишет в шапку `for Rust v0.2.1` — проверено
   прогоном; шапка называет версию Rust-ТАРГЕТА, не инструмента. Расхождения
   нет. Разбор целиком — в плане, поправка к Ф4.2 (vi).)*
   Шапки
