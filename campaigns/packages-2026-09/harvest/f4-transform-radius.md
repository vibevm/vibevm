...
3. *Wire не выводится из имени:* среди **полей** таких нет (проверка выше —
   ноль исключений); невыводимость живёт у **вариантов**: `KindName` →
   `"kind-name"` и `KindName0` → `"kind/name"` (`repomd/mod.rs:60-64`),
   `LazyPull` → `"lazy-pull"`. Их rename обязателен при любой форме.

**Общее правило:** №2 делает избыточными **все 309 полевых** `#[serde(rename)]`
