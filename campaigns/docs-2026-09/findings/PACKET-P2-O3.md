...
**Индекс git общий с параллельными воркерами.** Коммить только свои файлы
явной формой `git commit -m "<subject>" -m "<why>" -- <свои пути>`; никогда
`git commit` без путей и никогда `git add -A`. Гейты — с приватным каталогом
сборки `CARGO_TARGET_DIR=<scratch>\target-p2o3`, чтобы не ждать чужих
блокировок `target/debug/vibe.exe`.
