...
## Docker: доступен (проверено центральной сессией 2026-09-12, вечер)

Docker Desktop 4.60.1, движок 29.2.0 (linux/amd64), `docker run --rm
hello-world` прошёл. Все шаги пакета со сборкой образов и локальной пробой
в контейнерах выполняются как написано ниже. Сверх них — **статические
тесты** остаются обязательным гейтом: node-тест web-пакета над текстом
`nginx.conf` и `compose.yaml` (каждое требование A5.7 — правило в файле:
`/.vibe-site/` → 404, `q-manifest.json` → 404, immutable-кеш на
`/assets/`, `/build/`, `/fonts/` и не на `/_astro/`, `charset=utf-8` на
`.md`/`.xml`/`.txt`/`.json`, относительные редиректы `/doc` → `/doc/` и
`/en/` → `/`, `include csp.conf`, ни одного `Location: http://`,
плейсхолдеры имён и порта), и генератор `csp.conf` из `csp.txt` — как
скрипт над реальным `csp.txt` сборки. Если демон в ходе работы перестанет
отвечать — не чини Docker и не перезапускай его: доделай статическую
половину, а контейнерные шаги отметь в отчёте как не выполненные с
дословной ошибкой. Ничего на сервере (R-24): образы строятся и
запускаются только локально, `docker compose down` в конце, образы и
контейнеры свои — удалить, чужие не трогать.
