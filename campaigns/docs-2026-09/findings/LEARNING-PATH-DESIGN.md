...
| D | Проверки | ошибки `vibe check`: страница пути существует, стоит ровно один раз, все страницы пакета покрыты, `id` глав уникальны, перевод не объявляет страниц глав; отчёт `vibe doc check`: ссылки вперёд по пути вне приложения — измеряются, не блокируют | гейт на ссылки вперёд — ложные тревоги: страницы-указатели законно смотрят вперёд (все 18 таких ссылок пути — с двух ориентирующих страниц) |
...
|---|---|---|
| Ссылок вперёд вне приложения | 18 (без ссылок внутри папки) | 18, все — с двух ориентирующих страниц (замер `vibe doc check --chapters`; первый набросок скрипта не видел ссылок внутри одной папки и насчитал 13) |
