...
## Хвост от P2-O3 (гейт A2.5, отдельный коммит `feat(check): …`)

Отчёт P2-O3 оставил два незакрытых правила гейта: (1) непустой
`[i18n].available` у пакета вида `doc` — ошибка (`LOC-LANGUAGE-FIELD`:
отдельного поля языка нет, один пакет — один язык); (2) при `vibe check
--path <пакет-перевод>` источник перевода ищется и в проекте-родителе
(in-tree слот `vibevm/vibepacks/<группа>/<имя>/v<версия>` того же дерева),
а не только в store — иначе предупреждение «источник недоступен» ложное для
разработчика перевода.
