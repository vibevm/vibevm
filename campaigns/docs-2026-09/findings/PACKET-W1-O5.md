...
**Второй коммит.** Колонка «Contents» (W1-O6, `site/src/lib/contents.ts`) подписывает страницы заголовками источника даже там, где адаптация несёт страницу со своим заголовком (`titles` строится из `source.pages`, тогда как `pageCards` в `lib/view.ts` разрешает по изданию): на русской странице колонка читает «Every block once» над «Каждый блок по разу» (см. `findings/W1-O3B-shots/p-contents-ru-1440-light.png` и отчёт W1-O3B). Заголовок в колонке — из издания читателя, источник — только когда страницы в адаптации нет; тест на это.
Коммит: `fix(web): label the contents column in the reader's edition`.
...
Два коммита; восемь видов узнаются на карточках в обеих темах; неизвестный вид не ломает карточку;
