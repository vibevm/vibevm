...
7. Правила репозитория: Conventional Commits с телом «почему»; атом —
   ...
   **общем** `target/` репозитория (тёплый; ты единственный Rust-воркер, приватный каталог не нужен); удалять нечего в
...
## Хвост 0 — красный доктест из P2-O10 (первым коммитом)

`cargo test -p vibe-doc --doc lock_fields` красный на HEAD:
`crates/vibe-doc/src/surface/fields.rs:64` — `assertion failed: fields.iter().any(|f| f == "package")`.
Диагноз P4-O4 (отчёт, А-2): у `vibe_core::manifest::Lockfile` член `meta`
обязателен, и serde отвечает «missing field `meta`» раньше «unknown field»,
так что зонд `accepted::<Lockfile>` на верхнем уровне не получает списка
имён. Почини обход так, чтобы обязательные члены не прятали список
(зонд с заполненными обязательными полями или разбор обеих форм отказа),
тест на этот случай, доктест зелёный, `vibe doc surface` по руководству
даёт те же 55 полей lock-файла (число — в отчёт). Коммит:
`fix(doc): enumerate lock-file fields past the required meta table`.

## Параллельно: P4-O6 в web-пакете

Одновременно с тобой воркер P4-O6 делает библиотеку страниц web-пакета
data-driven: статическая сборка рендерит **каждое** издание из деревьев
`VIBE_DOC_OUT` (манифест + остров каждой страницы из `<страница>/index.html`),
а не фикстуру. До его посадки сборка сайта над настоящими деревьями
показывает только фикстурные страницы плюс скопированные поверхности —
это не твой дефект. A5.3 сдаёшь так: конвейерная половина (`vibe doc
build` уровня 0 и 1 в деревья, очередь, «render failed») — целиком; вызов
сборки сайта — с проверкой инварианта по фикстуре; когда в `git log`
появится `feat(web): render every edition from the documentation trees`,
повтори живой прогон над руководством и запиши числа страниц. Web-пакет
по-прежнему не править.
