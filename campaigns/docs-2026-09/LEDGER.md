...
| гейт фазы 3 (предварительно) | `458ac8a8` derived, `7fbe4345` примеры | `vibe doc check --coverage --min 100` — 516 из 516 обязательств, 571 пара аудиторий, 100 %; `--citations` — 759 правил, 0 неразрешённых; `--derived` — 72, два перезаписаны после появления `vibe doc` и `init --kind doc`; `--media` — 3 плейсхолдера, 0 ошибок; `--examples` — 55 совпали, 7 отложены по `deferred.toml`, 3 переблагословлены (карта получила рёбра `documents` от новых цитат); `--style` и `--translations` над фикстурой — после P2-O9; открыто: стиль-ревью A3.16, картинки F-32 |
