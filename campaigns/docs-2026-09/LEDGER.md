...
| A4.13 | `bc9de5c8` `feat(web): fall back to the source language without losing the reader` | фоллбэк-страницы, плашка, cookie `lang`; фильтр `noindex` из sitemap лежит в `tools/build.mjs` — переезжает в `root-files.mjs` пакетом P4-O3 |
