...
# The vibeterm / vibeframe terminal products moved to a separate repo
# (`vibevm-term`); they are no longer built here. To get `vibe term` /
# `vibe frame` working, clone vibevm-term and run `vibeterm self install`
# / `vibeframe self install` (Phase 2 of the extraction).
