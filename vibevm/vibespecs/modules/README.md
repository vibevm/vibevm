...
- @fact:common-pointer Foundation decisions that cross every crate live in
  [`vibevm/vibespecs/common/`](../common/). @status:doc/done
...
- @fact:design-pointer Non-normative design rationale — the *why* and the lore behind these
  PROPs — lives in [`vibevm/vibespecs/design/`](../design/); a PROP that has a
...
- @fact:idx-prop-001 [PROP-001: Git-backed registry](vibe-registry/PROP-001-git-backend.xml)
    — shell-out to `git` (not `libgit2`), `GitBackend` trait, cache
    layout, Windows UX. @status:doc/done
- @fact:idx-prop-002 [PROP-002: Decentralized per-package registry](vibe-registry/PROP-002-decentralized-registry.xml)
    — per-package repos, `[[registry]]`/`[[mirror]]`/`[[override]]`,
    content-addressed identity, lockfile v2. @status:doc/done
- @fact:idx-prop-008 [PROP-008: Qualified package naming](vibe-registry/PROP-008-qualified-naming.xml)
...
- @fact:idx-prop-010 [PROP-010: Local package cache](vibe-registry/PROP-010-local-package-cache.xml)
...
- @fact:idx-prop-021 [PROP-021: Submodule sources](vibe-registry/PROP-021-submodule-sources.xml)
...
- @fact:idx-prop-023 [PROP-023: Bridge packages](vibe-registry/PROP-023-bridge-packages.xml)
...
- @fact:idx-prop-030 [PROP-030: The embedded registry](vibe-registry/PROP-030-embedded-registry.xml)
...
- @fact:idx-prop-003 [PROP-003: Dep-model evolution](vibe-resolver/PROP-003-dep-evolution.xml)
...
- @fact:idx-prop-017 [PROP-017: Resolvo as the production resolver](vibe-resolver/PROP-017-resolvo-resolver.xml)
...
- @fact:idx-prop-005 [PROP-005: Optional package index](vibe-index/PROP-005-package-index.xml)
...
- @fact:idx-prop-007 [PROP-007: Workspace](vibe-workspace/PROP-007-workspace.xml)
...
- @fact:idx-prop-009 [PROP-009: Loading model](vibe-workspace/PROP-009-loading-model.xml)
...
- @fact:idx-prop-011 [PROP-011: Incremental install](vibe-workspace/PROP-011-incremental-install.xml)
...
- @fact:idx-prop-012 [PROP-012: Managed redirect block](vibe-workspace/PROP-012-managed-redirect-block.xml)
...
- @fact:idx-prop-020 [PROP-020: Install hooks](vibe-workspace/PROP-020-install-hooks.xml)
    — `[hooks]` in a package manifest, the pre/post phases, the
    interpreter selection, the trust gate and the hook environment.
    **Status: IMPLEMENTED.** @status:doc/done
- @fact:idx-prop-022 [PROP-022: Materialization modes](vibe-workspace/PROP-022-materialization-modes.xml)
    — snapshot (default) vs in-place materialization as a property of any
    package, the hardlink machinery, the destructive guard.
    **Status: IMPLEMENTED.** @status:doc/done
- @fact:idx-prop-025 [PROP-025: vibe-native binary delivery](vibe-workspace/PROP-025-binary-delivery.xml)
...
- @fact:idx-prop-034 [PROP-034: Transitive links and the static boot graph](vibe-workspace/PROP-034-transitive-links-boot-graph.xml)
...
- @fact:idx-prop-035 [PROP-035: The spec compiler](vibe-workspace/PROP-035-spec-compiler.xml)
...
- @fact:idx-prop-038 [PROP-038: Hybrid boot linking](vibe-workspace/PROP-038-hybrid-boot-linking.xml)
...
- @fact:idx-prop-015 [PROP-015: MCP integration](vibe-mcp/PROP-015-mcp-integration.xml)
    — `vibe mcp serve` / `install` / `status`: vibevm as an MCP server a
    coding agent queries about the lockfile and the installed corpus.
    **Status: IMPLEMENTED.** @status:doc/done
- @fact:idx-prop-026 [PROP-026: The tcg tool family](vibe-mcp/PROP-026-tcg-tool-family.xml)
...
- @fact:idx-prop-027 [PROP-027: MCP packages](vibe-mcp/PROP-027-mcp-packages.xml)
...
- @fact:idx-prop-036 [PROP-036: `vibe tree` — the spec-tree analyzer](vibe-cli/PROP-036-package-tree.xml)
...
- @fact:idx-prop-037 [PROP-037: `vibe tree` — the interactive TUI](vibe-cli/PROP-037-tree-tui.xml)
...
- @fact:idx-prop-042 [PROP-042: AIUI observation](vibe-cli/PROP-042-aiui-observation.xml)
...
- @fact:idx-prop-040 [PROP-040: The settings system](vibe-settings/PROP-040-settings.xml)
    — a three-level, schema-first, introspectable preference store behind
    `vibe prefs`. **Status: IMPLEMENTED.** @status:doc/done
- @fact:idx-prop-041 [PROP-041: The settings UI](vibe-settings/PROP-041-settings-ui.xml)
...
- @fact:idx-prop-039 [PROP-039: The action system](vibe-actions/PROP-039-action-system.xml)
...
- @fact:idx-prop-043 [PROP-043: The facts markup](vibe-facts/PROP-043-facts-markup.xml) · [PROP-047: Progress campaigns](vibe-progress/PROP-047-progress-campaigns.xml)
...
- @fact:idx-owner-guide [OWNER-GUIDE](vibe-progress/OWNER-GUIDE.xml)
