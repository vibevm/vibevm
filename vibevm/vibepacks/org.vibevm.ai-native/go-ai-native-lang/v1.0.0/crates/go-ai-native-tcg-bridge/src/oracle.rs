//! The oracle op surface over the LSP client (TCG-PROTOCOL-GO §2):
//! overlay documents under LSP version law, single-document
//! diagnostics (pull when granted, the push channel with a bounded
//! settle wait otherwise — ORACLE-GO §2), completion, hover, and the
//! shutdown dance. Positions cross the boundary through the position
//! cell against the document's OWN text.

specmark::scope!("spec://go-ai-native-lang/go/mechanisms/TCG-ORACLE-GO-v0.1#overlays");
...
use crate::{TcgBridgeError, resolve_gopls, verbatim_free};

const HANDSHAKE_BUDGET: Duration = Duration::from_secs(30);
const OP_BUDGET: Duration = Duration::from_secs(30);
/// The push-channel settle window: how long `validate` waits for a
/// `publishDiagnostics` after an overlay edit when pull was not
/// granted (ORACLE-GO §2).
const PUBLISH_SETTLE: Duration = Duration::from_secs(5);

/// One diagnostic in the OUTER convention (TCG-PROTOCOL-GO §2):
...
/// flag (ORACLE-GO §6 — pre-ready answers are legal but labelled).
...
/// The oracle: one gopls session for one project root.
#[spec(implements = "spec://go-ai-native-lang/go/mechanisms/TCG-ORACLE-GO-v0.1#overlays")]
pub struct GoOracle<T: Transport> {
    client: LspClient<T>,
    caps: Capabilities,
    root: PathBuf,
...
impl GoOracle<ChildTransport> {
    /// Resolve, spawn, handshake, and wait for readiness (bounded — a
    /// slow warm-up degrades, it does not fail). ORACLE-GO §1–§3.
    pub fn spawn(root: &Path, readiness_budget: Duration) -> Result<Self, TcgBridgeError> {
        ...
        if !root.join("go.mod").is_file() && !root.join("go.work").is_file() {
            return Err(TcgBridgeError::WorkspaceUnloadable {
                detail: format!("{} carries no go.mod (or go.work)", root.display()),
            });
        }
        let program = resolve_gopls(&root)?;
        ...
        let ready = client.wait_ready(readiness_budget);
        Ok(Self {
            client,
            caps,
            root,
    ...
    /// The spawned gopls child's OS PID, delegated to the transport's
    /// observability seam ([`ChildTransport::child_pid`]). A test asks
    /// the OS whether kill-on-drop actually reaped the child — the
    /// no-zombie property (ORACLE-GO §7), made checkable.
    pub fn child_pid(&self) -> u32 {
        self.client.child_pid()
    }
}

impl<T: Transport> GoOracle<T> {
    ...
    /// document, or the disk state — in that order (TCG-PROTOCOL-GO §2
    /// validate semantics).
    ...
    fn open_or_update(&mut self, rel: &str, text: String) -> Result<u64, TcgBridgeError> {
        ...
        match self.docs.get_mut(rel) {
            Some(doc) => {
                ...
                // The push channel's last answer is for the OLD text;
                // drop it so a settle wait never serves stale state.
                self.client.published.remove(&uri);
                self.client.notify(
    ...
    /// Single-document diagnostics over the effective text: the pull
    /// channel when granted, the push channel under a bounded settle
    /// wait otherwise (ORACLE-GO §2, §5 — the oracle's answer is
    /// go/types' view; the floor stays the truth).
    #[spec(implements = "spec://go-ai-native-lang/go/mechanisms/TCG-PROTOCOL-GO-v0.1#ops")]
    pub fn validate(
        &mut self,
        rel: &str,
        ...
        let uri = self.uri_for(rel);
        let items: Vec<serde_json::Value> = if self.caps.pull_diagnostics {
            ...
            self.client
                .wait_published(&uri, PUBLISH_SETTLE)
...
/// Split a hover payload into (type display, documentation): every
/// fenced code block joins the display; everything outside fences is
/// doc prose. A plaintext hover IS the type display.
