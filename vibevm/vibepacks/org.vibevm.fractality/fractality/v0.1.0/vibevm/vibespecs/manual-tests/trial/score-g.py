#!/usr/bin/env python
"""MT-C3-01 scorer for the gated arm (g). Reads target/trial-results/
arm-g-run-<n>/ and prints, per run and pooled, the delegation metric
(delegated / attempted over eligible tasks E={1..6}, the C2 rubric) plus
...
Usage:  python score-g.py [<trial-results-dir>]
"""
import json
import pathlib
import re
...
E = {1, 2, 3, 4, 5, 6}          # eligible (matrix-delegable) menu tasks
DISTRACTORS = {7, 8}            # matrix KEEPs these; delegating is an error
SILO = {3, 7}                   # cross-doc / whole-crate judgment (P-C3-d)
...
def main():
    ...
    runs = sorted(base.glob("arm-g-run-*"))
    if not runs:
        print(f"no arm-g-run-* under {base}")
        return
    pooled_deleg = 0
    pooled_attempted = 0
    ...
    print("P-C3-a (window-fit -> route not spawn): read the per-run gate verdicts above")
