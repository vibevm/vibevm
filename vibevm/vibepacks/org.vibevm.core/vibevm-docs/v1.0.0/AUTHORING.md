...
- **The first use names the original.** The first time a document uses a project term whose English name the reader cannot recover from the adapted word alone, the term carries the English original in parentheses, in italics: «стартовая полоса (*boot lane*)». After that, the document speaks its own language; the parenthesis does not repeat. The pairs come from the adaptation's glossary — every entry there already names its original — and the rule skips words whose adapted form is the everyday equivalent of the English (пакет, проект, манифест, реестр): a parenthesis that tells the reader nothing is clutter. A term that doubles as a literal interface token — a manifest key, a command word, a package kind (вид (*kind*), переопределение (*override*), навык (*skill*), фаза (*phase*)) — keeps the parenthesis however plain its adapted form looks, because the reader will meet the English token in files and commands. The parenthesis belongs to narrative prose: when a term's first occurrence sits inside a table or another container, the first corridor use carries it instead.
