...
CONTINUITY_TEXT = (
    ...
    "restarts. Bind affected build/test targets before case filters and retain "
    "the required behavioral and consumer-contract proof. A completed atom, "
    "milestone or push does not trigger a full panel; use verification-selection "
    "for scoped checks, evidence reuse and failed/invalidated/unrun panel steps. "
    "Keep the designated final comprehensive gate. Mutation diagnostics require concrete suspicion and lower cost than "
...
CLAUDE_CONTINUITY = (
    ...
    "restart. Select affected build/test targets before case filters; preserve "
    "behavior/consumer proof. Atom, milestone and push do not trigger full panels. "
    "Follow verification-selection for scoped checks and step reuse; retain the "
    "designated final comprehensive gate. Mutation diagnostics need concrete suspicion and lower cost than "
...
def read_optional_bytes(path: Path, code: str) -> bytes | None:
    try:
        return path.read_bytes()
...
def canonical_profile(settings: dict[str, Any], config: dict[str, Any]) -> str:
    profile = settings.get("planning_profile", config.get("planning_profile", "standard"))
...
def read_advisory_toml(path: Path) -> dict[str, Any] | None:
    try:
        raw = path.read_bytes()
    except OSError:
        return None
...
def formal_handoff_active(context: Path, custody: dict[str, Any]) -> bool:
    """Return true only for one coherent, matching, unterminated formal offer."""
    if custody.get("state") != "offering":
        return False
    offer_id = custody.get("offer_id")
    epoch = custody.get("epoch")
    if not isinstance(offer_id, str) or not offer_id.strip():
        return False
    if type(epoch) is not int or epoch < 0:
        return False
    if custody.get("context_id") != context.name:
        return False

    try:
        offer_paths = sorted((context / "handoffs").glob("*/offer.toml"))
    except OSError:
        return False
    matches: list[Path] = []
    for offer_path in offer_paths:
        offer = read_advisory_toml(offer_path)
        if offer is None:
            continue
        offer_schema = offer.get("schema")
        source_epoch = offer.get("source_epoch")
        proposed_epoch = offer.get("proposed_epoch")
        if (
            type(offer_schema) is not int
            or type(source_epoch) is not int
            or type(proposed_epoch) is not int
            or source_epoch < 0
            or proposed_epoch < 0
        ):
            continue
        if (
            offer_schema == SCHEMA
            and offer.get("kind") == "planned"
            and offer.get("status") == "offered"
            and offer.get("handoff_id") == offer_id
            and offer.get("context_id") == context.name
            and source_epoch == epoch
            and proposed_epoch == epoch + 1
        ):
            matches.append(offer_path.parent)
    if len(matches) != 1:
        return False
    bundle = matches[0]
    return not (bundle / "receipt.toml").exists() and not (bundle / "cancellation.toml").exists()
...
def render(snapshot: dict[str, Any]) -> tuple[bytes, bytes | None, dict[str, Any]]:
    ...
    profile = canonical_profile(settings, snapshot["config"])
...
def read_snapshot(context: Path) -> dict[str, Any]:
    ...
    custody_raw = read_optional_bytes(context / "custody.toml", "GOAL_PLAN_INVALID")
    ...
    custody = parse_toml(custody_raw, context / "custody.toml") if custody_raw is not None else {}
    ...
    return {
        ...
        "settings": settings,
        "custody": custody,
        "formal_handoff_active": formal_handoff_active(context, custody),
        "plan": plan,
        "config": config,
...
def assert_publish_allowed(snapshot: dict[str, Any]) -> None:
    if snapshot["formal_handoff_active"]:
        raise GoalError(
            "GOAL_FORMAL_HANDOFF_ACTIVE",
            "an explicit formal handoff is offering; receipt or cancel it before publication",
        )


def inputs_unchanged(context: Path, snapshot: dict[str, Any]) -> bool:
    if read_bytes(context / "plan.toml", "GOAL_INPUT_MOVED") != snapshot["plan_raw"]:
        return False
    settings_path = context / "settings.toml"
    config_path = context.parent.parent / "config.toml"
    current_settings = parse_toml(
        read_bytes(settings_path, "GOAL_INPUT_MOVED"), settings_path
    )
    current_config = parse_toml(read_bytes(config_path, "GOAL_INPUT_MOVED"), config_path)
    current_custody_raw = read_optional_bytes(context / "custody.toml", "GOAL_INPUT_MOVED")
    current_custody = (
        parse_toml(current_custody_raw, context / "custody.toml")
        if current_custody_raw is not None
        else {}
    )
    return (
        current_settings.get("goal_node") == snapshot["settings"].get("goal_node")
        and canonical_profile(current_settings, current_config)
        == canonical_profile(snapshot["settings"], snapshot["config"])
        and not formal_handoff_active(context, current_custody)
    )
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for attempt in range(2):
        ...
        can_write = False
        try:
            assert_publish_allowed(snapshot)
            can_write = True
        except GoalError:
            can_write = False
        if args.check:
        ...
        if not can_write:
            raise GoalError(
                "GOAL_FORMAL_HANDOFF_ACTIVE",
                "an explicit formal handoff is offering; goal publication is fenced",
            )
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument(
        "--holder-id",
        help="deprecated compatibility metadata; not an ordinary publication gate",
    )
    result.add_argument(
        "--session-id",
        help="deprecated compatibility metadata; not an ordinary publication gate",
