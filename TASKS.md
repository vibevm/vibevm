...
- [ ] **С2** — проводная волна: B-091(б) ✓9686836c · B-073 ✓замером
      (ce024d46) · B-072 ✓45a415ab · B-079-CLI ✓e3ec088e; **осталось:
      B-078 (journal nullable) и B-079-HTTP** (пакеты в scratchpad).
...
- [ ] **С4** — гигиена провода: B-047-пути · B-070+B-081 словарь-тест ·
      B-064 + sync-engines.
- [ ] **С5** — версия 1.0.0: замер читателей версии → workspace → все
      канонические пакеты → пины redbook → lock/vibedeps → epoch-маркеры
      (44 info → 0, санкционировано) → CHANGELOG.
- [ ] **С6** — публикация: пробы → волна publish в `vibespecs` (пустая,
      проверено 2026-08-20) → E2E с настоящего реестра → EOL/hash-проверка.
- [ ] **С7** — фаза F: credibility report (prose-judgement, без T — записано).
- [ ] **С8** — документация, вариант А: README · ALPHA-NOTES (семвер:
      «1.0.0 будет ломаться») · команды ядра · `docs/SITE-MANIFEST.toml`
      (машинный забор на сайт).
- [ ] **С9** — дистрибутив Windows: замер `vibe self` → дизайн → сборка →
      smoke.
- [ ] **С10** — релизный гейт: health-audit → пре-прогон ручных тестов →
      перекличка → чеклист инспекции → `_STATUS: ГОТОВО К РУЧНОЙ ИНСПЕКЦИИ
      ВЛАДЕЛЬЦА`. Тег `v1.0.0` — после инспекции, словом владельца.
