...
- [~] `feat(vibe-resolver)`: new crate with `DepProvider` / `DepSolver` traits; `NaiveDepSolver` (DFS, no backtracking) handles concrete deps + capabilities + obsoletes + conflicts + simple disjunctions; `MultiRegistryProvider` adapts `MultiRegistryResolver`. Resolvo / libsolv impls behind the same trait still pending — naive covers today's all-empty-deps fixtures and any first-cut realistic graph.
