...
///
/// The library path must be absolute. Point and optional IR schema expectations
/// are compared exactly with the selected generated manifest row before the
/// borrowed generated context can reach the plugin.
///
/// ```
...
pub struct NativeInvocation<'a> {
    ...
    /// Typed lifecycle/compiler/slot point expected from the manifest.
...
impl NativeLoader {
    ...
    pub fn invoke(&self, invocation: NativeInvocation<'_>) -> Result<Reply, NativeLoadError> {
        let (canonical, display_path) = validate_path(invocation.library)?;
...
fn finish_call(call: ffi::CallResult, path: &str) -> Result<Reply, NativeLoadError> {
    if call.status != 0 {
    ...
    admission::parse_reply(response.bytes(), path)
}
