...
/// The corpus: the nine redbook spec sources (the seven of XML-MEASURE
/// §6.1 plus the two PROP-049 member fragments). The corpus flipped to
/// XML sources on 2026-08-24 — except reserved-name files (README.md
/// stays Markdown by the owner's ruling) — so the loader is form-blind:
/// a `.md` source rides verbatim, a `.xml` source rides as its CANONICAL
/// MARKDOWN PROJECTION under its former `.md`-spelled name. Every
...
fn corpus() -> Vec<(String, String)> {
    ...
    while let Some(dir) = stack.pop() {
        ...
        for e in entries.flatten() {
            ...
            let (rel, md) = match ext {
                Some("md") => {
                    if p.file_name().is_some_and(|n| n == "LICENSE.md") {
                        continue; // reserved legal text, not house markup
                    }
                    let rel = p
                        .strip_prefix(&root)
                        ...
                        .to_string_lossy()
                        .replace('\\', "/");
                    (rel, std::fs::read_to_string(&p).expect("readable"))
                }
                Some("xml") => {
                    ...
                    let rel = p
                        ...
                        .replace(".xml", ".md");
                    let md = to_markdown(
                        &from_xml(&raw).unwrap_or_else(|e| panic!("{rel}: source XML: {e}")),
                    );
                    (rel, md)
                }
                _ => continue,
    ...
    files.sort();
    // Eight, not the historical nine: the ninth was LICENSE.md, which sat
    // in the corpus only because the old loader globbed every `.md` —
    // a licence is reserved legal text, not house markup (owner ruling
    // 2026-08-24), and it carried zero units, so every shape total below
    // is unchanged.
    assert_eq!(
        files.len(),
        8,
        "the redbook carries exactly eight spec sources (six of XML-MEASURE §6.1 \n         plus the two PROP-049 member fragments; LICENSE.md is reserved legal text)"
    );
    files
}
