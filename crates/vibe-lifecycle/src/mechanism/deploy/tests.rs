...
use super::{
    ...
    execute_deploy_targets, list_deployments,
};
use crate::mechanism::package::support::{empty_world, key, pin, registry, temp};
use crate::mechanism::{BUILTIN_VIBE_BIN_NAME, BUILTIN_VIBE_BIN_PIN};

/// One `[[deploy.target]]` with no provider pin, so §3.1 step 3 answers.
fn builtin_target(id: &str) -> DeployTarget {
    DeployTarget {
        id: id.to_owned(),
...
/// §7.0.2: "the one deploy builtin row (`#vibe-bin`) refuses as
/// provider-not-landed — a typed refusal, never a stub."
///
/// The refusal names the row AND the atom that lands it, so a reader
/// learns what is missing rather than that something broke.
#[test]
#[verifies("spec://org.vibevm.core/vibevm/common/PROP-054#OPEN-DEPLOY-TARGETS")]
fn the_reserved_deploy_builtin_refuses_as_provider_not_landed() {
    let fixture = Fixture::new("helper-bytes");
    let targets = [builtin_target("local-helper")];
    ...
    let error = execute_deploy_targets(&execution).expect_err("the builtin row has no provider");

    let DeployError::ProviderNotLanded { key, pin, atom } = &error else {
        panic!("expected the provider-not-landed refusal, got: {error}");
    };
    assert_eq!(key, "deploy:vibe-bin");
    assert_eq!(pin, BUILTIN_VIBE_BIN_PIN);
    assert_eq!(*atom, "R8-VIBE-BIN");
    assert!(
        error
            .to_string()
            .contains("deploys nothing rather than pretend")
    );
    // And nothing moved: no state home directory for this deployment.
    assert!(
        !state_home.join("intent.json").exists(),
        "a refused selection writes no journal",
    );
}
