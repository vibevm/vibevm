...
pub(super) mod index;
pub use index::RegistryRowIndex;
use index::{RegistryIdentity, new_registry_identity};
...
pub struct ExtensionRegistry {
    ...
    pub(super) identity: Arc<RegistryIdentity>,
}
...
impl Clone for ExtensionRegistry {
    fn clone(&self) -> Self {
        Self {
            ...
            effective_order: self.effective_order.clone(),
            notices: self.notices.clone(),
            // A clone owns different row allocations. Minting a fresh token
            // makes an index from either allocation reject the other.
