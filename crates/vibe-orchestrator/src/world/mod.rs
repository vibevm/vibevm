...
mod package_skill;
pub use package_skill::RECONCILE_KEY as PACKAGE_SKILL_RECONCILE_KEY;
...
pub(crate) fn plan_default(path: &Path, phases: &[Phase]) -> Result<RitualPlan> {
    let selected = crate::install::resolve_project_root(path)?;
...
/// from a workspace the caller ALREADY has.
///
/// Prepared, like its default-phase sibling: the surface owns the ONE pre-wipe
/// snapshot — the selected manifest it reads, the tree it builds from that
/// snapshot, and whatever per-run configuration it derives from it — and hands
/// the tree down. A rediscovery here would be a second byte snapshot of a tree
/// the command is about to destroy, and the snapshot is also where a surface
/// finds its own provider configuration, which is why no manifest rides on the
/// plan this returns.
///
/// ```no_run
/// use vibe_orchestrator::plan_clean_prepared;
/// # fn call(selected: &std::path::Path, workspace: &vibe_workspace::Workspace)
/// #   -> anyhow::Result<()> {
/// let plan = plan_clean_prepared(selected, workspace)?;
/// assert_eq!(plan.workspace_root(), workspace.root, "the clean point plans over the tree given");
/// # Ok(())
/// # }
/// ```
...
pub struct LoadedRegistry {
    ...
    world: EnvelopeWorld,
    /// The canonical workspace root which owns lifecycle state.
    pub workspace_root: PathBuf,
    /// The selected host's own identity.
    pub host_identity: HostIdentity,
    /// Which manifest role the selected node plays.
    pub manifest_kind: EffectiveManifestKind,
    /// The effective language stack, when one is selected.
...
///
/// ```no_run
/// let loaded = vibe_orchestrator::inspect(std::path::Path::new("."))?;
/// let _ = loaded.registry.notices();
/// # Ok::<(), anyhow::Error>(())
/// ```
pub fn inspect(path: &Path) -> Result<LoadedRegistry> {
...
fn load_registry(selected: &Path, mode: WorldLoadMode) -> Result<LoadedRegistry> {
    let selected = crate::install::resolve_project_root(selected)?;
    let workspace = discover_for_collection(&selected)?;
