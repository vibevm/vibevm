...
fn only_pre_clean_treats_an_absent_root_as_an_empty_installed_world() {
    ...
    // The clean point is PREPARED now: the surface owns the pre-wipe snapshot
    // and hands the tree down, so the test does what the adapter does.
    let selected = crate::install::resolve_project_root(project.path()).unwrap();
    let prepared = vibe_workspace::Workspace::discover(&selected).unwrap();
    assert!(
        plan_clean_prepared(&selected, &prepared)
            .unwrap()
            .executions
            .is_empty()
    );
