...
/// The slot half of a measurement, for the assertions below.
fn slot_of(
    ...
    match &failure.measurement {
        ...
        | Measurement::InstallBarrier {
            ...
            reports,
            packages_resolved,
        } => (progress, reports, *packages_resolved),
        Measurement::Lifecycle { .. } => panic!("a resume measures slot work"),
    }
}
...
fn a_failed_resume_carries_its_rows_progress_and_exact_error() {
    ...
    let (progress, reports, packages_resolved) = slot_of(&failure);
    assert_eq!(reports.len(), 1, "the resumed row survived");
    assert_eq!(reports[0].point, "slot:post-install");
    assert_eq!(
...
///
/// The neutral carrier is not a report decision — it is the seam that lets
...
fn the_neutral_transport_round_trips_every_measured_field() {
    ...
    let carried = crate::failure::carry(failure);
    ...
    let taken = crate::failure::take(carried)
        .unwrap_or_else(|error| panic!("the transport is exact and must come back out: {error:#}"));
    let (progress, reports, packages_resolved) = slot_of(&taken);
    assert_eq!(reports.len(), 1);
    assert_eq!(reports[0].point, "slot:post-install");
    assert_eq!(progress.nodes_regenerated, ["."]);
    assert_eq!(packages_resolved, 3);
...
fn the_production_capture_takes_once_on_failure_and_never_on_success() {
    ...
    assert_eq!(packages_resolved, 7, "and the carried count");
    assert!(
        failure.original.downcast_ref::<Sentinel>().is_some(),
...
fn the_production_capture_never_takes_on_a_successful_region() {
    ...
    let mut run =
        crate::install::InstallRun::new(std::path::PathBuf::from("/p"), InstallDisposition::Fresh);
