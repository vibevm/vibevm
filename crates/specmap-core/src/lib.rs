...
/// regenerated by `cargo xtask codegen` from `schemas/specmap.jtd.json`.
/// specmap-core owns its own data model — no `vibe-wire` edge (Traceability
/// Relocation Plan, Phase 1). `non_snake_case` is allowed because jtd-codegen
/// emits `pub camelCase` fields carrying `#[serde(rename)]`.
