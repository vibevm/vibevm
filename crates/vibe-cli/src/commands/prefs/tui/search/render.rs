//! Draw the Search Everywhere window (PROP-041 §7 `#settings-search`): a
//! rounded, titled panel with a query line, the hybrid "All" + per-category tab
//! strip (pills), the grouped results list (a coloured per-provider badge heads
//! each group in the All tab, one normalised row per hit), and a key hint.
//! Drawn last so it sits on top of the form/tree. All colour + glyphs come from
//! the shared [`crate::commands::tree::tui::theme::Theme`].
//!
...
use crate::commands::tree::tui::theme::{Role, Theme};
use crate::commands::tree::tui::ui::Window;

use super::SearchState;

/// Draw the window centered over `area`.
pub(crate) fn draw(area: Rect, buf: &mut Buffer, state: &SearchState, theme: &Theme) {
    ...
    let inner = Window::centered(
        area,
        buf,
        Line::styled(" Search settings ", theme.title()),
        w,
        h,
        theme,
    );
...
/// The query line: an accent prompt glyph, the typed text, and a block cursor.
...
/// The tab strip: the active tab a filled accent pill, the rest dim.
...
fn provider_color(provider: &str, theme: &Theme) -> Color {
    match provider {
        "settings" => theme.color(Role::Accent),
        "actions" => theme.color(Role::Gold),
        _ => theme.color(Role::Foam),
    }
}
    ...
    for (offset, row) in state.rows.iter().skip(start).take(height).enumerate() {
        ...
        match row {
            SearchRow::Header {
                ...
                // A coloured pill badge + a dim count.
                let badge = Style::new()
...
/// the left, the secondary (keybinding / migration target / "why disabled"
