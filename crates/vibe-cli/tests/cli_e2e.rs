...
#[test]
fn install_git_source_then_repeat_install_no_args_reports_already_installed() {
    if !git_available() {
    ...
    // Repeat install with no arguments. Behaviour is the same as for
    // registry-resolved packages: re-resolve from `[requires]` and
    // refuse the second install of an unchanged package with the
    // standard "already installed" error. This guarantees git-source
    // declarations are not silently re-applied if the operator runs
    // `vibe install` repeatedly during exploration.
    vibe()
        .env("VIBE_REGISTRY_CACHE", &cache)
        .arg("install")
        .arg("--path")
    ...
    // Lockfile still has one entry, source_kind = Git.
    let lock_text = fs::read_to_string(project.path().join("vibe.lock")).unwrap();
    let lock: vibe_core::manifest::Lockfile = toml::from_str(&lock_text).unwrap();
    assert_eq!(lock.packages.len(), 1);
    assert_eq!(
...
fn make_redirect_stub_bare_repo(
    root: &Path,
    repo_name: &str,
    target_url: &str,
    ref_policy: &str,
...
fn install_via_redirect_pass_through_tag() {
    ...
    // Content from the TARGET landed in the project tree.
    let materialised = project.path().join("spec/flows/internal/MANIFEST.md");
    assert!(
