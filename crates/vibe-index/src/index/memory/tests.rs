...
/// B-072's own cell — that an identical rewrite changes zero bytes and
/// keeps its stamp. Split out so every cell stays inside the file budget;
/// it scans with the fixtures and the fixed clock declared below.
#[path = "tests/idempotence.rs"]
mod idempotence;

/// PROP-057's own cell — the documentation card, its two edges, and the
/// one fold that answers the reverse questions. Same fixtures, same
/// clock, and a subject of its own.
