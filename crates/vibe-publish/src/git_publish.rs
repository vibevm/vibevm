...
fn staged_tree_changed(cwd: &Path) -> Result<bool, PublishError> {
...
fn run_remote_git_in(cwd: &Path, args: &[&str], clone_url: &str) -> Result<Output, PublishError> {
    let output = git_command(cwd, args).output().map_err(|e| {
        PublishError::Git(format!("spawning git {}: {e}", join_args_redacted(args)))
    })?;
...
fn is_concurrent_ref_rejection(stderr_lowercase: &str) -> bool {
...
pub fn redact_credentials(s: impl AsRef<str>) -> String {
    ...
    while i < bytes.len() {
        ...
        // Default: copy one Unicode scalar and advance by its UTF-8 width.
        // URL schemes/user-info are ASCII, but surrounding diagnostics and
        // paths need not be; byte-at-a-time copying would corrupt them and
        // could make the next `s[i..]` slice start off a char boundary.
        let ch = s[i..]
            .chars()
            .next()
            .expect("i is always within the input string");
        out.push(ch);
        i += ch.len_utf8();
    }
    out
