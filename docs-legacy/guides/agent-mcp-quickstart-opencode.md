...
**`vibe install flow:org.vibevm.world/wal` fails inside the agent's run.** Read-only clone needs no token. Probe with `git ls-remote https://github.com/vibespecs/org.vibevm.world.wal` (the canonical repo under `naming = "fqdn"`). If that fails, the issue is in your network / DNS, not in vibevm.
