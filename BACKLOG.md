...
- ##B062-IT-IS-NOT-THEORY **Это не теория — счёт открыт в тот же день.** В одной строке бэклога (`##B056-ODR-PARALLEL`) полгода жила фраза, которую постройка опровергла; в другой (`##B056-ORDER-AND-RECURSION`) стоял `@spec/plan` на работе, сделанной сутки назад; в третьей (`##B056-LANDED`) число тестов отстало на четыре. Все три нашлись руками, потому что находить их нечем. В спеках ровно этот класс ловит `text-stability.py` за секунды. @impl/done
