...
- @fact:ENTRY-THE-FILE-IS-MID-MIGRATION **How to read this file today, and it is
  ...
  **13 carry no sign of closure anywhere in their body** and the rest read as
  ...
  ([`##WAL-C-MEASURE-WHAT-IS-BUILT-FIRST`](spec/WAL.xml)). Completing the
...
| @fact:B074-LOCATOR **locator** | `crates/progress-core/src/parse/` — разбор блоков и фактов; `vibe progress check --exhaustive` не сообщает ничего. Специмены (починены тем же заходом): `spec/design/loading-and-boot-model.xml`, `spec/design/workspace-and-qualified-naming.xml`, `spec/design/tui-visual-language.xml` |
...
| @fact:B070-LOCATOR **locator** | `crates/vibe-core/src/package_ref/kind.rs` — enum-первоисточник; `crates/vibe-index/src/types/kinds.rs` — вторая копия enum; `schemas/list_report.jtd.json`, `schemas/registry_sync_report.jtd.json`, `crates/vibe-cli/resources/package-tree.schema.v1.json` — три схемы; плюс ~9 рукописных списков в help-текстах, сообщениях об ошибках, `xtask/src/batch_review/refs.rs`, `spec/common/PROP-000.xml`, `spec/boot/00-core.xml` |
...
- @fact:B046-NO-TYPED-MARKER **Чего в рельсах НЕТ — и это несущий пробел, которого запись не видела.** Отличить «AI-Native языковой стек» от любого другого пакета механически сегодня **нечем**. Поле `kind` — закрытое перечисление `flow | feat | stack | tool | mcp` (`crates/vibe-core/src/package_ref.rs:45-49`), и `kind = "stack"` несут **девять** канонических манифестов, из которых языковых стеков — три: рядом стоят семейные агрегаторы `rust-ai-native` / `typescript-ai-native` / `go-ai-native` (PROP-028, `kind = "stack"` ровно так же) и тест-фикстуры. Единственное сегодняшнее отличие — суффикс имени `-lang`, то есть **текст, а не объявление**. Ближайший механический признак — что у агрегатора нет `[boot_snippet]`, а у языкового стека есть с `category = "stack"`, — это вывод по косвенному, а не декларация, и норма без чекера дрейфует ровно так же ([`##WAL-C-A-NORM-WITHOUT-A-CHECKER-DRIFTS`](spec/WAL.xml)). **Поэтому у развилки появился нулевой вопрос, который решается ДО выбора из трёх вариантов: чем пакет объявляет, что он — AI-Native язык.** Любой из трёх вариантов без ответа на него будет угадывать по имени.
...
| @fact:B050-FILED **filed by** | босс-дизайн батча 3 волны Б, 2026-08-04 (`spec/design/new-rule-classes.xml` §3) |
...
| | |
|---|---|
| @fact:B091-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/common/PROP-044#machinery`](spec/common/PROP-044-change-native-formats.xml#machinery) §4.2 — JTD выбран **за бедность**, чтобы агент не мог выразить опасное; это первый случай, когда бедность режет по живому полю |
...
| | |
|---|---|
| @fact:B095-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/common/PROP-044#machinery`](spec/common/PROP-044-change-native-formats.xml#machinery) `##M-WIDE-INTEGERS-AS-STRINGS` — общее правило, отчеканенное на `repomd.size` |
...
| | |
|---|---|
| @fact:B078-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/common/PROP-044#laws`](spec/common/PROP-044-change-native-formats.xml#laws) — закон 1: описание формата не вправе расходиться с форматом; и §4.2, где схема названа описанием ПРОВОДА |
...
| | |
|---|---|
| @fact:B079-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/common/PROP-044#our-formats`](spec/common/PROP-044-change-native-formats.xml#our-formats) — `##FMT-UNINVENTORIED`: «The seven CLI `--json` reports (scripts and agents parse them) … each is "something a foreign parser reads", each gets a registry entry, a schema, an epoch and a corpus» |
...
| | |
|---|---|
| @fact:B080-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/common/PROP-044#machinery`](spec/common/PROP-044-change-native-formats.xml#machinery) — §4.5: читатель, которому не хватает возможностей, карантинит запись, и отказ всплывает В ТОЧКЕ ПРИМЕНЕНИЯ |
...
| | |
|---|---|
| @fact:B081-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/common/PROP-044#agents`](spec/common/PROP-044-change-native-formats.xml#agents) — G9: «a vocabulary exists in exactly one schema; both wire sides, Rust types, docs and prose lists are generated from it» |
...
| | |
|---|---|
| @fact:B082-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/common/PROP-044#machinery`](spec/common/PROP-044-change-native-formats.xml#machinery) — §4.7: «while it is closed, CI refuses any change under `schemas/**` or `formats/**`» |
...
| | |
|---|---|
| @fact:B083-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/modules/vibe-index/PROP-005#form-factor`](spec/modules/vibe-index/PROP-005-package-index.xml#form-factor) — `##INDEX-URL-CONFIG` (TOML-пример) и `##INDEX-URL-DEFAULT` («Default: `<registry-url>/index`») |
...
| | |
|---|---|
| @fact:B084-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/modules/vibe-index/PROP-005#layout`](spec/modules/vibe-index/PROP-005-package-index.xml#layout) — `##REPOMD-TRUST-POINT`; рядом `#trust` `##TWO-INTEGRITY-LAYERS` |
...
| | |
|---|---|
| @fact:B085-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/modules/vibe-index/PROP-005#http`](spec/modules/vibe-index/PROP-005-package-index.xml#http) — блок маршрутов; плюс `#reindex` `##TWO-REINDEX-MODES` и `##TRIGGER-HTTP`; потребитель обещания — `#wire-up` `##WIRE-POST-RECEIVE` |
...
| | |
|---|---|
| @fact:B086-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/modules/vibe-index/PROP-005#config`](spec/modules/vibe-index/PROP-005-package-index.xml#config) — `##CONFIG-PRECEDENCE`; смежно `#cli` `##CLI-SURFACE` (`--data-dir` / `$VIBE_INDEX_DATA_DIR` / умолчание `./vibe-index-data`) |
...
| | |
|---|---|
| @fact:B087-ANCHOR **anchor** | [`spec://org.vibevm.core/vibevm/modules/vibe-index/PROP-005#persistence`](spec/modules/vibe-index/PROP-005-package-index.xml#persistence) — `##ATOMIC-WRITE-PROTOCOL` шаг 4 `##AW-FSYNC-DIR` |
...
| @fact:B088-LOCATOR **locator** | ссылок на план в `spec/**` **четыре**, а не две: `spec/common/PROP-044-change-native-formats.xml:11` (`##PURPOSE`), `:536` (`##SOURCES`), `:3` (комментарий элемента `<status>`) и `spec/research/schema-evolution-2026-08/README-PROVENANCE.xml:22`; панель `tools/self-check.sh` шага, который бы их заметил, не содержит |
| @fact:B088-SEVERITY **severity** | P3 |
| @fact:B088-DISPOSITION **disposition** | `closed` — **закрыта 2026-08-20 (партия P3)**: триггер сработал (change-native план «ИСПОЛНЕНО И СВЁРНУТО» 2026-08-18) — реестр `campaigns/CLOSED-PLANS.toml` (schema 1, первый жилец) + шаг панели `spec does not link to a closed plan` (тупой fixed-string греп path+basename по spec/**, `spec/WAL.xml` исключён по построению, отсутствие реестра — само красное); ТРИ жанра различены по форме: обе живые ссылки (из записанных четырёх две умерли эволюцией раньше) пере-словлены боссом в жанр-3 — имя+дата, без пути. Доказано срабатыванием на хосте: синтетическая ссылка → красный с file:line и коучингом, чистое дерево → зелёный. Примечание посадки: код гейта — воркер codex, принят диффом двух файлов; его самовольный коммит/панель/правки боссовых зон отвергнуты и записаны инцидентом лейна |
...
| | |
|---|---|
| @fact:B090-ANCHOR **anchor** | [PROP-016](spec/common/PROP-016-source-mirrors.xml) и флоу `source-mirrors`: `##EVERY-HOST-IS-A-DOWNSTREAM-READ-REPLICA`, `##HISTORY-REACHES-A-HOST-ONLY-THROUGH-THE-FAN-OUT` |
...
| @fact:B093-LOCATOR **locator** | `cargo xtask codegen` (`xtask/src/codegen/mod.rs`, `run_codegen`), выходные каталоги `crates/vibe-wire/src/generated/` и `packages/org.vibevm.ai-native/core-ai-native/**/src/generated/`; воспроизведение — любой обрыв процесса между очисткой и записью |
...
| | |
|---|---|
| @fact:B094-ANCHOR **anchor** | `spec/design/command-nodes.xml` `#cut` — ruling соседнего места: распознавание идёт по derive самого clap, «потому что пометка, без которой можно добавить подкоманду, есть норма без чекера». Здесь та же норма и тот же дефект |
...
| | |
|---|---|
| @fact:B106-ANCHOR **anchor** | свип конверсии K3/K4 (`spec/terraforms/CONVERT-SOURCE-APPLICATION-v0.1.xml` ##K2-INVENTORY) — остаток «dangling» после условной перезаписи `.md`→`.xml` |
