...
##MAP-WAVES-DIGEST Волны одной строкой (полные составы — в карте): **А** — детерминированная загрузка (B-011 самым высоким приоритетом → B-006/B-031/B-028); **Б** — паритет гейтов и новые классы правил (B-029/033/034/039/030 под циклом B-035 → B-036/037/038 → B-025/026); **В** — карта и её потребители (B-013 → одна смена формата B-019а+B-016.1+B-017 → B-018 → B-020/021); **Г** — хост догоняет дисциплину (B-040, B-005, spec-метки схем) — оппортунистически. Вне волн: B-042 (далёкое будущее), B-015 (запаркована), B-032, B-043. @doc/done
...
- ##B011-COLLISION-REQUIREMENT **Расширение 2026-08-02 — коллизии меток при склейке (из F-217/F-218).**
  ...
  уже чеканит живой адрес `spec://vibevm/boot/STATIC#root` на 26 «корней» сразу
...
| ##B030-DISPOSITION **disposition** | `planned` — **решение владельца 2026-08-02, пункт 2.2 = (а) сейчас + (б) в бэклог, «и дополнительно проверить, как с этим вопросом обстоят дела у Rust и TypeScript — возможно им тоже нужно построить проверки»** |
...
| ##B031-LOCATOR **locator** | 1 384 цитаты вида `spec://vibevm/...`; `specmap.toml` `namespace = "vibevm"`; резолвер `crates/vibe-spec/src/resolver.rs` знает хост как спец-случай |
| ##B031-SEVERITY **severity** | P2 |
| ##B031-DISPOSITION **disposition** | `planned` — **решение владельца 2026-08-02, дословно:** «не логичней ли самому корневому пакету vibevm в vibe.toml дать нормальное fully-qualified имя как у всех остальных пакетов, и дальше чтобы все ссылки работали по обычным правилам, без исключений? Резолвер при этом должен начать учитывать не только структуру внутри spec/packages/vibedeps, но и учитывать адресацию внутри vibe.toml ВЕЗДЕ, включая корневой пакет. Таким образом мы теряем короткую нотацию spec://vibevm/..., но получаем универсальность. Я бы назвал корневой пакет spec://org.vibevm.core» |
...
| ##B033-DISPOSITION **disposition** | `planned` — **решение владельца 2026-08-02 по предъявлению F-185**; его рамка на всю семью B-033/B-034/B-035: «По сути мы не можем писать на Typescript и Go пока не поправим вот это» |
...
- ##B035-NORM **Шаг стройки: поднять принцип в спеку.** «Не слабее Rust без записанной причины» сегодня живёт только этой записью; при первой стройке семьи поднять его в дисциплину owner-approved диффом (дом — решить при аудите), чтобы правило пережило бэклог. Смежность: B-030 — тот же вопрос для одной конкретной идиомы (ассерции соответствия); этот аудит — его обобщение; B-023 — синтаксический tier, отдельная ось.
...
- ##B038-DESIGN **Дизайн-вопрос внутри (решить при постройке, вернуть владельцу).** Принимает ли Rust computed-имена ячеек `{Variant}{Seam}` (как Go, где это правда и есть живая демонстрация), или растовые ячейки именуются свободно и линт проверяет только словарь и уникальность. От ответа зависит текст `:57` — он сейчас стоит как цель.
