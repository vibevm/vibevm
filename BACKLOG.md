...
### B-089 — операторские и потребительские документы индекса не знают ни хэндшейка, ни отказа {#b-089}

| | |
|---|---|
| @fact:B089-ANCHOR **anchor** | флоу `dev-runtime-docs` `##EVERY-SETUP-TOUCHING-CHANGE-UPDATES-THE-DOC-IN-THE-SAME-COMMIT`; предмет — `crates/vibe-index/docs/{format,consumer-protocol,operator-handbook}.md`, названные в [`PROP-005 §3.1`](spec/modules/vibe-index/PROP-005-package-index.md#crate-layout) |
| @fact:B089-LOCATOR **locator** | `docs/format.md` (251 строка): `hello.json` — **0** вхождений, `unavailable` — **0**, `state/journal` — **0**, `rescan-org` — **0**. `docs/consumer-protocol.md` (92 строки): `hello.json` — **0**, `unavailable` — **0**. Контроль непустоты: `repomd` — **4** в каждом из двух файлов, то есть и файлы, и паттерн работают |
| @fact:B089-SEVERITY **severity** | P2 |
| @fact:B089-DISPOSITION **disposition** | `open` — не сделано сознательно: это отдельная связная работа (три файла), а не хвост посадки спек-диффов; смешивать её с §11 значило бы нарушить атомарность коммита |
| @fact:B089-FILED **filed by** | замер при исполнении §11 плана change-native форматов, 2026-08-18 |

- @fact:B089-WHY-THIS-IS-WORSE-THAN-THE-SPEC-VERSION **Почему наружная копия той же лжи хуже внутренней.** `consumer-protocol.md` объявлен «HTTP API reference» — это документ, по которому СТОРОННИЙ разработчик пишет клиента. Клиент, построенный по нему сегодня, не спросит `hello.json` (то есть не узнает ни о мире, ни о переезде индекса) и не обработает `404` с расширением `unavailable` (то есть примет отказ за отсутствие пакета). Спека лжёт своему автору; потребительский документ лжёт чужому.
- @fact:B089-THE-FLOW-MAKES-IT-BINDING **Норма, которая это делает обязанностью, а не пожеланием.** Флоу `dev-runtime-docs` требует править нагруженный документ ТЕМ ЖЕ коммитом, что и изменение, и прямо запрещает откладывать: «deferral is exactly where the drift these files exist to prevent lives». Фазы 6.1 и 6.2 изменили входную точку потребителя и форму ответа и документов не тронули.
- @fact:B089-WHAT-THE-FIX-IS **Что именно чинится.** *(i)* `consumer-protocol.md` — лестница обнаружения (хэндшейк на двух кандидатах, потом `repomd`, потом живой путь), три исхода пробы и форма `unavailable` в теле `404`. *(ii)* `format.md` — `hello.json` в корне, `state/journal/` как правдоносный слой, отсутствие уровня `<kind>/` у `by-name`. *(iii)* `operator-handbook.md` — глагол `rescan-org` и глобальный `--log-level`. Все три уже описаны в PROP-005 после выправки 2026-08-18, так что работа — проекция, а не изобретение.
- @fact:B089-THE-CONTROL-IS-THE-POINT **Как это было найдено, и почему метод стоит повторить.** Нулевые счётчики были прочитаны доказательством только после того, как тот же греп по тем же файлам дал 4 на слове, которое там обязано быть. Без контроля «ноль вхождений `hello.json`» неотличим от опечатки в пути.
