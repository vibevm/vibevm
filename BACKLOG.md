...
| @fact:B117-DISPOSITION **disposition** | `open` — закрыть в T8, первом атоме, который вообще потребляет `declared_path` для матчинга; до него щель не жива, потому что ни один селектор не исполняется (T6b `SelectorSubject` gap). Узкая починка — отвергать `\` в сегменте пути при разборе `SpecAddress`, но это публичный тип со своим радиусом поражения и собственным набором тестов, поэтому она отдельный атом, а не контрабанда в T7 |
