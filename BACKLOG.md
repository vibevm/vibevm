...
### B-061 — `implements`-ребро от объявления заявляет покрытие, которого нет: шов `Watcher` {#b-061}

| | |
|---|---|
| ##B061-ANCHOR **anchor** | `PROP-040 §10` `##file-watch` (REQ, `@spec/done`) + `PROP-014 §2.4` `##EDGE-MODEL-EDGES` (словарь глаголов ребра) |
| ##B061-LOCATOR **locator** | `crates/vibe-settings/src/events/mod.rs:432` — `pub trait Watcher` с `specmark::spec(implements = "…PROP-040#file-watch")` на трейте И на методе; `impl Watcher for` во всём дереве совпадает ровно дважды, и оба — тестовые дублёры (док-пример `events/mod.rs:407`, `Noop` в `events/tests.rs:250`); вне модуля трейт не упоминает никто — измерено грепом по `crates/` и `xtask/` |
| ##B061-SEVERITY **severity** | P2 |
| ##B061-DISPOSITION **disposition** | `open`; докблок приведён к измеренному 2026-08-05, вопрос о глаголе ребра НЕ решён — это решение о словаре карты, а не правка |
| ##B061-FILED **filed by** | босс-дизайн B-040 по цензусу швов, волна Г, 2026-08-05 |

- ##B061-SUT **Суть, по-простому.** Трейт объявляет форму наблюдателя за файлом и несёт метку «реализует REQ `#file-watch`». Реализации нет ни одной — только тестовые заглушки. Но карта не различает ребро от объявления и ребро от работающего кода: в индексе и то и другое выглядит как «этот REQ кем-то реализован». Покрытие заявлено формой, а не доставлено ею. @impl/done
- ##B061-THE-DOCBLOCK-SAID-OTHERWISE **Как это держалось.** Докблок трейта прямо утверждал, что конкретная реализация (`notify` + дебаунс) «живёт в хосте (`vibe-cli`)». Не живёт: греп по всему дереву даёт два совпадения, оба тестовые. Читатель, проверявший шов по комментарию, получал ответ «построено» три раза подряд — от комментария, от метки и от карты. @impl/done
- ##B061-WHY-IT-IS-NOT-JUST-A-COMMENT **Почему это не просто устаревший комментарий.** Дисциплина держится на том, что покрытие REQ измеримо. Если ребро от голого объявления считается покрытием, то любой непостроенный шов, у которого написан трейт, выглядит закрытым — и чем аккуратнее проект объявляет формы заранее, тем больше у него ложного покрытия. Это дефект класса «сигнал есть, содержания нет», а не опечатка. @spec/done
- ##B061-FIX-SHAPE **Форма починки — вопрос словаря, а не строки.** Варианты, каждый со своей ценой: *(1)* новый глагол ребра («declares» / «specifies-shape») — честнее всего, но расширяет словарь `PROP-014 §2.4`, а он общий для трёх языков и вендорится; *(2)* правило «`implements` на трейте без прод-реализации — находка гейта» — не трогает словарь, но требует, чтобы гейт умел считать реализации трейта, чего он сегодня не умеет; *(3)* снять метку с объявления и вешать её только на реализацию — дёшево и теряет связь «вот где обещано». Выбор — за проектированием, не за этой записью. @spec/plan
- ##B061-RELATED **Смежность.** [B-060](#b-060) — тот же жанр: назначенная единица разметки, которой разметки нет. Оба найдены измерением того, что записано как построенное. @doc/done

### B-060 — назначенная единица разметки не размечена и не может быть прочитана: схемы JTD {#b-060}

| | |
|---|---|
| ##B060-ANCHOR **anchor** | `PROP-014 §2.3` `##RULE-GENERATED-CODE-IS-EXCLUDED` — «generated code is excluded from orphan checks via a directory marker; the *generator input* (JTD schema file) is the taggable unit instead» |
| ##B060-LOCATOR **locator** | 0 из 7 `schemas/*.jtd.json` несут spec-адрес (`grep -rn "spec://" schemas/` → rc=1); сканер сравнивает расширение буквально — `rscan.rs:333` (`!= Some("rs")`), `mdspec.rs:422` и `:498` (`!= Some("md")`), `ratchet.rs:78` (`!= Some("rs")`), то есть `.json` не открывается ни одним обходом; `CodeItem` (`generated/specmap/mod.rs:36`) не имеет поля адреса — принадлежность выражена ребром от СИМВОЛА кода к URI |
| ##B060-SEVERITY **severity** | P2 |
| ##B060-DISPOSITION **disposition** | `open`; текст правила и обе конфиг-копии приведены к измеренному 2026-08-05, постройка сканера не начата — стоячее слово владельца по F-132: «сделать как будет возможность» |
| ##B060-FILED **filed by** | замер долга схем F-132, волна Г, 2026-08-05 |

- ##B060-SUT **Суть, по-простому.** Крейт `vibe-wire` изъят из гейта дисциплины, и причина изъятия ссылается на то, что размечать надо не сгенерированный код, а вход генератора — семь JTD-схем. Первая половина правила настоящая: пути с `/generated/` действительно пропускаются. Вторая — решение, которое никто не исполнил и сегодня исполнить нечем: схемы тегов не несут, а если бы несли, их бы никто не прочитал. Получилось изъятие, чьё оправдание ссылается на механизм, которого нет. @impl/done
- ##B060-WHY-THE-CHEAP-FIX-IS-A-WISH **Почему дешёвая починка — не починка.** У каждой схемы есть верхнеуровневый блок `metadata`, так что ключ `spec` кладётся туда одной строкой — и не меняет ничего, потому что сканера для JSON нет. Дописать ключ значило бы получить разметку, которую не видит ни карта, ни гейт, ни ratchet: ровно тот жест «правило без чекера — это пожелание», который дисциплина запрещает. @spec/done
- ##B060-WHAT-A-REAL-FIX-COSTS **Что стоит настоящая починка — измерено, с честной вилкой.** Новый `impl CodeScanner` (трейт и `CompositeScanner` существуют ровно для этого) ~50–80 строк; три места композиции захардкожены на `&RustScanner` (`index.rs:57-58`, `:341`, `:368`); host-политика `specmap.toml` не включает `schemas` в `scan_roots` (одна строка конфига); паритет ratchet — ещё ~15–20 строк, если теги схем должны быть ему видны. Вилка не сходится в одно число по существу, а не по лени: у «тега схемы» нет однозначного дома в модели — `CodeItem` не носит адреса, тег есть ребро от символа, а у JSON-документа символа нет. Сперва решается, что считать «единицей» схемы, и только потом считаются строки. @impl/done
- ##B060-THE-VERDICT-WAS-CONFIRMED-ON-HALF **Как это прожило кампанию.** Факт был просужен `confirmed`, и обе его доказательные ссылки — `rscan.rs:314` и `ratchet.rs:82` — про первую половину предложения. Вторая клауза не получила ни одной ссылки и никем не проверялась: составной факт с двумя проверенными адресами выглядит проверенным целиком. Это способ, которым ложное утверждение проходит суд, не солгав ни одной строкой доказательства. @impl/done
- ##B060-RELATED **Смежность.** F-132 (запись долга, чья предпосылка называла несуществующий файл `schemas/specmap.jtd.json` — на самом деле `specmap`-схема живёт в пакете движка). [B-059](#b-059) — соседняя болезнь того же рода: конфиг, который выглядит работающим и молча не делает ничего. @doc/done
...
- ##B056-LANDED **Построено 2026-08-05 — четырьмя посадками, плюс пятая, которой в дизайне не было.** Свёртка приняла последовательность (`fold_sources`, старое имя — вырожденный случай, все прежние тесты прошли без правок); конвейер стал передавать ВСЕ директивы в порядке объявления (этим закрыт [B-055](#b-055)); закон циклов дотянут до рёбер `#source` тем же обходчиком — второго не появилось; резолвер научился перечислять по образцу с обязательной сортировкой; и наконец рёбра `#source` свелись в ОДНУ функцию, через которую ходят и страж, и свёртка. Итог по крейту: **206 юнит-тестов** (замер `cargo test -p vibe-spec` 2026-08-05; строка «202» держалась до пятой посадки и её тестов), конформ 0 новых находок, длины файлов в бюджете (`use_graph.rs` 590 из 600 — следующая правка там начинается с разреза). @impl/done
- ##B056-WHAT-THE-BUILD-REFUTED **Стройка опровергла два собственных допущения дизайна, и оба нашлись измерением, а не ревью.** Первое: «дубль секции-только-в-источнике поймает пост-слияночная проверка уникальности» — не поймает, потому что гейт намеренно терпит повтор ЗАГОЛОВКА (в слитом виде он неотличим от законного `:add`-склеивания), и провенанс к тому моменту потерян; ловить обязана свёртка, которая ещё знает, кто что принёс. Второе: «дедупликация есть по построению обхода» — обход дедуплицирует УЗЛЫ, а свёртка есть текстовое включение, и одно не влечёт другого: в ромбе общий источник входил в документ дважды, что безобидно для прозы и смертельно для фактов — обычная композиция двух плагинов над общей базой падала бы на дубле якоря. Отсюда страж включения: текст узла входит ровно один раз, первым путём детерминированного порядка. @impl/done
...
| ##B055-DISPOSITION **disposition** | **`closed` 2026-08-05** — снят посадкой 2 стройки [B-056](#b-056); см. `##B055-LANDED` |
...
- ##B055-LANDED **Закрыто 2026-08-05 второй развилкой, а не первой.** Постройка B-056 сделала выбор в пользу честной поддержки нескольких источников: конвейер собирает ВСЕ директивы `#source` в порядке объявления и передаёт их свёртке, которая теперь принимает последовательность. Отказ на второй директиве не понадобился — он был бы дешевле ровно до того момента, как владелец заказал плагинную форму, где несколько источников и есть смысл механизма. Вырожденные пути остались байтовыми: документ без `#source` и документ с одной `#source` дают ровно прежний результат, и все прежние тесты конвейера прошли без правок. Попутно вылечена и диагностика: при пяти источниках недостижимый называет СЕБЯ, а не seed и не первого. @impl/done
