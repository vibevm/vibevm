...
| ##B062-DISPOSITION **disposition** | `open`, **развилка владельца** — расширение корпуса это решение о ЦЕНЕ (около 400 якорей к первому суду), а не правка |
| ##B062-FILED **filed by** | волна Г, 2026-08-05 — всплыло при закрытии реестрового долга B-056 |

- ##B062-SUT **Суть, по-простому.** Эти документы размечены так же, как спеки: у каждого факта якорь и статус вроде `@impl/done`, то есть «построено». Но кампания их не видит вовсе, поэтому статус никем не проверяется и вердикта не получает. Разметка выглядит как контракт и работает как украшение. @impl/done
