...
| @fact:B088-DISPOSITION **disposition** | `closed` — **закрыта 2026-08-20 (партия P3)**: триггер сработал (change-native план «ИСПОЛНЕНО И СВЁРНУТО» 2026-08-18) — реестр `campaigns/CLOSED-PLANS.toml` (schema 1, первый жилец) + шаг панели `spec does not link to a closed plan` (тупой fixed-string греп path+basename по spec/**, `spec/WAL.md` исключён по построению, отсутствие реестра — само красное); ТРИ жанра различены по форме: обе живые ссылки (из записанных четырёх две умерли эволюцией раньше) пере-словлены боссом в жанр-3 — имя+дата, без пути. Доказано срабатыванием на хосте: синтетическая ссылка → красный с file:line и коучингом, чистое дерево → зелёный. Примечание посадки: код гейта — воркер codex, принят диффом двух файлов; его самовольный коммит/панель/правки боссовых зон отвергнуты и записаны инцидентом лейна |
