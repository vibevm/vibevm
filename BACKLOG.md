...
| ##B056-DISPOSITION **disposition** | `planned` — **предложение владельца 2026-08-04:** «можем ли мы сделать полноценное множественное наследование для контрактных документов… и даже систему плагинов: `#source spec://org.vibevm.plugins/plugin-*`, дальше подхватываются все подходящие спеки и тела соответствующих секций склеиваются» |
...
- ##B056-THE-HOLE **Где дыра — `:replace` при N источниках.** Сегодня он значит «контрактный текст игнорируется, канонический — текст источника»; при нескольких источниках определённый артикль теряет смысл. Решить: выбрасывает ли `:replace` только контрактную сторону (а источники между собой продолжают складываться по порядку), или последний `:replace` съедает и предыдущие. Склонность — первое, плюс «два источника `:replace`-ят один якорь → ошибка сборки», по той же логике громкости, что у фактов. Это решение владельца, не вывод. @spec/plan
- ##B056-ORDER-AND-RECURSION **Порядок и рекурсия — два обязательных решения.** Порядок не наследуется от файловой системы: явный список — порядок объявления, глоб — обязательная сортировка, иначе детерминизм компилятора ломается. Рекурсия (источник, который сам контракт) сегодня отсутствует; её надо либо поддержать со стражем циклов, либо запретить явно — дом для стража в спеке компилятора уже есть. @spec/plan
