...
| @fact:B079-DISPOSITION **disposition** | `planned`, **CLI-половина закрыта 2026-08-20 (марафон, слайс С2.5, воркер R2-ENVELOPES-CLI + боссов хвост)**: семь конвертов `--json` (`get list search capabilities purls outdated verify`) получили JTD-схемы `schemas/index_cli/e1/**` (словари `version_entry`/`package_kind`/`group`/`version` реюзом), семь записей `[format.index-cli-*-report]` в реестре, 8 авторских корпусных документов (дословный stdout реального бинаря против детерминированной фикстуры, `cmp` байт-в-байт), сгенерированные ридеры (FormatId → 31), parity-тесты `wire_parity_index_cli.rs` (round-trip + writer==corpus + громкий отказ на сломанной форме; неизвестное ПОЛЕ сознательно терпимо — `foreign_parsers="many"`). Боссов хвост: `verify.expected_size/actual_size` переведены на строковый канон по правилу `##M-WIDE-INTEGERS-AS-STRINGS` (u64-модуль поднят в общий `types/wire_decimal.rs` — второе применение, как и предсказано первым). Остатки: **HTTP-половина `/v1/**` — вторым пакетом** (реестр это прямо говорит шапкой секции); `unavailable` — кандидат в `formats/vocabularies.json` (сейчас локальная definition в шести схемах); `list.kind` — plain string+nullable, пока postproc не выучит nullable-enum (записано в description) |
...
| @fact:B092-DISPOSITION **disposition** | `open` — правка механическая (заменить префикс на трёх строках), но заводится строкой, а не делается попутно: она принадлежит гигиене адресуемости, а не слайсу снятия пакетов, и её настоящая цена — чекер, а не три замены |
...
| @fact:B093-DISPOSITION **disposition** | `open` — форма починки известна и названа собственным уроком проекта (писать в новый каталог и переставлять указатель), но задевает генератор, а не вызов, и потому делается отдельным шагом |
...
| | |
|---|---|
| @fact:B094-ANCHOR **anchor** | `spec/design/command-nodes.md` `#cut` — ruling соседнего места: распознавание идёт по derive самого clap, «потому что пометка, без которой можно добавить подкоманду, есть норма без чекера». Здесь та же норма и тот же дефект |
...
| @fact:B094-DISPOSITION **disposition** | `open` — починка требует `clap` в dev-зависимостях крейта (сегодня его там нет) и, значит, движения `Cargo.lock`; это отдельная посадка, а не хвост чужой |
