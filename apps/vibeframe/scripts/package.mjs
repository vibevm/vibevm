#!/usr/bin/env node
/**
 * vibeframe packaging — produce a relocatable, self-contained directory that the
 ...
 *
 * Output: `<out>/vibeframe-<plat>-<arch>/` — `electron(.exe)` at the root,
...
const out =
  outIdx >= 0 && process.argv[outIdx + 1]
    ...
    : resolve(appDir, '..', 'build', 'vibeframe-dist');
...
const children = readdirSync(out).filter((n) => n.startsWith('vibeframe-'));
if (children.length !== 1) {
  process.stderr.write(
    `[package] expected exactly one 'vibeframe-*' child under ${out}, found ${children.length}: ${children.join(', ')}\n`,
  );
  process.exit(1);
}
