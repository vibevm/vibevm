/**
 * vibeframe — Electron main process.
 *
 ...
 * constructed — `@xterm/headless` is not even required — and vibeframe behaves
...
process.on('uncaughtException', (err) => {
  console.error('[vibeframe] uncaughtException:', err);
});
 ...
 * still points at this pid (a newer vibeframe may have overwritten it).
 */
 ...
 * never touches it — vibeframe runs even when that dependency is absent.
 */
function startControlServer(initialCols, initialRows) {
  ...
  const server = http.createServer((req, res) => {
    ...
    Promise.resolve()
      ...
      .catch((err) => {
        console.error('[vibeframe] control request error:', err);
        try {
  ...
  server.on('error', (err) => {
    console.error('[vibeframe] control server error:', err);
  });
  ...
  server.listen(0, '127.0.0.1', () => {
    ...
    try {
      ...
      console.error('[vibeframe] failed to write discovery file:', err);
    }
    console.error(`[vibeframe] AIUI control server on http://127.0.0.1:${port}`);
  });
}
...
async function createWindow() {
  ...
  const spawnPty = (spawnCols, spawnRows) => {
    ...
    ptyProc = pty.spawn(file, args, {
      ...
      //
      // `VIBEFRAME=1` tells a nested `vibe tree` it is already inside vibeframe, so
      ...
      env: { ...process.env, COLORTERM: 'truecolor', VIBEFRAME: '1' },
    });
  ...
  ipcMain.on('vibeframe:set-icon', (_event, name) => {
...
app.whenReady().then(createWindow).catch((err) => {
  console.error('[vibeframe] failed to start:', err);
  quit();
});
