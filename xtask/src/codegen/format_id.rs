...
pub(super) fn emit_format_id(root: &Path, out_dir: &Path) -> Result<()> {
    ...
    out.push_str("mod properties;\n");
    let module = out;
    let mut properties = String::new();
    properties.push_str(
        "// Generated by `cargo xtask codegen` from `formats/REGISTRY.toml`. DO NOT EDIT.\n\n",
    );
    properties.push_str("use super::{ForeignParsers, FormatId, UnknownFields};\n\n");
    let out = &mut properties;
    ...
    super::write::write_generated(&target, &module)?;
    super::write::write_generated(&sub_out.join("properties.rs"), out)?;
