...
//!
//! This file is the driver plus the first pass; the other two — renaming
//! field identifiers to snake_case (dropping the identity renames), and
//! opening vocabularies per the schema's `x-vocabulary` — live in the
//! sibling `snake_case` and `open_vocabulary` modules, split along those
//! responsibility seams as the pair outgrew the 600-line budget.
//!
...
//! `pub <ident>: …` field line), so it runs second. Opening vocabularies
//! then writes hand-rolled `impl Serialize` / `impl Deserialize` blocks
//! into the file — text the pinned emission shape does not contain — and
//! any shape-keyed pass running after it would be reading a document
//! that is no longer the generator's. The passes also never read their
//! own output: `generate_into` wipes and regenerates the tree before
//! every run, so what they see is always fresh generator emission.
//!
...
/// Read `file`, run all three post-processing passes over it, write the
...
/// pass is keyed to the shape too, so it runs second; opening
...
pub(crate) fn rewrite_generated(file: &Path, resolved: &Path, schema: &Path) -> Result<()> {
    ...
    let opened = open_vocabularies(&snaked, &name, resolved, schema)?;
