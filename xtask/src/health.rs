//! `cargo xtask health` — the Discipline health collector (DISCIPLINE-SWEEP
//! v0.1's fact-gatherer).
//!
...
//! diff IS the health delta. No LLM; no network by default — the optional
//! `--mirrors` flag is the sole exception. Automated fact-gathering is
//! itself a Discipline value (a check that could be a checker is a WISH until
//! it is one).
//!
//! Output: a machine-readable JSON snapshot (default
//! `terraform/health/latest.json`) plus a human summary on stdout. Advisory
...
pub(crate) fn run_health(out_rel: &str, mirrors: bool) -> Result<()> {
    ...
    let mut report = json!({
        "schema": 1,
        "note": "Discipline health snapshot — generated by `cargo xtask health` \
                 (DISCIPLINE-SWEEP v0.1's fact-gatherer). Advisory early-warning / \
    ...
    // Optional, off-by-default network probe (keeps the default run
    // deterministic + offline): is every mirror target level with mainline?
    if mirrors {
        let (head, statuses) = sync_report(&root)?;
        println!(
        ...
        for s in &statuses {
            let (label, state, remote) = match &s.state {
                SyncState::InSync => ("sync   ", "in_sync", Value::Null),
                SyncState::Drift(sha) => ("DRIFT  ", "drift", json!(sha)),
                SyncState::Missing => ("MISSING", "missing", Value::Null),
            };
            println!("  {label}{}", s.name);
            target_rows.push(json!({ "name": s.name, "state": state, "remote_main": remote }));
        }
        if let Some(obj) = report.as_object_mut() {
            obj.insert(
                "mirrors".to_string(),
                json!({ "local_main": head, "targets": target_rows }),
            );
        }
    }
