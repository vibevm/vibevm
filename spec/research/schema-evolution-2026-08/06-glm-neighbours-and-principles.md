...
Второй независимый разбор по теме **G1-NEIGHBOURS**. Предмет — наш каталог
...
Прежде чем отвечать на вопросы — фиксирую расхождения со сводом, потому что на
них держат часть дальнейшего.

**1. Объединение `RepomdFileEntry` — не «без признака».** Свод `[ИЗ СВОДА]`
...
**2. «15 мест `deny_unknown_fields`» — верно.** Пересчитал в
`crates/vibe-index/src/types/`: `repomd` (1) + `relations` (6) + `entry/mod`
(1) + `content` (5) + `aggregate` (2) = **15** `[ИЗ ДЕРЕВА]`. Совпало.

**3. «5 закрытых словарей» — я насчитал 4.** В типах каталога строковых
...
**4. Спека противоречит не только коду, но и себе — и это сильнее, чем в своде.**
...
- `FORWARD-COMPAT` `[ИЗ ДЕРЕВА: spec/modules/vibe-index/PROP-005-package-index.md:302]`:
  «readers of v2 written by an old vibevm gracefully ignore unknown fields»,
  статус `impl/done`.
- `NEVER-SILENT-SCHEMA` `[ИЗ ДЕРЕВА: PROP-005-package-index.md:624]`:
...
Это одно и то же событие (старый читатель видит запись новой версии) с двумя
...
**5. Толерантность клиента — не «уже правильны», а неполна и притом случайна.**
...
Свод перечисляет приёмы поодиночке: тегировать объединения, писать пустое
...
> **Долговечность формата — это дисциплина адресованного неведения: каждой
...
Приёмы выводятся из него как «где живёт данная форма неведения»:
...
Королларий, отличающий выживших: они **назначают границу сознательно и пишут
...
Из принципа сразу следует проверка любого решения: **«куда приземлится
...
Свод сравнивает приёмы; я сравниваю **структурные условия**, потому что именно
они делают чужой приём неприменимым.

**Отличие 1 — Автор один, и это инструмент, а не открытая человеческая подача.**
...
**Отличие 2 — На критическом пути чтения нет живого сервера.** Потребитель
берёт **статический файл** по raw-URL или git clone
(`[ИЗ ДЕРЕВА: PROP-005-package-index.md:66]` — raw-URL fetchable без clone;
...
**Отличие 3 — Читателей сегодня ноль, и они перечислимы (кривая стоимости
...
**Отличие 4 — Каталог и описываемый им артефакт делят одну git-историю.**
...
**Аналогия Avro-в-git внутренне противоречива самому тезису свода.** Свод
...
**Бонус — Cloudflare как «ближайший аналог».`[ИЗ СВОДА]` называет разбор
...
В теме соседи перечислены, но **анализ** в `Части 2` свода дан лишь для
PyPI/PEP, NuGet и Homebrew. Самый близкий нам структурный двойник назван в
теме, но **не разобран**:

- **Индекс crates.io** `[ПО ПАМЯТИ, НЕ ПРОВЕРЕНО]` — это тоже JSON-in-git
...
Два дополнительных, под наши подслючаи:

- **Helm `index.yaml`** `[ПО ПАМЯТИ, НЕ ПРОВЕРЕНО]` — сгенерированный индекс
...
«Путь» в нашем случае — это **путь файла в git-репозитории индекса**, он же
raw-URL: `repomd.json`, `primary.jsonl`, `by-name/<name>.json`
(`[ИЗ ДЕРЕВА: PROP-005-package-index.md:66, 118-205]`). «Новый путь» для
ломающего изменения = публиковать каталог v2 по **другому пути**, оставив v1
замороженным на старом:

- вариант пути: подкаталог `v2/repomd.json` (или суффикс `repomd.v2.json`, или
  ветка/тег `index-v2`);
- старый потребитель идёт по `repomd.json` — там по-прежнему валидный v1;
- новый потребитель идёт по `v2/repomd.json`.

**Что это стоит.** (1) **Двойная запись** на каждом reindex — писать приходится
...
**Что происходит с тем, кто ходит по старому пути через год.** Он получает
...
**Почему нам это дешевле, чем PyPI/NuGet** (отличие 2 + 4 из §2): у живого
...
NuGet `[ИЗ СВОДА]`: документированные ресурсы вечны, недокументированные
...
- **Верхний уровень (вечные): значение, на котором потребитель ВЕТВИТСЯ —
...
- **Нижний уровень (свободные к эволюции/удалению, несут «неизвестное»):
...
**Пробел, который вскрывает упражнение.** У NuGet нижний уровень — это **место**
...
**Свидетельство, что границу не проводили сознательно.** `delivery` — строгий
...
1. **`RepomdFileEntry` — не «без признака»**: тег `kind` вшит в вариант
...
- **JTD-codegen уже частично построен** (`vibe-wire`, `schemas/`, `tools/jtd-codegen`)
