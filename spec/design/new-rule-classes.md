...
@fact:standing-on The measurement this design stands on — three read-only censuses
...
Every number below is theirs unless it says «boss-measured». @status:doc/done

@fact:batch-shape **The batch's shape in one paragraph.** Three builds that look
...
So batch 3 is one conveyor: **card + checker + exhibit, three times.** @status:doc/done

## 1. What the censuses changed about the plan {#census-changed}

@fact:finding-no-severity **There is no severity class in the engine, and B-036
...
satisfies. @status:spec/done

@fact:finding-denominator-exists **The position denominator already ships.**
...
the comment's line. @status:spec/done

@fact:finding-comment-walks-exist **Two of three extractors already walk every
...
text, so a raw scan is a local addition, not an architecture change. @status:spec/done

@fact:finding-vocabulary-is-absent **The marker vocabulary the guides evoke is
...
exhibited on fixtures**, because the live tree will not exhibit it. @status:spec/done

@fact:finding-toolchain-pins-stable **`rust-toolchain.toml` pins `stable`, and it
...
day: no nightly toolchain installed, `cargo dylint` not present. @status:spec/done

@fact:finding-ts-vehicle-is-present **The TS vehicle is already in the tree.**
...
flat config with **no CLI change to the floor**. @status:spec/done

## 2. B-036 — the invariant-comment position rule {#b036}

@fact:b036-rule **The rule.** `invariant-comment-position`: a comment carrying an
...
guide's own: move it to the file's top or bottom, or split the file. @status:spec/done

@fact:b036-inputs **Inputs, decided.** Denominator `Fact::FileMetrics { lines }`
...
frontend versions, which re-extracts. Budgeted, not discovered. @status:spec/done

@fact:b036-config **Where the knobs live.** Two ROOT keys, beside `max_file_lines`
...
`NEVER`), minimum length = 120 lines. @status:spec/done

@fact:b036-fingerprint **Fingerprint: never line-keyed.** The stop.rs lesson
(`budget.rs:71-75`) applies exactly — key by file + marker + per-file ordinal,
so an edit above the comment does not re-key a frozen baseline entry. @status:spec/done

@fact:b036-scope Scope like `FileLength`: `in_src` only, test context out. @status:spec/done

@fact:b036-exhibit **Exhibit, because the live tree will not.** The dirty fixture of
...
same landing. @status:spec/done

## 3. B-037 — the custom REQ-citing lint layer {#b037}

@fact:b037-promise **What is promised, per language, verbatim.** Rust: «custom
...
(`diagnostic-cites-req`) that does not exist. @status:spec/done

@fact:b037-grammar-is-the-contract **The grammar is the contract, and it already
...
bug this channel exists to prevent. @status:spec/done

@fact:b037-ts-build **TypeScript — BUILD, fully.** A plugin package beside
...
`warn`, and why.** @status:spec/done

@fact:b037-rust-position **Rust — the vehicle is blocked by a toolchain policy, and
...
named route, and no rule is quietly relaxed. @status:spec/done

@fact:b037-go-position **Go — no vehicle is promised, and the honest move is to
...
`analysis.Analyzer`. Recorded here, routed with the Rust half. @status:spec/done

@fact:b037-doc-correction **The guides' clause is corrected in the same landing.**
...
and Go's state what enforces the grammar today and name the routed build. @status:spec/done

## 4. B-038 — the pending cards get cards and checkers {#b038}

@fact:b038-fork **The owner's fork №1, taken 2026-08-04: computed cell names.** The
canonical cell name is `Pascal(variant)` followed by **the seam spelled as
written** — `SatDepSolver`, not `SatDepsolver`; the naive
«pascal both halves» composer mangles multi-word seams and is not the rule. @status:spec/done

@fact:b038-fork-cost **The cost, boss-measured over the WHOLE tree** (the census's
...
every rename is compiler-checked and internal. @status:spec/done

@fact:b038-one-rule-two-languages **One engine rule serves Rust and Go.** Go
...
carries a recorded reason — it has no cell manifest to compute from. @status:spec/done

@fact:b038-lands-frozen **The rule lands with a frozen baseline; the renames ride a
...
having). @status:spec/done

@fact:b038-r060 **R-060 — the card is new, and its checker lands vacuous.** «Test
...
which declared-matrix idioms you deliberately treat as compliant.** @status:spec/done

@fact:b038-id-space **The id space, measured: the R3-series registry IS `ATLAS.md`**
...
and is filed to the backlog rather than fixed in passing. @status:spec/done

## 5. What this batch deliberately does not do {#non-goals}

@fact:non-goal-severity No severity/warning model in the engine (§1 — the ratchet
already carries the semantic). @status:spec/done

@fact:non-goal-seven-cards Not all seven pending cards — three of them plus R-060's
new one. The remaining four ride the same conveyor later, which is exactly how
`01-PATTERN-CARD-FORMAT.md` describes its own scope. @status:spec/done

@fact:non-goal-vocabulary No closed-token vocabulary and no synonym/shadow
detector — the fork's variant B needed all three and the owner did not take it;
the naming rule checks composition, which needs none of them. @status:spec/done

@fact:non-goal-nightly No nightly toolchain and no `dylint` library until the owner
rules on the toolchain question (§3). @status:spec/done
