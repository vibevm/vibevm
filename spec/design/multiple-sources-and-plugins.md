...
##plugins-only-enumeration-is-new **The glob adds exactly one capability.** `#source spec://org.vibevm.plugins/plugin-*` needs the resolver to enumerate installed packages matching a pattern instead of resolving one coordinate. Everything downstream is the sequence fold of §3. @impl/done
