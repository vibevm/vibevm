...
##origin-question PROP-007 shipped the workspace data model (M1.17) but deliberately left one question
...
into which member's `spec/` does its content land? @doc/done

##scope-of-answer This session answered it. The answer turned out to be far larger than picking a
...
##narrow-question-illusion PROP-007 §6 q3 reads as a narrow placement question. It is not. @doc/done

##flat-model-shape vibevm's boot model ([`VIBEVM-SPEC.md` §6](../../VIBEVM-SPEC.md)) is a **flat, single,
...
one boot sequence → one entry point. @doc/done

##workspace-breaks-lead A workspace breaks every one of those assumptions at once: @doc/done

- ##break-n-nodes **N nodes** — the root plus every member. @doc/done
- ##break-n-entry-points **N entry points** — a developer `cd`s into any member and opens an agent there.
  This is PROP-007's load-bearing principle: "the user works in a sub-project and
  doesn't notice it is part of something bigger." @doc/done
- ##break-n-boot-sequences **N boot sequences** — each entry point needs a sequence coherent *for that node*. @doc/done
- ##break-shared-deps **One shared dependency set** — unified resolution (PROP-007 §2.4) means one version
  of each external dependency across the whole workspace. @doc/done

##flat-model-verdict The flat model cannot be stretched over this. It must be replaced. @doc/done

##OWNER-CONSTRAINT-NO-REWRITE The owner's hard constraint, stated at the outset: **installing a dependency must
...
naïve "bubble everything into the root `spec/boot/`" would amount to. @doc/done

---

## 3. The owner's mental model — boot as linking

##linking-frame The owner framed the loading mechanism directly as **static vs dynamic linking**, and
that framing became the spine of the design. @doc/done

| C++ / the linker | vibevm |
|---|---|
| ##ROW-LINK-COMPILE Compile + link time @doc/done | `vibe install` @doc/done |
| ##ROW-LINK-LOAD Load + run time @doc/done | start of an AI session @doc/done |
| ##ROW-LINK-OBJECT Object file / library @doc/done | a package (flow / feat / stack / tool) @doc/done |
| ##ROW-LINK-EXECUTABLE The executable @doc/done | a node's effective boot sequence @doc/done |
| ##ROW-LINK-LOADER The dynamic loader (`ld.so`) @doc/done | the AI agent following INCLUDE pointers @doc/done |
| ##ROW-LINK-DT-NEEDED `DT_NEEDED` entry @doc/done | an INCLUDE pointer (a `spec://` reference) @doc/done |
| ##ROW-LINK-STATIC Static linking (`.a` copied into the binary) @doc/done | `vibe install` inlines a dependency's boot @doc/done |
| ##ROW-LINK-DYNAMIC Dynamic linking (`.so` referenced, resolved at load) @doc/done | the INCLUDE pointer stays; the agent resolves it @doc/done |
| ##ROW-LINK-VENDOR `vendor/` / `~/.cargo/registry` @doc/done | the materialised-dependency tree @doc/done |

##AGENT-IS-THE-LOADER The load-bearing consequence: **at session start, the AI agent is the loader.** Every
...
dynamically-linked one needs the loader to find and map every `.so`. @doc/done

---

## 4. The model — four principles

##P1-TWO-TREES **P1 — Two trees.** A node's authored `spec/` (only the node's author writes it) and
...
**once** for the whole workspace. @doc/done

##P2-COMPUTED-SEQUENCE **P2 — The boot sequence is computed, not assembled by hand.** Each node has an
...
the hierarchy gives cost-scoping for free. @doc/done

##P3-INDEX-PER-ENTRY-POINT **P3 — One generated index per entry point; the agent never walks the graph.**
...
the zero-dependency cross-agent property that is the whole point of VIBEVM-SPEC §6.1. @doc/done
...
##forks-lead Four forks were put to the owner. The options, the resolution, the reasoning. @doc/done

### Fork 1 — the form of static boot delivery

##fork1-options Options offered: a single inlined aggregate (cheapest at boot, but duplication and
the lost-in-the-middle risk §6.4 already rejected); a resolved path index (clean, but
1 + N reads); a hybrid. @doc/done

##FORK1-REFINED-HYBRID **Resolution — a refined hybrid.** Not one of the three: a model where `inline`,
...
the must-not-be-missed content physically first. @doc/done

### Fork 2 — where materialised dependencies live

##FORK2-COMMITTED-TREE **Resolution — a committed dependency tree.** A fresh clone is bootable with no
...
believed stale or a previous index pass was wrong. @doc/done

### Fork 3 — uniform vs workspace-only

##FORK3-UNIFORM **Resolution — uniform.** A single-package project is a degenerate (zero-member)
...
no-legacy hard break already set the precedent. @doc/done

### Fork 4 — milestone scope

##FORK4-UNIFIED-VIEWS **Resolution — boot + effective spec, unified.** The boot index and the `vibe build`
...
build") are two views it emits. @doc/done
...
- ##GEN-INDEX-MD **`INDEX.md`** — the ordered, resolved manifest of the rest of the boot sequence.
  ...
  at boot. @doc/done
- ##SESSION-START-ORDER Session-start order: `CLAUDE.md` → `STATIC.md` → `INDEX.md` and the files it names. @doc/done

##cost-profile-lead Cost profile: @doc/done
...
the loading model generalises subskill delivery rather than inventing a parallel axis. @doc/done

---

## 7. Consequences and findings

- ##FINDING-NUMBERING **Numbering.** The `NN-` prefix namespace (10–89, author-chosen) cannot survive a
  ...
  *category* (the existing range bands — foundation / flow / stack / user-override —
  ...
  impossible by construction. @doc/done
- ##FINDING-MIRROR-BREAKAGE **Mirror-layout breakage.** VIBEVM-SPEC §13.1's mirror layout — source path = target
  ...
  become package-relative or `spec://` URIs, rewritten by `vibe` at materialisation. @doc/done
- ##FINDING-PUBLISH-REGEN **Published-copy regeneration.** A package published by `vibe workspace publish` is
  ...
  an index-regeneration step. @doc/done
- ##FINDING-DOGFOOD **vibevm dogfoods itself.** The vibevm repository is itself a vibevm project;
  ...
  `INDEX.md` join them. The migration is part of the milestone. @doc/done

---

## 8. What this supersedes and parks

- ##SUBSUMES-INSTALL-BUILD **Workspace-aware `vibe install` / `vibe build`** (PROP-007 §9.3, §6 q3) is no longer
  a separate deferred item — it is *subsumed*: it becomes the install/build half of
  PROP-009. @doc/done
- ##parks-version-workspace **`version = { workspace = true }`** (PROP-007 §6 q4) and the **publish-signalling
  polish** (`--archive` etc., PROP-007 §9.3) are parked behind PROP-009 — recorded, not
  dropped. @doc/done
- ##prop-008-unaffected **PROP-008** (qualified naming) is unaffected; it still follows PROP-005 (index). @doc/done

---

## 9. Session log

- ##session-log-2026-05-21 **2026-05-21.** Session restored from `CONTINUE.md` + `spec/WAL.md`. The owner
...
- ##ptr-prop-003 [PROP-003 §2.5](../modules/vibe-resolver/PROP-003-dep-evolution.md) — subskills and delivery modes. @doc/done
