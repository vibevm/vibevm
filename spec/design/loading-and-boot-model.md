...
@fact:origin-question PROP-007 shipped the workspace data model (M1.17) but deliberately left one question
...
@fact:scope-of-answer This session answered it. The answer turned out to be far larger than picking a
...
@fact:workspace-arc-continuity It belongs to the workspace arc and continues
[`workspace-and-qualified-naming.md`](workspace-and-qualified-naming.md). @status:doc/done

---

## 2. The problem the deferred question was hiding

@fact:narrow-question-illusion PROP-007 §6 q3 reads as a narrow placement question. It is not. @status:doc/done

@fact:flat-model-shape vibevm's boot model ([`VIBEVM-SPEC.md` §6](../../VIBEVM-SPEC.md)) is a **flat, single,
...
@fact:workspace-breaks-lead A workspace breaks every one of those assumptions at once: @status:doc/done

- @fact:break-n-nodes **N nodes** — the root plus every member. @status:doc/done
...
@fact:flat-model-verdict The flat model cannot be stretched over this. It must be replaced. @status:doc/done

@fact:OWNER-CONSTRAINT-NO-REWRITE The owner's hard constraint, stated at the outset: **installing a dependency must
...
naïve "bubble everything into the root `spec/boot/`" would amount to. @status:doc/done

---

## 3. The owner's mental model — boot as linking

@fact:linking-frame The owner framed the loading mechanism directly as **static vs dynamic linking**, and
that framing became the spine of the design. @status:doc/done

| C++ / the linker | vibevm |
|---|---|
| @fact:ROW-LINK-COMPILE Compile + link time @status:doc/done | `vibe install` @status:doc/done |
| @fact:ROW-LINK-LOAD Load + run time @status:doc/done | start of an AI session @status:doc/done |
| @fact:ROW-LINK-OBJECT Object file / library @status:doc/done | a package (flow / feat / stack / tool) @status:doc/done |
| @fact:ROW-LINK-EXECUTABLE The executable @status:doc/done | a node's effective boot sequence @status:doc/done |
| @fact:ROW-LINK-LOADER The dynamic loader (`ld.so`) @status:doc/done | the AI agent following INCLUDE pointers @status:doc/done |
| @fact:ROW-LINK-DT-NEEDED `DT_NEEDED` entry @status:doc/done | an INCLUDE pointer (a `spec://` reference) @status:doc/done |
| @fact:ROW-LINK-STATIC Static linking (`.a` copied into the binary) @status:doc/done | `vibe install` inlines a dependency's boot @status:doc/done |
| @fact:ROW-LINK-DYNAMIC Dynamic linking (`.so` referenced, resolved at load) @status:doc/done | the INCLUDE pointer stays; the agent resolves it @status:doc/done |
| @fact:ROW-LINK-VENDOR `vendor/` / `~/.cargo/registry` @status:doc/done | the materialised-dependency tree @status:doc/done |

@fact:AGENT-IS-THE-LOADER The load-bearing consequence: **at session start, the AI agent is the loader.** Every
...
@fact:P1-TWO-TREES **P1 — Two trees.** A node's authored `spec/` (only the node's author writes it) and
...
@fact:P2-COMPUTED-SEQUENCE **P2 — The boot sequence is computed, not assembled by hand.** Each node has an
...
@fact:P3-INDEX-PER-ENTRY-POINT **P3 — One generated index per entry point; the agent never walks the graph.**
...
@fact:P4-THREE-INCLUSION-TYPES **P4 — Three inclusion types: `inline`, `static`, `dynamic`.** Declared per dependency
...
@fact:forks-lead Four forks were put to the owner. The options, the resolution, the reasoning. @status:doc/done

### Fork 1 — the form of static boot delivery

@fact:fork1-options Options offered: a single inlined aggregate (cheapest at boot, but duplication and
the lost-in-the-middle risk §6.4 already rejected); a resolved path index (clean, but
1 + N reads); a hybrid. @status:doc/done

@fact:FORK1-REFINED-HYBRID **Resolution — a refined hybrid.** Not one of the three: a model where `inline`,
...
@fact:FORK2-COMMITTED-TREE **Resolution — a committed dependency tree.** A fresh clone is bootable with no
...
@fact:FORK3-UNIFORM **Resolution — uniform.** A single-package project is a degenerate (zero-member)
...
@fact:FORK4-UNIFIED-VIEWS **Resolution — boot + effective spec, unified.** The boot index and the `vibe build`
...
@fact:inclusion-types-lead Each dependency in a consumer's `vibe.toml` carries an inclusion type (working syntax
...
- @fact:GEN-STATIC-MD **`STATIC.md`** — the verbatim concatenation of every `static`-typed contribution in
...
@fact:cost-profile-lead Cost profile: @status:doc/done

| Type | Reads at boot | Content on disk | Use |
|---|---|---|---|
| @fact:ROW-COST-INLINE `inline` (ships as `link = "static"`) @status:doc/done | ~1 (already in `STATIC.md`) @status:doc/done | duplicated (bounded — few items) @status:doc/done | critical disciplines, top-level skills @status:doc/done |
| @fact:ROW-COST-STATIC `static` (ships as `link = "dynamic"`) @status:doc/done | 1 + N (N parallelisable) @status:doc/done | lives once @status:doc/done | the default — ordinary dependencies @status:doc/done |
| @fact:ROW-COST-DYNAMIC `dynamic` @status:doc/done | 1 + N + graph-walk @status:doc/done | lives once @status:doc/done | conditional / context-gated boot @status:doc/done |

@fact:dynamic-is-lazy-pull `dynamic` is, mechanically, the subskill `lazy-pull` delivery mode (PROP-003 §2.5) —
...
- @fact:FINDING-NUMBERING **Numbering.** The `NN-` prefix namespace (10–89, author-chosen) cannot survive a
...
- @fact:SUBSUMES-INSTALL-BUILD **Workspace-aware `vibe install` / `vibe build`** (PROP-007 §9.3, §6 q3) is no longer
...
- @fact:session-log-2026-05-21 **2026-05-21.** Session restored from `CONTINUE.md` + `spec/WAL.md`. The owner
...
- @fact:ptr-prop-009 [PROP-009](../modules/vibe-workspace/PROP-009-loading-model.md) — the contract; shipped M1.18. @status:doc/done
- @fact:ptr-prop-007 [PROP-007](../modules/vibe-workspace/PROP-007-workspace.md) — the workspace data model; §6 q3 is the question this answers. @status:doc/done
- @fact:ptr-workspace-naming [`workspace-and-qualified-naming.md`](workspace-and-qualified-naming.md) — the preceding design session. @status:doc/done
...
- @fact:ptr-prop-003 [PROP-003 §2.5](../modules/vibe-resolver/PROP-003-dep-evolution.md) — subskills and delivery modes. @status:doc/done
