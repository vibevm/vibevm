...
- **Package registry (future, M1+):** `git@gitverse.ru:anarchic/vibespecs.git` originally; the M1.1-revision per-package model targets the `vibespecs` organization at `git@gitverse.ru:vibespecs/<kind>-<name>.git` per package. The hand-written e2e fixtures live at `fixtures/registry/` (M0 monorepo shape, hermetic).
