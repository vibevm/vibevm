...
10. @fact:FORK-F10-IMPLICATION **F10 — `friends-only` implies `friend = true` on its own edge. PROPOSED** (##FRIENDS-ONLY-IMPLIES-FRIEND): under the new `false` default a friends-only chain otherwise needs two marks per hop, and forgetting the grant half silently kills the chain; vouching without relation is incoherent anyway. Explicit `friend = false` beside the mark wins and earns a dead-mark lint. Awaiting the owner's word. @status:spec/work
