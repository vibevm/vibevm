...
the future dependency-declared form would extend). @spec/done
...
- ##two-embed-ways A package may want to carry another repository inside it. Git offers two ways:
  a **submodule** (a reference — `.gitmodules` + a pinned gitlink commit) or a
  **vendored copy** ("git in git" — the foreign tree committed into the package). @spec/done
- ##vendored-works The vendored case already works: it is just files in the package tree. @spec/done
...
- ##mechanism-general But the mechanism
  is general: any package may legitimately embed a pinned dependency repo. @spec/done

### 1.2 What this is — submodule as a first-class embedded source {#what}

##WHAT-VVM-DOES vibevm fetches a package's submodules when it fetches the package, updates them
...
- ##FORM-DEPENDENCY-DECLARED **dependency-declared** (future) — a package's manifest names an embedded
...
- ##REJ-RESOLVE-AS-PACKAGE **Resolving a submodule as a vibevm package** through the depsolver —
  ...
  one-package rule. @spec/done
- ##REJ-SHALLOW-DEFAULT **Shallow submodule clones (`--depth 1`) by default** — rejected as the
  default: a shallow submodule can miss the exact gitlink commit and fail
  checkout; depth control is a possible opt-in, not the baseline. @spec/done
- ##REJ-PARSE-GITMODULES **vibevm parsing `.gitmodules` itself** — rejected: git already resolves
  submodules correctly across auth and nesting; reimplementing it would be a
  fragile re-do of solved work. @spec/done

## 4. Out of scope {#out-of-scope}

- ##OOS-DEPENDENCY-FORM **The dependency-declared submodule form** (§2.2) — specified as the
  ...
  path" direction. @spec/done
- ##OOS-NON-GIT-SOURCES **Submodules under non-git package sources** — a path- or future
  binary-source package has no git context to recurse; embedded sources there
  await the dependency-declared form. @spec/done
- ##OOS-RECURSIVE-RESOLUTION **Recursive vibevm resolution inside a submodule** — a submodule's own
  `vibe.toml`, if any, is not honoured; the submodule is opaque content. @spec/done
