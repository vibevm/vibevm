...
(boot assembly consuming the resolution). @spec/done

## 1. Motivation {#motivation}

- ##in-tree-packages vibevm ships its own packages in-tree, under `packages/`. @spec/done
- ##friction When `vibe` is built
  ...
  `[[registry]]` block. @spec/done
- ##unpublished-state With the packages **not published to any network registry**
  (the current state), that friction is the *only* way to use them at all. @spec/done
...
expansion under a different name. @spec/done

### 3.3 Project-local sources {#project-local}

- ##LOCAL-AUTO-OPEN REQ. A project carrying `<project_root>/packages/` (where `project_root` is
...
- ##D1-ORIGIN-SELECTED **D1 — precedence is origin-selected, not fixed.** *Rejected:* one global
  ...
  install `origin` already encodes which case applies. @spec/done
- ##D2-EMBEDDED-NAME **D2 — `embedded`, not `local`, in name and `source_kind`.** *Rejected:*
  ...
  **D2 revised (§3.3 amendment):** the reserved `--prefer-local` /
  ...
  broader one is still unscoped. @spec/done
- ##D3-EXPLICIT-ABOVE **D3 — explicit sources stay above embedded.** *Rejected:* embedded above
  overrides / path / git. Those are deliberate per-dependency choices; an ambient
  default must not override a decision the author wrote down. @spec/done
- ##D4-WARN-CI-OFF **D4 — warn + CI-off, not hard-error.** *Rejected:* hard-error in CI on an
  ...
  recur. @spec/done
- ##D5-AMBIENT-VISIBLE **D5 — ambient but visible.** *Rejected:* a purely silent default. `vibe
  doctor` and resolution output always surface the embedded registry, so the
  behaviour is discoverable, not magic. @spec/done
