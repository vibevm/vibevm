...
<status stage="spec" state="done" action="continue" comment="B0 2026-07-24: proposed 2026-06-24; umbrella of the four bridge-packages specs; fact grain 2026-07-24"/>

##status-line **Status:** proposed 2026-06-24 — owner-requested design session. The umbrella
...
mechanism stands alone; bridges are where they line up. @spec/done
...
- ##BRIDGE-DEF A **bridge package** closes the gap without the author's involvement: a
  *maintainer* volunteers to steward someone else's repository and publishes an
  ordinary vibevm package that **wraps** it. @spec/done
- ##BRIDGE-MEANS The bridge brings the foreign repo
  in by one of git's two means and, where needed, runs hooks to shape it into
  something vibevm can consume. @spec/done

### 1.2 What this is — a thin convention over three mechanisms {#what}

##thin-convention A bridge is not a new kind of package or a new subsystem. It is: @spec/done

- ##conv-ordinary-package an ordinary package (still one of `flow` / `feat` / `stack` / `tool`), @spec/done
- ##conv-embedded-content carrying the foreign repo as **vendored** content or a **submodule**
  ([PROP-021](PROP-021-submodule-sources.md)), @spec/done
- ##conv-hooks optionally **prepared** by install hooks ([PROP-020](../vibe-workspace/PROP-020-install-hooks.md)), @spec/done
...
- ##conv-skill-projection with any skill projected selectively from the bridged subtree
  ([PROP-015 #skill-include](../vibe-mcp/PROP-015-mcp-integration.md#skill-include)). @spec/done

##plus-flag …plus one flag that says "this is a bridge." @spec/done

## 2. Decisions {#decisions}

### 2.1 A bridge is marked by a flag, not a kind {#flag}

##req-flag `req r1` @spec/done

- ##BRIDGE-FLAG `[package].bridge = true` marks a package as a bridge. @spec/done
- ##FLAG-NOT-KIND It does **not** change
  the package's `kind` (a bridged skill is still a `feat`/`tool` as appropriate)
  or its identity. @spec/done
- ##FLAG-JOBS The flag is metadata with two jobs: it documents that the
  ...
  (§2.4). @spec/done
- ##FLAG-DEFAULT Default `false`; the overwhelming majority of packages are not bridges. @spec/done

### 2.2 Two classes — vendored and submodule-backed {#classes}

##req-classes `req r1` @spec/done

##TWO-CLASSES A bridge embeds the upstream repo one of two ways: @spec/done
...
- ##CLASS-VENDORED **Vendored ("git in git")** — the maintainer copied the upstream tree into
  ...
  cheapest case — the flag (§2.1) plus, if the layout needs shaping, hooks. @spec/done
- ##CLASS-SUBMODULE **Submodule-backed** — the package references the upstream via a git
  ...
  updated on update, so the bridge tracks upstream without re-vendoring. @spec/done
...
##req-composition `req r1` @spec/done

##COMPOSE-OPTIONAL A bridge is the point where the three orthogonal mechanisms compose, but it
mandates none of them: @spec/done

- ##OPT-NO-HOOKS a bridge **without hooks** is valid (the upstream layout already fits); @spec/done
- ##OPT-NO-SUBMODULE a bridge **without a submodule** is valid (vendored); @spec/done
...
- ##CANONICAL-FULL-CASE The canonical full case — submodule-backed + `pre-install` hook to shape the
  ...
  piece is independently usable outside any bridge. @spec/done
...
##req-maintainer `req r1` @spec/done

- ##MAINTAINER-VS-AUTHOR The bridge's **maintainer** is distinct from the upstream **author**. @spec/done
- ##GROUP-PROVENANCE The
  ...
  supports, [PROP-015 §0](../vibe-mcp/PROP-015-mcp-integration.md)). @spec/done
- ##PROVENANCE-VISIBLE A consumer
  can therefore always see *who stewards this* and *what it wraps*. @spec/done
- ##SKILL-VIA-INCLUDE A skill that
  ...
  ([PROP-015 #skill-include](../vibe-mcp/PROP-015-mcp-integration.md#skill-include))
  to pick the relevant files out of an upstream tree full of unrelated content. @spec/done
...
- ##ACC-FLAG-PARSES `[package].bridge` parses as a boolean, defaults `false`, and does not alter
  `kind` or identity. @spec/done
- ##ACC-VENDORED-PLAIN A vendored bridge installs as a plain `snapshot` package (flag + optional
  hooks), with no submodule/materialization machinery engaged. @spec/done
- ##ACC-SUBMODULE-FULL A submodule-backed bridge fetches upstream ([PROP-021](PROP-021-submodule-sources.md)),
  ...
  and projects its skill via the `include` selector. @spec/done
