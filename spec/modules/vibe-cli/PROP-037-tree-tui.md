...
a full application contract. @status:impl/done

@fact:related **Related:** [PROP-036](PROP-036-package-tree.md) (the `vibe tree` analyzer +
...
`legacy-spec/terraforms/TREE-TUI-PLAN-v0.1.md`. @status:spec/done

@fact:language-line **Language:** the shipped UI is **English** (§1.6); the **i18n mechanism ships**
(PROP-039 §8, §13.4) with English the only mandatory-complete locale — no
non-English catalogues are shipped now (§12). @status:spec/done

@fact:GRANULAR-ADDRESSABLE This contract is deliberately **granular and addressable** (owner directive):
every feature is its own `{#anchor}` REQ, cited by the code via `specmark`, so a
large feature set stays a map rather than a tangle. A REQ is the unit of work. @status:spec/done
...
- @fact:layers-intro The application separates, as fixed layers, (a) the vibevm data it renders, (b)
  ...
  (d) the event/control logic. @status:spec/done
- @fact:LAYERING-LAW The one-line law: **styling never leaks into
  control or app logic, and vibevm logic never leaks into the app.** @status:spec/done

### 1.1 The four layers {#layers}

@fact:FOUR-LAYERS REQ. The code is organized as four layers with one-directional dependencies
(each may depend only on those above it): @status:spec/done

1. @fact:LAYER-BACKEND **vibevm backend** — `vibe-core` / `vibe-workspace` / `vibe-spec` (the
   `PackageTree` builder, PROP-036). Unowned by this app. @status:spec/done
2. @fact:LAYER-MODEL **Model** — the app's data: the borrowed/owned `PackageTree` plus all UI
   state (§1.3). Pure data + logic-on-data; **no rendering, no event types**. @status:spec/done
3. @fact:LAYER-VIEW **View** — the component library (§2) and the theme (§2.2). Renders Model +
   component state into a ratatui buffer; **no app control flow**. @status:spec/done
4. @fact:LAYER-CONTROLLER **Controller** — event routing, the keymap registry (§5.1), the modal stack
   (§6), and the actions that mutate the Model (§1.5). @status:spec/done

@fact:MVC-CYCLE The MVC cycle: event → Controller (routes to the focused component / top modal,
produces an Action) → Action mutates Model → View renders Model + component
state through the Theme. @status:spec/done

### 1.2 The vibevm boundary {#backend-boundary}

@fact:BACKEND-BOUNDARY-REQ REQ. The app consumes vibevm through exactly one seam — the `PackageTree` model
...
dragging the UI with it. @status:spec/done

### 1.3 Model {#model}

@fact:MODEL-REQ REQ. The Model owns: the analyzed `PackageTree` (read-only vibevm data); and the
...
without a terminal. @status:spec/done

### 1.4 View — components + theme {#view}

@fact:VIEW-REQ REQ. All on-screen output is produced by the component library (§2). A component
...
only the theme, never component logic. @status:spec/done

### 1.5 Controller — events, keymap, modals, actions {#controller}

@fact:CONTROLLER-REQ REQ. The Controller receives each terminal event and routes it: to the **top
...
`Control::Changed`). @status:spec/done

### 1.6 English-only, i18n-ready {#i18n}

@fact:I18N-READY-REQ REQ. All user-facing strings are English and live behind a single string
...
is shipped now (§12); the indirection is the only present obligation. @status:spec/done

---

## 2. The component library {#components}

@fact:component-library-intro A reusable, vibevm-owned widget library — the abstraction that stops "a million
implementations of the same control" from accumulating. @status:spec/done

### 2.1 Component strategy — wrap, extend, or invent {#component-strategy}

@fact:COMPONENT-STRATEGY REQ. Components are built in this order of preference: **(1) wrap** a `rat-widget`
...
`ui::` facade; call sites never touch `rat_widget::` directly. @status:spec/done

### 2.2 The theme {#theme}

@fact:THEME-REQ REQ. A single `Theme` value carries every color, text style, border style,
...
(§1.4): a restyle touches only the theme. @status:spec/done

@fact:visual-language-note The visual language is **a first-class part of this contract**, not an
...
the normative REQs the code is traceable to are the four anchors below. @status:spec/done

#### 2.2.1 Palette tokens {#palette-tokens}

@fact:PALETTE-TOKENS-REQ REQ. Colour reaches a component only through a **`Palette`** — a data-driven
...
`button_off`. No component names a `Color` literal; it names a role. @status:spec/done

@fact:FIVE-PALETTES REQ. Five palettes ship, each a complete role→`Color` mapping. **Rosé Pine**
...
hex values for all five are normative in the design doc §3. @status:spec/done

@fact:PALETTE-IS-LIGHT REQ. A palette carries an `is_light` flag; `selection` is composed (`accent`
...
through the settings system (§9) and overridable at the CLI/env. @status:spec/done

#### 2.2.2 Glyph vocabulary {#glyph-vocabulary}

@fact:GLYPH-CONSTANTS REQ. Every glyph is a constant on the `Theme`, never a hardcoded string at a
...
replacement table is normative in the design doc §2. @status:spec/done

@fact:NO-ASCII-SEMANTIC REQ. In the primary UI (Tier ≥ 1) there are no `+`/`-`/`*`/`#`/`x`/`.` used as
**semantic** glyphs. Those ASCII characters appear only behind the Tier 0
fallback (§2.2.3). @status:spec/done

#### 2.2.3 Rendering tiers {#rendering-tiers}

@fact:FOUR-TIERS REQ. Rendering degrades through four tiers — **3** (truecolor: full RGB, rounded
...
ANSI mono, ASCII `+-|` frames, `#` indicators). @status:spec/done

@fact:TIER-DETECTION REQ. Tier detection is a **pure function** over the environment —
...
otherwise overridden via `vibe.tree.tier`. @status:spec/done

@fact:TIER-PROJECTION REQ. Degradation is a **projection**: one `Theme` is built for Tier 3 and
...
component. @status:spec/done

#### 2.2.4 Window aesthetics {#window-aesthetics}

@fact:WINDOW-AESTHETICS-REQ REQ. A window (§2.3) is not a fallback: it composes a solid `base` panel, a
...
title chip carry the "floating panel" reading without truecolor. @status:spec/done

#### 2.2.5 Spacing & rhythm {#spacing}

@fact:SPACING-REQ REQ. The theme's **spacing** (§2.2) is normative, not decorative: a component
...
place: @status:spec/done

- @fact:PAD-CONSTANTS **`PAD_X` = 2, `PAD_Y` = 1 — interior padding.** Inside every window/dialog
  ...
  a violation. @status:spec/done
- @fact:GUTTER-CONSTANT **`GUTTER` = 1 — group gutter.** A control inside a `Group` frame (§2.6) is
  inset from that frame's stroke by the gutter, and its selection highlight bar
  is inset with it, so neither the label nor the bar touches the border. @status:spec/done
- @fact:RHYTHM-RULE **Rhythm.** Sibling controls are separated — framed groups by a blank row
  between them, inline hint/footer runs by a `•` separator with surrounding
  space — so the eye reads structure, not a wall of glyphs. @status:spec/done

@fact:spacing-lore-note The values are lore-explained in the design doc §6 `#spacing-rhythm`; the footer
(§5.2) is the canonical inline application. @status:spec/done

### 2.3 Window / Panel {#window}

@fact:WINDOW-COMPONENT REQ. A `Window` component: a bordered, titled region drawn over a cleared rect,
...
and lays them out; it owns no app logic. @status:spec/done

### 2.4 Menu — the dropdown list {#menu}

@fact:MENU-COMPONENT REQ. A `Menu` component: a centered, window-framed list of options; `↑`/`↓` move
...
throughout this contract (F3 mode menu §7.1; sort options §7.2). @status:spec/done

### 2.5 Button {#button}

@fact:BUTTON-COMPONENT REQ. A `Button` component: a labelled, focusable control; the focused button is
highlighted; `Enter` activates it. Buttons participate in a window's focus order
(arrow / Tab navigation). Used by dialogs (OK, Save, Cancel). @status:spec/done

### 2.6 Group / Fieldset {#group}

@fact:GROUP-COMPONENT REQ. A `Group` component: a visually delimited cluster of child components
...
structure (§7.2). @status:spec/done

### 2.7 RadioGroup {#radio-group}

@fact:RADIO-GROUP-COMPONENT REQ. A `RadioGroup` component: a `Group` whose children are mutually-exclusive
...
§10.2). @status:spec/done

### 2.8 TextField {#text-field}

@fact:TEXT-FIELD-COMPONENT REQ. A `TextField` component: a single-line editable text input (the file-path
entry, §10.5). Simple for now (a later REQ enriches it); wraps rat-widget's text
input where possible (§2.1). @status:spec/done

### 2.9 Card / Form {#card}

@fact:CARD-COMPONENT REQ. A `Card` component: a `Window` laid out as a labelled vertical form — see
§8 for its full design contract (background/foreground, bold headers, line
spacing, wrapping, per-line copy). @status:spec/done

### 2.10 The standard "Coming Soon" modal {#coming-soon}

@fact:COMING-SOON-MODAL REQ. A single reusable `ComingSoon` modal — a `Window` titled with the feature
...
point to `ComingSoon` is how a feature is "reserved" before it is built. @status:spec/done

---

## 3. The Tree widget + filter pipeline {#tree-widget}

@fact:tree-abstraction-intro The core reusable abstraction: one tree widget, fed by a configurable pipeline. @status:spec/done

### 3.1 The Tree widget {#tree}

@fact:TREE-WIDGET-REQ REQ. A single `Tree` component renders a forest of `PackageTree` nodes — the
...
widget's (per-instance) state. @status:spec/done

### 3.2 The filter / shape pipeline {#tree-filters}

@fact:FILTER-PIPELINE REQ. Data reaches the `Tree` widget only after passing a configurable
...
are pipeline settings, selectable by the user and persisted (§9). @status:spec/done

### 3.3 The three tree shapes {#tree-shapes}

@fact:THREE-SHAPES REQ. The pipeline offers three shapes, selectable per context on the F2 sort menu
(§7.2), **default = (a)**: @status:spec/done

- @fact:SHAPE-MEMBERS-ROOTS **(a) members-as-roots + full subtrees** — each package in the filter set is a
  forest root shown with its entire dependency subtree (cross-type deps
  included); DAG dedup via `(*)`. @status:spec/done
- @fact:SHAPE-LOAD-FOREST **(b) load-type forest** — a package is a root only if no other member of the
  same filter set depends on it; children are its same-set dependencies
  (cross-set deps omitted). @status:spec/done
- @fact:SHAPE-PRUNED **(c) pruned tree** — the tree from the declared roots, keeping only branches
  that reach a member of the filter set. @status:spec/done

@fact:shapes-validate All three are pipeline configurations over §3.2 — implementing them validates
that the filter/shape abstraction is right. @status:spec/done

---

## 4. Modes {#modes}

@fact:THREE-MODES REQ. `vibe tree` has three display modes, each a Tree-widget (§3.1) configuration
(§3.2) — none is a flat list: @status:spec/done

### 4.1 Tree mode {#mode-tree}
@fact:MODE-TREE-REQ REQ. One tree over the whole package set (the current default). Filter = all;
shape + order per §3.2/§7.2. @status:spec/done

### 4.2 Sub-tables mode {#mode-subtables}
@fact:MODE-SUBTABLES-REQ REQ. **Several trees stacked vertically** — one Tree instance per effective-load
...
list. @status:spec/done

### 4.3 Tabs mode {#mode-tabs}
@fact:MODE-TABS-REQ REQ. **One tree per tab** — a tab bar (`Static` / `Dynamic` / `No-boot`); the
active tab shows that partition's tree (per §3.3). `Shift`+`←`/`→` switches tabs
(§5.3); plain arrows navigate the active tab's tree. @status:spec/done

### 4.4 Mode selection {#mode-select}
@fact:MODE-SELECT-REQ REQ. The active mode is chosen from the F3 menu (§7.1) — not a bare cycle key.
The active mode is shown in the status line and persisted (§9). @status:spec/done
...
@fact:KEYMAP-REGISTRY-REQ REQ. A single **keymap registry** maps each context (base mode, and each modal)
...
registry; no context re-implements dispatch. @status:spec/done

### 5.2 The F-key scheme + footer {#keys}

@fact:F-KEY-SCHEME REQ. Primary actions are on function keys: `F1` Search Everywhere (§7.3), `F2`
...
`Shift+F6` → `↑F6`) to keep hints short. @status:spec/done

@fact:FOOTER-TWO-ROWS REQ. The footer renders as **two centred rows** (§2.2.5 `#spacing`): the F-key
...
rhythm like the rest of the surface (§2.2.5). @status:spec/done

### 5.3 Tree interaction keys {#tree-keys}

@fact:TREE-KEYS-REQ REQ. Within a tree: `↑`/`↓` move selection (+ scroll), `←`/`→` pan horizontally,
...
from the F-key scheme (they are navigation, not commands). @status:spec/done

### 5.4 Focus groups + Tab Order {#focus-groups}

@fact:FOCUS-GROUPS-REQ REQ. A screen's controls are partitioned into **focus groups** — cohesive
...
(that is `Enter`). @status:spec/done

@fact:FOCUS-MARKED REQ. The focused group is visually marked (the theme's selection/accent), so the
...
dialog (§7.4) cycles OK → Cancel. @status:spec/done

---

## 6. The modal stack {#modal-stack}

@fact:MODAL-STACK-REQ REQ. Modals form a **stack**: opening a modal pushes it; it draws over
...
triggers quit-confirm (§7.4), it does not pop. @status:spec/done
...
@fact:F3-MODE-MENU-REQ REQ. `F3` opens a `Menu` (§2.4) of the display modes (§4). Selecting one switches
the mode. This replaces any bare mode-cycle key. @status:spec/done

### 7.2 F2 — the sort menu {#f2-sort-menu}
@fact:F2-SORT-MENU-REQ REQ. `F2` opens a dialog whose content depends on the active mode: @status:spec/done
- @fact:sort-tree-tabs **tree & tabs modes** — one `RadioGroup` (§2.7): `alphabetical` / `topological`
  (the sibling order, §3.2). One group; no group chrome needed. @status:spec/done
...
@fact:F1-SEARCH-REQ REQ. `F1` opens the **Search Everywhere** window — the `vibe-actions` Search
...
providers ship (PROP-039 §10.4): @status:spec/done
- @fact:PROVIDER-PACKAGES **Packages** — by name (`PackageProvider` over the `PackageTree`); selecting
  reveals the package in the tree. @status:spec/done
- @fact:PROVIDER-CARDS **Cards** — inside **every field** of the package detail cards (§8)
  (`PackageFieldProvider`: name, version, kind, license, load-type, origin, path,
  deps, diagnostics…); selecting opens the card focused on that field. @status:spec/done
- @fact:PROVIDER-ACTIONS **Actions** — all `vibe.tree` actions (§13.5) by address, **name, and
  ...
  with their "why disabled" reason and their keybinding. @status:spec/done

@fact:SEARCH-ENGINE-NOTES Matching falls back to the **name/description** lane when nothing matches by id or
...
the same engine with no TUI change. @status:spec/done

### 7.4 Escape — quit with confirmation {#quit-confirm}
@fact:QUIT-CONFIRM-REQ REQ. At the base screen, `Esc` opens a confirm dialog ("Really quit?") with
...
modals — an accidental single `Esc` must not discard the session. @status:spec/done

---

## 8. The detail card {#detail-card}

@fact:DETAIL-CARD-REQ REQ. `Enter` on a package opens the detail **Card** (§2.9) — a real form, not a
glued text blob: @status:spec/done
- @fact:card-paper a light panel background with dark text (a "paper" card), distinct from the
  tree beneath; @status:spec/done
- @fact:card-headers **bold** field headers; blank-line / padded spacing between fields; @status:spec/done
- @fact:card-close an `esc [x]` close affordance drawn as a pseudographic cross in the top-right
  corner; @status:spec/done
...
@fact:SETTINGS-PERSISTENCE REQ. UI state persists under `~/.vibe/tree/` as one or more JSON files. Saved:
...
`~/.vibe/` config root, distinct from the project's `.vibe/`.) @status:spec/done
...
@fact:COPY-PROVIDERS-REQ REQ. Each screen (tree/sub-tables/tabs; the card) supplies a **copy provider** —
...
in tabs mode only the active tab is serialized. @status:spec/done

### 10.2 F6 copy / Shift+F6 copy-settings {#copy-flow}
@fact:COPY-FLOW-REQ REQ. `F6` copies the current screen straight to the clipboard as text (the
...
= file pushes the file-path modal (§10.5) over it (§6 stack). @status:spec/done

### 10.3 Markdown export {#copy-markdown}
@fact:MARKDOWN-EXPORT-REQ REQ. Markdown is a real serialization, not a screen-buffer scrape: the tree
renders as a pseudographic tree inside a fenced block (or a nested list); the
card renders as well-formatted Markdown (headers + fields), not a raw text dump. @status:spec/done

### 10.4 PNG export {#copy-png}
@fact:PNG-RESERVED REQ. PNG export (a rasterized tree image; a designed package info-card image) is
**reserved** — selecting PNG opens the `ComingSoon` modal (§2.10) until the
rasterization (font + image crates) is built. Named non-goal-for-now (§12). @status:spec/done

### 10.5 Destination — clipboard vs file {#copy-dest}
@fact:COPY-DEST-REQ REQ. Clipboard writes via the platform clipboard. File destination pushes a
...
copy-settings. The path field is plain text for now (a later REQ enriches it). @status:spec/done

---

## 11. AI-Native Rust discipline {#discipline}

@fact:DISCIPLINE-REQ REQ. All code implementing this contract follows the AI-Native Rust discipline
...
feature set navigable. @status:spec/done

---

## 12. Non-goals {#non-goals}

- @fact:NG-LOCALIZATION **Non-English localization content** — the i18n *mechanism* ships (PROP-039 §8,
  §13.4); no non-English catalogues are shipped now, and English is the only
  mandatory-complete locale. @status:spec/done
- @fact:NG-PNG **PNG export** — reserved behind `ComingSoon` (§10.4) until the rasterization
  spike lands. @status:spec/done
- @fact:NG-AIUI **The AIUI surface itself** — not built now; this TUI is its prototype (§13.1,
  PROP-039 §11.3). The Search-Everywhere `StructureProvider` (AI-Native specmap
  nodes) is likewise reserved (§7.3) — the same engine, added later. @status:spec/done
- @fact:NG-PLANTUML **PlantUML / Mermaid copy formats** — later additions to §10.2. @status:spec/done
- @fact:NG-SETTINGS-UI **A settings UI** — settings are edited via the menus (§7) and persisted (§9);
  no dedicated settings editor screen. @status:spec/done
- @fact:NG-NON-TTY **Non-tty operation** — `--json` / `--plain` (PROP-036) remain the machine and
  fallback surfaces; this contract governs the interactive TUI only. @status:spec/done

---

## 13. Built on the action system (`vibe-actions`, PROP-039) {#action-system}

@fact:action-system-intro This TUI is the **first consumer** and the **prototype surface** of the action
system (PROP-039). This section adapts the architecture above onto it and is
**authoritative** where it upgrades an earlier section. @status:spec/done

### 13.1 The TUI is a Surface; the Model is the serialisable view {#as-surface}
@fact:AS-SURFACE-REQ REQ. The TUI is a `Surface` (PROP-039 §11.1) over `vibe-actions`; it owns rendering
...
driven headless with no change to this crate's model/controller. @status:spec/done

### 13.2 Commands are addressed actions {#as-actions}
@fact:AS-ACTIONS-REQ REQ. Every TUI command is a `vibe-actions` **Action** (PROP-039 §3) in the group
...
enum. @status:spec/done

### 13.3 The keymap binds keys to addresses {#as-keymap}
@fact:AS-KEYMAP-REQ REQ. The mode-aware keymap (§5.1) binds each key/chord, per context, to an
...
remain direct (navigation, not commands). @status:spec/done

### 13.4 i18n is real {#as-i18n}
@fact:AS-I18N-REQ REQ. This **upgrades §1.6** from "i18n-ready" to the shipped `vibe-actions` i18n
...
is checked by the legibility gate (PROP-039 §8.4). @status:spec/done

### 13.5 The action catalogue {#action-catalogue}
@fact:ACTION-CATALOGUE-REQ REQ. The `vibe.tree` actions at ship — each an addressed Action with a name +
description; the **key is its default binding** (the map, not the identity): @status:spec/done

| Address (`action://vibe.tree/…`) | Key | Name | Description |
|---|---|---|---|
| @fact:ROW-ACT-SEARCH `search.everywhere` @status:spec/done | `F1` @status:spec/done | Search Everywhere @status:spec/done | Search packages, cards, and actions; run a found action. @status:spec/done |
| @fact:ROW-ACT-SORT `sort` @status:spec/done | `F2` @status:spec/done | Sort & shape… @status:spec/done | Choose the ordering and tree shape for the current view. @status:spec/done |
| @fact:ROW-ACT-MODE `mode.set` @status:spec/done | `F3` @status:spec/done | Switch mode… @status:spec/done | Switch between tree, sub-tables, and tabs display. @status:spec/done |
| @fact:ROW-ACT-COPY `copy` @status:spec/done | `F6` @status:spec/done | Copy @status:spec/done | Copy the current screen (Markdown) to the clipboard or a file. @status:spec/done |
| @fact:ROW-ACT-COPY-SETTINGS `copy.settings` @status:spec/done | `↑F6` @status:spec/done | Copy settings… @status:spec/done | Choose the copy format and destination. @status:spec/done |
| @fact:ROW-ACT-FOLD `fold.toggle` @status:spec/done | `Space` @status:spec/done | Fold / unfold @status:spec/done | Fold or unfold the selected node. @status:spec/done |
| @fact:ROW-ACT-CARD `card.open` @status:spec/done | `Enter` @status:spec/done | Open details @status:spec/done | Open the detail card for the selected package. @status:spec/done |
| @fact:ROW-ACT-TABS `tab.next` / `tab.prev` @status:spec/done | `↑→` / `↑←` @status:spec/done | Next / previous tab @status:spec/done | Move between tabs in tabs mode. @status:spec/done |
| @fact:ROW-ACT-QUIT `quit` @status:spec/done | `Esc` (base) @status:spec/done | Quit @status:spec/done | Leave `vibe tree` (with confirmation). @status:spec/done |

@fact:CATALOGUE-ENUMERABLE REQ. This catalogue is the **enumerable** source for both the footer and the Search
...
keymap, and Search Everywhere with no further wiring (PROP-039 §4.3, §12.2). @status:spec/done

### 13.6 What this leaves to PROP-039 {#as-boundary}
@fact:AS-BOUNDARY-REQ REQ. The action core — address, registry, params, context, invoke, i18n, the keymap
...
rendering never leaks into the action core.** @status:spec/done
