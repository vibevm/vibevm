...
- ##ARTIFACT-STATIC-MD **`STATIC.md`** — the verbatim concatenation, in priority order, of every `static`-typed (§2.4) contribution in the node's effective boot. Read first. Generated only when the node has `static` contributions. @impl/done
