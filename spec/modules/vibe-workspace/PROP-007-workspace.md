...
##milestone-line **Milestone:** design proposal; targets a new `M1.17` ([`ROADMAP.md`](../../../ROADMAP.md)). Not implementation-locked. @spec/work

##status-line **Status:** M1.17 Phases 1–5 implemented 2026-05-21 — the workspace data model, the `vibe-workspace` discovery engine, path-source dependencies, `[workspace.versions]` placeholders, and `vibe workspace publish` are shipped (clippy-clean, fully tested). Workspace-aware `vibe install` is the remaining piece — see §6 and §8. @impl/work
