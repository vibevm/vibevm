...
by its Phase 4.** Module: `vibe-tcg` (new) + `vibe-mcp` (adapter) +
`vibe-workspace` (binary resolution).
...
`stack:org.vibevm/typescript-ai-native` (the oracle itself ships THERE;
vibevm ships only this thin product seam).
...
- `language` (required; v0.1 accepts `"typescript"` only — any other
  value is a ToolError NAMING the supported set, so the Rust twin later
...
1. resolve the CURRENT project's lockfile → the stack slot that
   ...
   TypeScript) — the PROP-025 `[[binary]]` walk, through the SHARED
   `vibe-workspace` binary-resolution cell (extracted from vibe-cli by
   this campaign so CLI and registry cannot drift);
...
- The same call on a project WITHOUT the stack returns the
  not-installed recipe; `language: "rust"` returns the supported-set
  error.
