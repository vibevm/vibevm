...
@fact:milestone-line **Milestone:** M1.26 candidate («MCP sovereignty» —
[MCP-SOVEREIGNTY-PLAN-v0.1](../../../legacy-spec/terraforms/MCP-SOVEREIGNTY-PLAN-v0.1.md)). @status:impl/done

@fact:status-line **Status:** IMPLEMENTED — the kind and the manifest laws (§2.1–§2.3)
...
@fact:related **Related:** [PROP-015](PROP-015-mcp-integration.md) (the product MCP
...
- @fact:motivation-gap The discipline stacks ship engines, gates, CLIs, and type oracles —
...
@fact:MCP-KIND-DEF An **`mcp` package** is one whose primary deliverable is one or more
...
- @fact:SERVER-IS-BINARY The server IS a [PROP-025](../vibe-workspace/PROP-025-binary-delivery.md)
...
@fact:req-exact-pin `req r1` @status:impl/done

- @fact:VENDORING-WHY Cargo path-deps cannot cross package slots (PROP-024 §2.4), so an mcp
...
@fact:req-registration `req r1` @status:impl/done

@fact:REG-TODAY `vibe mcp install` today writes vibevm's own server into agent configs
(PROP-015). @status:impl/done

@fact:REG-PACKAGE-DISCOVERY It grows package discovery: every installed package of kind
`mcp` contributes its `[[mcp_server]]` entries, written into the target
agents' configs with @status:impl/done

- @fact:REG-COMMAND-PATH `command` = the absolute, **verbatim-free** path to the slot-resident
...
@fact:req-consent `req r1` @status:impl/done

- @fact:CONSENT-TRUST-ACT Registering a server schedules package code execution at agent-session
...
@fact:req-vibe-free `req r1` @status:impl/done

- @fact:VIBE-FREE-SERVING An mcp package's servers run without vibe: the artifact is launched by
...
@fact:COMPOSITION-LAW Every package-role feature applies to `mcp` packages exactly as to the
...
|---|---|---|
| @fact:ROW-CODE-BEARING Code-bearing layout @status:impl/done | PROP-024 @status:impl/done | the package IS code-bearing by definition; `spec/` for prompt content, crates at root @status:impl/done |
| @fact:ROW-BINARIES Binaries @status:impl/done | PROP-025 @status:impl/done | `[[mcp_server]].binary` references them; `vibe bin list/build/exec` see them like any other @status:impl/done |
| @fact:ROW-SKILLS Skills @status:impl/done | PROP-015 §2.8, PROP-018 §2.4 @status:impl/done | `[[skill]]` legal; a server may ship its teaching skill @status:impl/done |
| @fact:ROW-BOOT-SNIPPET Boot snippet @status:impl/done | PROP-009 @status:impl/done | legal but not required; agents learn servers via registration, not boot text @status:impl/done |
| @fact:ROW-HOOKS Hooks @status:impl/done | PROP-020 @status:impl/done | pre/post-install hooks run in the slot as usual @status:impl/done |
| @fact:ROW-MATERIALIZATION Materialization modes @status:impl/done | PROP-022 @status:impl/done | snapshot/in-place per the standard rules @status:impl/done |
| @fact:ROW-BRIDGES Bridges / submodules @status:impl/done | PROP-023 / PROP-021 @status:impl/done | no special-casing @status:impl/done |
| @fact:ROW-PUBLISH Publish @status:impl/done | PROP-002 §2.10 @status:impl/done | standard registry publish; the exact-pin law travels in the manifest @status:impl/done |
| @fact:ROW-MUTABILITY In-workspace mutability @status:impl/done | PROP-011 §2.6 @status:impl/done | dev-loop re-materialisation applies @status:impl/done |

## 3. Rejected alternatives {#rejected}

- @fact:REJ-ANY-KIND **`[[mcp_server]]` as an any-kind surface** (the plan's original
...
1. @fact:OPEN-MULTI-SERVER Multi-server packages (legal today) — does `vibe mcp install` offer
