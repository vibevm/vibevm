...
##placement-lead Куда можно ставить маркер — шесть гранулярностей (PROP-043 §3.8): @doc/done

- ##PLACE-DOCUMENT в преамбуле, до первого заголовка (весь документ). В файле без
  преамбулы — а это стандартная форма в этом репозитории — маркер сразу после первого
  заголовка и есть документный; @doc/done
- ##PLACE-SECTION отдельной строкой сразу после заголовка (секция) — кроме первого
  заголовка файла без преамбулы: там эта позиция занята документным маркером; @doc/done
- ##PLACE-PARAGRAPH первым или последним токеном внутри абзаца (абзац); @doc/done
- ##PLACE-LIST-ITEM последним токеном внутри элемента списка (элемент списка); @doc/done
- ##PLACE-TABLE-CELL внутри ячейки таблицы (ячейка); @doc/done
- ##PLACE-FRAGMENT парным тегом вокруг текста (фрагмент). @doc/done
- ##PLACE-FACT-ANCHOR Любая из этих единиц может нести якорь факта `##<ID>` в начале —
  тогда она адресуема по `spec://…#<ID>`. Закон anchored-when-marked (PROP-043 §3.9):
  размеченный факт обязан быть заякорен; @doc/done
...
- ##FOUR-SURVIVORS Между кампаниями из зоны кампании хранятся только четыре вещи
  ...
  переживают кампанию, но они живут в спеках — это корпус, а не зона. @doc/done
