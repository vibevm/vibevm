...
`flow:org.vibevm/core-ai-native` under `spec/mechanisms/`
...
sources at `packages/org.vibevm/core-ai-native/<version>/spec/mechanisms/` —
vibevm is the packages' dev repo). vibevm's `specmap.toml` resolves the
`spec://core-ai-native/…` URIs through its `[[external_specs]]` entry, so
...
`spec://core-ai-native/mechanisms/<DOC>#<anchor>` (anchors unchanged).
...
playbooks) is the installed package `flow:org.vibevm/core-ai-native`, and
