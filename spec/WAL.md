...
1. @fact:WAL-NEXT-F14 **Ф1.4 — слоты (решения D7 + D14):** в типы записи
   ...
   `frozen: bool`, `tombstone` на уровне `NameEntry`; в манифест — `frozen:
   Option<bool>` рядом с `epoch` из Ф1.2 (`crates/vibe-core/src/manifest/package.rs`);
   читатели учат ОДНО поведение: запись с незнакомым `must_understand` —
   пропуск с warn (полноценный карантин — Ф6.2). @status:spec/plan
2. @fact:WAL-NEXT-F14-MEASURE-FIRST **Перед нарезкой Ф1.4 — замер, как перед
   Ф1.3.** Три пакета этой сессии были замерами, и два опровергли по опорной
   координате; третий (`F1-3-PROBE`) опроверг два утверждения самого плана.
   Дешевле спросить дерево, чем строить по записанному. @status:spec/plan
3. @fact:WAL-NEXT-OWNER **За владельцем:** S2 (переименовать `_`-репо в
   ...
   `content_hash` в локфайле, и он ОБОЙДЁН по конструкции: реестровая копия
...
- @fact:WAL-KI-HASH-RECIPE-TRAPS **Ф1.3 СЕЛА; что она исправила в записанном —
  держать под рукой, это переоткрывается только новым спайком.** Замер
  `F1-3-PROBE` и посадка (2026-08-14) опровергли ТРИ утверждения, стоявшие в
  плане и в этом файле; находка —
  [`harvest/f1-3-hash-blast-radius.md`](../campaigns/packages-2026-09/harvest/f1-3-hash-blast-radius.md).
  (1) **«Реализаций алгоритма ровно две» — неверно.** Tree-walk-копий две
  (`vibe-index/src/content_hash.rs`, `vibe-registry/src/shippable.rs` — не
  `content_hash.rs`, как говорил план), но продюсеров формы `sha256:` ТРИ:
  третий, `commit_content_hash` (`git_package_registry/fetch.rs:484`), выводит
  хэш из коммита, а не из дерева, и именно он пишет `content_hash` в локфайл для
  **in-place** пакетов. Рецепт, описанный как «исключения + нормализация +
  порядок обхода», его не описывает вовсе — **именованный дефер**.
  (2) **Посылка Б.3 о расхождении Windows/Unix — опровергнута.** `Ord` у `Path`
  **покомпонентный**, поэтому рецепт 0 разделителя не видит и платформенно
  стабилен. Различает рецепты не разделитель, а порядок «по компонентам» против
  «по нормализованным байтам», и расходятся они лишь когда сосед продолжает имя
  каталога байтом **ниже** `/` (`spec-x.md`, `-`=0x2D); на байте, названном
  планом (`specX.md`, `X`=0x58), рецепты СОВПАДАЮТ. Фикстура
  `golden-order-trap-0.1.0` несёт обе формы: `spec-x.md` — ловушка, `specX.md` —
  контроль.
  (3) **Паритет-тест не сторожил того, чем назван** — звал одну реализацию и
  сверял с константой. Теперь сверяет обе реализации при обоих рецептах.
  **И один дефект, который поймал не гейт, а потребитель:** реализация сменила
  порядок рецепта 0 (сортировка строки вместо `PathBuf`), то есть тихо сдвинула
  хэш всякого дерева с каталогом-префиксом соседа. Ни один golden поймать не мог;
  поймал `vibe check` — 0 → 7 предупреждений `local_source_freshness`, шесть по
  нетронутым пакетам, и обратно 1 после починки. **Счётчики предупреждений
  панели — доказательство, а не шум.** @status:impl/done
