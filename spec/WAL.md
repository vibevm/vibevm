...
**Remaining on the inline compiler:** source-merge into the pipeline (§8 phase 3, deferred with a note); contract-cycle admission (§9); link tables (§10, the vtable analogue — install-time edge index giving the structural side "global knowledge" cheaply); full file/package markers (§11); transitive-inline (§12); the first-loaded structural loader prompt (§13); and the payoff — **wiring the compiler into `bootgen`** to replace the naive `INLINE.md`. Migration is demo-corpus-first, then all of `org.vibevm.world`, vibevm's core specs last (§15). No blocker; each slice is a safe stop.
