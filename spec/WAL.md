...
@fact:WAL-PHASE **Change-native стройка, ТЗ №1: Ф0 ЗАКРЫТА, Ф1.1 и Ф1.2 СЕЛИ.
Следующий шаг — Ф1.3 (рецепт хэша), и его пакет ПЕРЕПИСЫВАЕТСЯ под замер
`F1-3-PROBE`: план в этой точке опирается на два утверждения, которые замер
опроверг (`##WAL-KI-HASH-RECIPE-TRAPS` пп. 3–4).** Порядок внутри Ф1 жёсткий:
Ф1.3 → Ф1.4 → Ф1.5, каждый шаг отдельным коммитом. @status:impl/done
...
- @fact:WAL-KI-HASH-RECIPE-TRAPS **Ф1.3 — четыре ловушки; две найдены боссом
  заранее, две домерены замером `F1-3-PROBE` (2026-08-14, находка
  ...
  ДЕСЕРИАЛИЗАЦИИ (`try_from = "String"`, `:41`); поле `LockedPackage.content_hash`
  имеет ИМЕННО этот тип (`lockfile.rs:294`), поэтому локфайл с `sha256-tree/1:`
  сегодня просто не распарсится. `parse` обязан выучить обе формы.
  ...
  нет — её надо добавить, иначе рецепт 1 замораживается, ничего не доказывая.
  (3) **«Реализаций алгоритма ровно две» — НЕВЕРНО, и это была ошибка ЭТОГО
  файла.** Tree-walk-копий действительно две (`vibe-index/src/content_hash.rs:40`
  и `vibe-registry/src/shippable.rs:77` — не `content_hash.rs`, как говорит план),
  но продюсеров формы `sha256:` — ТРИ: третий, `commit_content_hash`
  (`vibe-registry/src/git_package_registry/fetch.rs:484`), выводит хэш из коммита,
  а не из дерева, и именно он пишет `content_hash` в локфайл для **in-place**
  пакетов. Рецепт, описанный как «исключения + нормализация + порядок обхода»,
  его не описывает вовсе.
  (4) **Паритет-тест сторожит не то, чем назван.** `tests/content_hash_parity.rs`
  зовёт ТОЛЬКО `vibe_index::compute_content_hash` и сверяет с замороженной
  константой; `vibe-registry` он не вызывает и две реализации между собой не
  сравнивает — то есть расхождение, о котором предупреждают комментарии «MUST
  stay in lockstep», этим сторожем не ловится.
  **Следствие для нарезки Ф1.3:** буквальное «сменить эмиссию только у индекса»
  безопасно (клиент индекса `index_client/wire.rs` поле `content_hash` не читает
  вовсе), но неустойчиво — контракт lockstep и периметр самой Ф1.3 тянут за собой
  реестровую копию, и как только она меняется, две формы встречаются ровно в двух
  продакшн-сравнениях: `vibe-check/src/checks/local_source_freshness.rs:92`
  (`fresh != pkg.content_hash.as_str()`) и `fetch.rs:385`
  (`Some(expected) if expected == content_hash`). @status:impl/done
