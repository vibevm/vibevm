...
| `redirect_block` | error | Each agent instruction file at the project root (`CLAUDE.md` / `AGENTS.md` / `GEMINI.md`) carries a well-formed managed block — zero `<vibevm>` markers, or exactly one ordered `<vibevm>` … `</vibevm>` pair ([PROP-012](../../spec/modules/vibe-workspace/PROP-012-managed-redirect-block.md)). Anything else is an error: a mutating `vibe` command would refuse to proceed against it. |
| `lockfile_files` | error / warning | Every package in `vibe.lock` has a materialised `vibedeps/<kind>-<name>/<version>/` slot on disk (error if missing — run `vibe reinstall --force`). No `vibedeps/` slot is absent from the lockfile (warning — an orphan a version bump or a dropped dependency should have pruned). |
