...
| `schema_version` | uint | yes | Lockfile-format major version. Must be `4` — `Lockfile::read` rejects any other value. Earlier schema versions (1, 2, 3) are not read; the fix for an old lockfile is always to regenerate it with `vibe install`. vibevm is pre-release, so there is no on-disk migration path. |
...
`Lockfile::read` accepts only `schema_version = 4`. An older lockfile (schema 1, 2, or 3) is **rejected** — there is no read-path migration. The fix is always to regenerate: delete `vibe.lock` (or just run `vibe install`) and let the resolver write a fresh v4 file. vibevm is pre-release, so no on-disk migration path is provided and none is needed.
