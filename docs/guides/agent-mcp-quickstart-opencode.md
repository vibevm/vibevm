...
**`vibe install flow:wal` fails inside the agent's run.** Read-only clone needs no token. Probe with `git ls-remote https://github.com/vibespecs/org.vibevm_wal` (the canonical repo `flow:wal` resolves to under `naming = "fqdn"`). If that fails, the issue is in your network / DNS, not in vibevm.
