...
| Flow | One line |
| --- | --- |
| @fact:ROW-TWO-PROCESS-MODEL `two-process-model` @1.0.0 @status:impl/done |  Human and AI as coprocessors; the human owns coherence; files are the only shared memory. @status:impl/done |
| @fact:ROW-WAL `wal` @1.0.0 @status:impl/done |  The checkpoint file (WAL) and cold-resume snapshot; session wind-down and resume rituals; the `wal-status` skill. @status:impl/done |
| @fact:ROW-SYNC-FROM-CODE `sync-from-code` @1.0.0 @status:impl/done |  The sanctioned reverse path: reconcile the spec when code changed first, with human approval. @status:impl/done |
| @fact:ROW-ATOMIC-COMMITS `git-atomic-commits` @1.0.0 @status:impl/done |  One commit, one idea; Conventional Commits; pushed history is frozen. @status:impl/done |
| @fact:ROW-ADDRESSABLE-SPECS `addressable-specs` @1.0.0 @status:impl/done |  `spec://` URIs, stable anchors, size budgets, and the spec tree layout. @status:impl/done |
| @fact:ROW-DECISION-RECORDS `decision-records` @1.0.0 @status:impl/done |  Decisions, not facts: reason + rejected alternatives + revisit trigger, at the governing anchor. @status:impl/done |
| @fact:ROW-CONFLICT-PROTOCOL `conflict-protocol` @1.0.0 @status:impl/done |  Human > Spec > Tests > Code; REVIEW markers; the conservative-default path when the spec is silent. @status:impl/done |
| @fact:ROW-CAMPAIGN-PLANS `campaign-plans` @1.0.0 @status:impl/done |  Cold-executable campaign plans: phase gates, falsifiable predictions, execution and deferral ledgers. @status:impl/done |
| @fact:ROW-DISCOVERY-PROMPT `discovery-prompt` @1.0.0 @status:impl/done |  The DISCOVERY collaborative-research prompt, packaged verbatim with a usage guide. @status:impl/done |
| @fact:ROW-ATTRIBUTION-POLICY `git-attribution-policy` @1.0.0 @status:impl/done |  A deliberate authorship posture: human-authored surface by default, disclosure documented as the alternative. @status:impl/done |

@fact:project-practice-table-lead The project-practice wave: @status:impl/done

| Flow | One line |
| --- | --- |
| @fact:ROW-OPERATING-MODES `operating-modes` @1.0.0 @status:impl/done |  Codeword-triggered work postures; red lines that survive every mode. @status:impl/done |
| @fact:ROW-HEALTH-AUDIT `health-audit` @1.0.0 @status:impl/done |  The periodic judgment sweep over what the per-commit gate is blind to; a skill and an append-only trend. @status:impl/done |
| @fact:ROW-MANUAL-TESTS `manual-tests` @1.0.0 @status:impl/done |  Human-runnable walkthroughs for the integration surfaces automation cannot prove. @status:impl/done |
| @fact:ROW-SECRETS-HYGIENE `secrets-hygiene` @1.0.0 @status:impl/done |  Surface-secrets never printed or persisted; scope discipline; third-party-code consent. @status:impl/done |
| @fact:ROW-LICENSING `licensing` @1.0.0 @status:impl/done |  A deliberate licence posture; permissive-only dependencies; the EULA-to-open path; a drafting skill. @status:impl/done |
| @fact:ROW-SOURCE-MIRRORS `source-mirrors` @1.0.0 @status:impl/done |  Single-writer multi-homing; manifest-driven fail-loud fast-forward-only fan-out. @status:impl/done |
| @fact:ROW-SPEC-GENRES `spec-genres` @1.0.0 @status:impl/done |  Contract vs lore vs research vs plans — what goes where, who wins, two-way links. @status:impl/done |
| @fact:ROW-COMPARATIVE-RESEARCH `comparative-research` @1.0.0 @status:impl/done |  Evergreen competitor studies with two-way gap analysis and numbered roadmap deltas. @status:impl/done |
| @fact:ROW-MANAGED-BLOCKS `managed-blocks` @1.0.0 @status:impl/done |  How a tool writes into files it does not own — one delimited block, deterministic scanning (for tool authors). @status:impl/done |
| @fact:ROW-QUALIFIED-NAMING `qualified-naming` @1.0.0 @status:impl/done |  Namespaces for package ecosystems: groups, identity tuples, collision vs conflict (for ecosystem designers). @status:impl/done |
| @fact:ROW-TOOL-DESIGN-LESSONS `tool-design-lessons` @1.0.0 @status:impl/done |  Paid-for lessons for self-updating tools and package systems. @status:impl/done |
