...
@fact:scope-of-this-document **Scope of this document.** A copy-ready skeleton for a comparative
research document, then a clause-by-clause commentary on each
section, then one short worked fragment. @status:impl/done

@fact:sibling-document-pointers The laws these clauses
enforce live in
[`COMPARATIVE-RESEARCH-PROTOCOL.md`](COMPARATIVE-RESEARCH-PROTOCOL.md);
this file is the shape you fill. @status:impl/done

## The skeleton {#skeleton}

@fact:COPY-THE-BLOCK-AND-FILL-EACH-PLACEHOLDER Copy this whole block into a new research document and fill each
placeholder. @status:impl/done

@fact:KEEP-THE-SECTION-ORDER Keep the section order — it is the reading shape a
cold reader navigates by. @status:impl/done
...
- @fact:CLAUSE-STATUS-LINE **Status line.** State up front that the document ratifies
  nothing. This is the deltas-not-decrees law made visible on line
  one, so no reader mistakes a proposal for a decision. @status:impl/done
- @fact:CLAUSE-PURPOSE **Purpose.** One paragraph, and it must contain the two-way promise
...
- @fact:CLAUSE-SOURCE-CORPUS-AND-TABLE **Source corpus + table.** Every source with an access date and the
  subject's version. This table is half of the re-fetch list; filling
  it now is cheaper than reconstructing it later. @status:impl/done
- @fact:CLAUSE-READING-SHAPE **Reading shape.** A one-line section map so a cold reader navigates
  without reading linearly. Months later, this is how a refresher
  finds the delta table in ten seconds. @status:impl/done
- @fact:CLAUSE-SECTION-ONE-IN-THEIR-OWN-WORDS **§1 in their own words.** The subject's pitch, quoted, before any
  analysis. If your document opens with your *summary* of their pitch,
  you have already broken the quote-first law. @status:impl/done
- @fact:CLAUSE-SECTION-TWO-INVENTORY **§2 inventory.** Quote-then-map, one capability at a time. "Maps
  to: no equivalent" is a finding, not a blank — it is a §3 gap in
  the making. @status:impl/done
- @fact:CLAUSE-SECTIONS-THREE-AND-FOUR **§3 trail / §4 lead.** Both are mandatory. A document that ships
  with §4 empty has not looked hard enough — every project made
  *some* decision its competitor did not. @status:impl/done
- @fact:CLAUSE-SECTION-FIVE-DELTAS **§5 deltas.** Numbered, prioritized, homed. The number is what the
...
- @fact:CLAUSE-SECTION-SIX-OPEN-QUESTIONS **§6 open questions.** What the sources did not answer. Honest gaps
  in the study itself, and the first questions to ask on refresh. @status:impl/done
- @fact:CLAUSE-SECTION-SEVEN-RE-FETCH-LIST **§7 re-fetch list.** The refresh recipe: which URLs in what order,
  and the capture version — without it, refresh becomes rewrite. @status:impl/done

## Worked fragment {#worked}

@fact:a-short-invented-illustration A short invented illustration — a study of a generic build tool
*Quarry*: two quotes, one delta. @status:impl/done

@fact:real-documents-run-longer Real documents run longer. @status:spec/done
...
@fact:the-fragment-obeys-all-five-laws-lead The fragment obeys all five laws in miniature: @status:impl/done

- @fact:MINIATURE-LAWS-ONE-AND-TWO dated verbatim quotes
  (Laws 1-2), @status:impl/done
- @fact:MINIATURE-LAW-THREE §4 answering §3 the other way (Law 3), @status:impl/done
- @fact:MINIATURE-LAW-FOUR D1 numbered and
  ratifying nothing (Law 4), @status:impl/done
- @fact:MINIATURE-LAW-FIVE a closing re-fetch list carrying the capture date and the
  subject's version at capture (Law 5). @status:impl/done

## Summary {#summary}

- @fact:SUM-COPY-THE-SKELETON-WHOLE Copy the skeleton whole; keep the section order — it is the reading
  shape. @status:impl/done
- @fact:SUM-FILL-THE-SOURCE-TABLE-AS-YOU-FETCH Fill the source table as you fetch; it is half the re-fetch list. @status:impl/done
- @fact:SUM-QUOTE-BEFORE-MAPPING Quote before mapping; map "no equivalent" as a real finding. @status:impl/done
- @fact:SUM-SHIP-BOTH-GAP-DIRECTIONS Ship §3 *and* §4; an empty lead section means you stopped early. @status:impl/done
- @fact:SUM-NUMBER-PRIORITIZE-AND-HOME-EVERY-DELTA Number, prioritize, and home every delta; ratify none. @status:impl/done
