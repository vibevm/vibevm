...
##PACKAGE-INSTALLS-A-DEFENSIVE-SECRETS-POSTURE A vibevm `flow` package that installs a **defensive secrets posture**
for a repository worked by a coding agent. @impl/done

##agent-sessions-remove-the-traditional-safety-margin Agents read a lot and their
...
log is a full disclosure. @spec/done

##PACKAGE-MAKES-THE-RULES-EXPLICIT-MECHANICAL-AND-ALWAYS-LOADED This package makes the handling rules
explicit, mechanical, and always-loaded so no code path and no session
puts a credential value on a surface. @impl/done

##THE-CORE-IDEA-IS-THE-SURFACE-SECRET The core idea is the **surface secret**: a credential whose *value*
never appears on any surface the tooling produces, while its *source*
(an env-var name, a file path) may be printed freely. @impl/done

##FOUR-LAWS-PLUS-SCOPE-DISCIPLINE-AND-A-CONSENT-GATE Four laws follow
...
gate for install-time code. @impl/done

##package-contents-lead This package ships three pieces of content plus a boot snippet: @impl/done

- ##CONTENT-THE-FULL-PROTOCOL `spec/flows/secrets-hygiene/SECRETS-HYGIENE-PROTOCOL.md` — the full
  ...
  suspected-leak drill (rotate first, investigate second). @impl/done
- ##CONTENT-THE-SCOPE-DISCIPLINE `spec/flows/secrets-hygiene/scope-discipline.md` — the
  ...
  a low-trust source cannot override a trusted answer. @impl/done
- ##CONTENT-THE-THIRD-PARTY-CODE-CONSENT `spec/flows/secrets-hygiene/third-party-code-consent.md` — the
  ...
  not inline strings, and secrets kept out of hook environments. @impl/done
- ##CONTENT-THE-BOOT-SNIPPET `spec/boot/57-flow-secrets-hygiene.md` — boot snippet loaded at
  session start: the standing rules and the never-do list. @impl/done
...
- ##COMPOSES-HEALTH-AUDIT `flow:health-audit`: add a periodic audit line that scans for new
  output paths (logs, JSON fields, error messages) that could echo a
  secret value, catching drift as the tool grows surfaces. @impl/done
- ##COMPOSES-CONFLICT-PROTOCOL `flow:conflict-protocol`: when it is genuinely uncertain whether a
  value is secret, the conservative default governs — treat it as
  secret and ask, rather than guessing it is safe to print. @impl/done

## Philosophical background {#background}

##crystallized-from-the-origin-projects-token-secrecy-law Crystallized from the origin project's token-secrecy law — a global
...
##generalized-here-to-any-product-credential-and-agent Generalized here to any product, credential, and coding
agent. @spec/done

##collections-spirit-is-the-redbook The collection's spirit is the book *AI-native development*,
which ships in Russian inside `flow:redbook` at `spec/book/ru/`. @spec/done
