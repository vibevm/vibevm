...
- ##BOUNDARY-FLOW-WAL **`flow:wal`** handles session continuity. A successful sync may
  ...
  this one. @impl/done
- ##BOUNDARY-FLOW-ATOMIC-COMMITS **`flow:atomic-commits`** handles commit discipline. A sync commit
  follows Conventional Commits and carries `docs(spec)` as its type;
  that framing is defined by the atomic-commits flow, not here. @impl/done
