...
##PACKAGE-INSTALLS-THE-ADDRESSABLE-SPECIFICATIONS-PRACTICE A vibevm `flow` package that installs the **addressable
specifications** practice into a project: `spec://` URIs, stable
`{#anchor}`s on every heading that decides something, and the spec
tree layout that makes both resolvable with zero tooling. @impl/done

##SPEC-FILES-ARE-THE-IPC-CHANNEL-NOT-DOCUMENTATION Spec files in a human-agent team are not documentation — they are
the IPC channel between two processes, and the first requirement on
that channel is addressability. @spec/done

##paraphrase-costs-hundreds-of-tokens-an-anchor-costs-twenty "You did the verification wrong"
...
300 s" costs about twenty and hits exactly. @spec/done

##PACKAGE-IS-THAT-DIFFERENCE-AS-A-STANDING-CONTRACT This package is that
difference, made into a standing contract. @impl/done

##package-contents-lead This package ships three pieces of content plus a boot snippet: @impl/done

- ##CONTENT-THE-PROTOCOL `spec/flows/addressable-specs/ADDRESSABLE-SPECS-PROTOCOL.md` —
  ...
  prompt for adapting the practice to a concrete project. @impl/done
- ##CONTENT-THE-AUTHORING-RULES `spec/flows/addressable-specs/authoring-rules.md` — how to write
  ...
  once cited. @impl/done
- ##CONTENT-THE-SPEC-TREE-LAYOUT `spec/flows/addressable-specs/spec-tree-layout.md` — the reference
  tree: PROP vs FEAT, the what-goes-where decision table, and the
  `.human/` private buffer enforced by ignore-file invisibility. @impl/done
- ##CONTENT-THE-BOOT-SNIPPET `spec/boot/15-flow-addressable-specs.md` — boot snippet loaded at
  session start: the correction contract, the single-source and
  placement rules, and the never-do list. @impl/done
...
- ##COMPOSES-DECISION-RECORDS `flow:decision-records` — records live at the anchors they govern;
  a decision without an address cannot be cited or superseded
  cleanly. @impl/done

## Philosophical background {#background}

##practice-extracted-from-the-book The practice is extracted from *AI-native development*, chapter 2
...
the practical file structure). @spec/done

##CHAPTER-SHIPS-IN-RUSSIAN-INSIDE-REDBOOK The chapter ships in Russian inside
`flow:redbook` at `spec/book/ru/`. @impl/done

##BOTTLENECK-IS-TELLING-THE-MACHINE Short version: the human knows
instantly what the agent got wrong; the bottleneck is telling the
machine — so make every fact in the project pointable in one URI. @spec/done
