//! `typescript-ai-native-tcg` — the agentic type oracle's delivery binary
...
fn main() -> Result<()> {
    ...
    let code = match cli.command {
        Cmd::Serve { root } => typescript_ai_native_tcg::run_serve(std::path::Path::new(&root))?,
        Cmd::Validate {
            file,
            content_from,
            json,
            root,
        } => typescript_ai_native_tcg::run_validate(
        ...
        Cmd::Scope {
            ...
            json,
            root,
        } => typescript_ai_native_tcg::run_scope(
            std::path::Path::new(&root),
            &file,
            position.as_deref(),
            json,
        ...
        Cmd::Complete {
            ...
            json,
            root,
        } => typescript_ai_native_tcg::run_complete(
            std::path::Path::new(&root),
            &file,
            &position,
            prefix.as_deref(),
        ...
        Cmd::Type {
            ...
            typescript_ai_native_tcg::run_type(std::path::Path::new(&root), &file, &position, json)?
        }
        Cmd::Bench {
            corpus,
            report,
            root,
        } => typescript_ai_native_tcg::run_bench(std::path::Path::new(&root), &corpus, &report)?,
    };
