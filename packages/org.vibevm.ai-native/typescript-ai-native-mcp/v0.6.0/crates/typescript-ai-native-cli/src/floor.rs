//! `typescript-ai-native floor` — the seven-step verification floor
...
pub fn run_floor(root: &Path, opts: &FloorOptions) -> Result<()> {
    ...
    if !is_disabled("conform") {
        header(opts, "typescript-ai-native-conform check");
        let ok = typescript_ai_native_conform::run_check(
            root,
    ...
    if !is_disabled("specmap") {
        header(opts, "typescript-ai-native-specmap --check");
        let ok = typescript_ai_native_specmap::run_specmap_typescript(root, true)
            .map(|()| true)
            .unwrap_or_else(|e| {
                eprintln!("{e}");
                false
        ...
        if baseline.exists() {
            ...
            eprintln!(
                "\nfloor: no tests baseline at {} — the test-gate step arms when \
                 `typescript-ai-native init` writes it",
