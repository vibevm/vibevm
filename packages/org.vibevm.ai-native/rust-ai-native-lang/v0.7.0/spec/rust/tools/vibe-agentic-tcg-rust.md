...
##COMMISSIONED-AS-THE-RUST-TWIN *Commissioned 2026-07-07 as the Rust twin of the agentic tcg
...
AGENTIC-TCG-RUST-PLAN v0.1.* @impl/done
...
cut for exactly this arrival: a new value, not new tools).* @impl/done

## 1. What problem it solves {#problem}

##RUST-AGENTS-LEARN-TYPE-ERRORS-AT-WRITE-GRAIN Rust is this project's primary language, and a Rust-editing agent today
learns about a type error at write-grain latency: write the file, run
`cargo check` (seconds), parse stderr, retry. @spec/done

##MOST-OF-THE-MASKS-VALUE-IS-DELIVERABLE-WITHOUT-IT The TS campaign proved
...
##THE-LOAD-BEARING-DIFFERENCE-RA-IS-NOT-RUSTC The Rust twin closes the same gap with one load-bearing difference,
stated here rather than discovered later: **the TS oracle IS the
compiler** (the LanguageService is tsc's engine — agreement by
construction), while **rust-analyzer is NOT rustc**. @spec/done

##RA-NATIVE-DIAGNOSTICS-ARE-PARTIAL Its native
diagnostics are a separate implementation with deliberately partial
coverage. @spec/done

##RUST-ORACLE-IS-AN-HONEST-APPROXIMATION The Rust oracle is therefore an honest APPROXIMATION: fast,
...
- ##STANCE-STAND-ON-THE-CONSUMERS-RUST-ANALYZER **Stand on the consumer's rust-analyzer, over LSP.** The oracle
  ...
  version-binding grounds. @impl/done
- ##STANCE-OVERLAYS-NEVER-DISK **Overlays, never disk.** LSP text-document overlays (`didOpen` /
  ...
  answered in 1–2 ms warm. @impl/done
- ##STANCE-CONFIG-IS-LOAD-BEARING **Config is load-bearing.** rust-analyzer's most valuable native
  ...
  blind — the Phase-0 spike's iteration 1 proved it by silence. @impl/done
- ##STANCE-ENRICHMENT-IS-IN-PROCESS **Enrichment is in-process — one engine, zero extra hops.** The gate's
  ...
  ratchet baseline, with Class-F advice citing GUIDE REQs. @impl/done
- ##STANCE-FIDELITY-POSTURE-IS-SPEC **The fidelity posture is spec, not fine print.** The differential
  ...
  asymmetry — it flips red the day r-a starts covering it. @impl/done
- ##STANCE-DUAL-TRANSPORT **Dual transport, agent's choice** (PROP-018 §2.8): a persistent
  ...
  (`vibe bin exec rust-ai-native-tcg -- validate …`) for agents without MCP. @impl/done
...
##three-processes-lead Three processes; the facts hop of the TS topology is gone because
enrichment happens inside the relay: @impl/done
...
- ##COMPONENT-THE-BRIDGE **The bridge** (`crates/rust-ai-native-tcg-bridge`, this package): the
  ...
  five-way typed error taxonomy, replay-tested without rust-analyzer. @impl/done
- ##COMPONENT-THE-CLI **The CLI** (`crates/rust-ai-native-tcg`, bin **`rust-ai-native-tcg`**, the package's
...
- ##STAGE-A-CONSULTATION-ORACLE **Stage A — the consultation oracle (THIS brief):** validate / scope /
  ...
  backlogged cross-language Stage B). @impl/done
- ##STAGE-B-RICHER-DISCIPLINE-ADVICE **Stage B — richer discipline advice:** newtype-constructor
  suggestions at seams, policy-ranked completions, cell-topology
  answers — fields the protocol already carries. @spec/done
- ##STAGE-C-DELIVERY-EXPERIMENTS **Stage C — delivery experiments** (cross-language, with the TS twin,
  when the owner commissions Stage B of the delivery line): MCP-mounted
  and write-path-hook arms over a rust task battery on
  `research/rust-demo`. @spec/done
- ##STAGE-D-RA-AP-EMBEDDING **Stage D — the `ra_ap_*` embedding (FAR BACKLOG, owner-dispositioned
  ...
  owner's word; recorded in vibevm ROADMAP.md's Far backlog. @spec/done
- ##STAGE-E-TOKEN-LEVEL-TCG **Stage E — token-level TCG** (`rust-ai-native-tcg.md`, VERY-FAR-future):
  ...
  answers changes. @spec/done

## 5. Licensing posture {#licensing}

- ##LICENSING-RUST-ANALYZER rust-analyzer: MIT/Apache-2.0, and the consumer brings their own
  component (we spawn, never link, never vendor). @spec/done
...
- ##LICENSING-PLDI-PACKAGE-IS-NOT-A-DEPENDENCY The PLDI'25 reproduction package: **not a dependency, not opened**
  for the agentic line (the standing clean-room rule,
  `spec/boot/90-user.md`); this campaign's concept sources are our own
  briefs, the LSP spec, and rust-analyzer's public documentation. @impl/done
...
- ##RISK-RA-IS-NOT-RUSTC **r-a is not rustc — the defining risk, owned in the spec.** Native
  ...
  and every consumer-facing doc says: the floor is the truth. @spec/done
- ##RISK-NO-GUARANTEE-BY-CONSTRUCTION **No guarantee by construction.** The agent may ignore the oracle —
  ...
  measured question, not this brief's claim. @spec/done
...
- ##RISK-RA-VERSION-CAPABILITY-VARIANCE **r-a version/capability variance.** Weekly releases; capabilities
  ...
  is named in TCG-ORACLE-RUST. @spec/done
...
##ONE-LINE-SUMMARY `vibe-agentic-tcg-rust` gives a coding agent millisecond-latency,
...
(the only thing this is NOT) waits for an inference substrate far away. @impl/done
