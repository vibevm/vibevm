//! `rust-ai-native-tcg serve` — the persistent enriching relay
...
fn position_param(
    params: &serde_json::Value,
) -> Option<rust_ai_native_tcg_bridge::position::OuterPosition> {
    let p = params.get("position")?;
    if let Some(s) = p.as_str() {
        return parse_position(s).ok();
    }
    Some(rust_ai_native_tcg_bridge::position::OuterPosition {
        ...
        "scope" => match &file {
            Some(f) => {
                let pos = position_param(params).unwrap_or(
                    rust_ai_native_tcg_bridge::position::OuterPosition {
                        line: 1,
                        character: 0,
                    },
                );
...
pub fn run_serve(root: &Path) -> Result<i32> {
    ...
    eprintln!(
        "rust-ai-native-tcg serve: oracle up — rust-analyzer {}, quiescent={}, {}",
