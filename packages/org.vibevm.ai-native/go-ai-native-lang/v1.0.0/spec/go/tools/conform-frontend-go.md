...
@fact:status-line *Status: **SHIPPED with this package** (GO-AI-NATIVE-PLAN Phases 4–5) —
...
`go-ai-native-extract-bridge` NDJSON protocol.* @status:impl/done

@fact:GO-COUNTERPART-OF-THE-SIBLING-FRONTENDS *The Go counterpart of
...
linter.* @status:impl/done

## 1. The division of labour with the native Go tooling {#division}

@fact:kind-line-division `req r1` @status:impl/done

@fact:GO-TOOLCHAIN-CARRIES-THE-TYPE-CORRECTNESS-HALF Go's own toolchain already carries the **type / correctness** half:
...
§1, §5). @status:spec/done

@fact:THAT-HALF-IS-WELL-TYPED-AND-LOCALLY-SANE Those answer *"is this well-typed and locally sane?"* — the
half the language does natively and well. @status:spec/done

@fact:frontend-answers-the-structural-half-lead This frontend answers the **other** half — the *structural /
architectural* rules no Go tool expresses, the ones `conform check`
already enforces for Rust and TypeScript: @status:impl/done

- @fact:RULE-FILE-LENGTH-BUDGET the file-length budget (position is a resource, guide §3); @status:impl/done
- @fact:RULE-CELL-ISOLATION cell isolation (a cell imports seams + core only, never sibling
  cells; the registry is the only cell importer — guide §2/§6); @status:impl/done
- @fact:RULE-BAN-CENSUS-AS-FACTS the ban census as facts (`init()` in a cell, blank imports, ambient
  ...
  findings in the Class-F `violates REQ …; fix surface: …` grammar; @status:impl/done
- @fact:RULE-SEAM-ERROR-CONTRACT the seam-error contract (`go-seam-error-cites-req` — a seam's closed
  error set carries its REQ URI, guide §5): one rule, both halves — the structure half flags a `*Error` type that owns an `Error()` but has no `Spec` field; the message half flags an `Error()` whose rendered text carries no REQ, where a REQ is counted as cited by the literal `spec://` OR the `violates REQ` marker (Go renders the URI out of the `Spec` field, while the format string itself carries the `violates REQ %s` marker); @status:impl/done
- @fact:RULE-DEVIATION-ESCAPE-HATCH the deviation escape hatch (`//spec:deviates … reason="…"`), honoured
  the way `#[spec(deviates)]` is for Rust. @status:impl/done

@fact:ONE-ENGINE-ONE-GRAMMAR-ONE-BASELINE Routing these through conform keeps **one rule engine, one finding
...
a rule cannot drift between projections. @status:impl/done

## 2. What go-extract is {#extractor}

@fact:kind-line-extractor `req r1` @status:impl/done

@fact:GO-EXTRACT-IS-A-FACT-PRODUCER A fact producer: parse a `.go` file and emit the language-neutral fact
stream the rules consume. @status:impl/done

@fact:GO-EXTRACT-IS-STDLIB-ONLY **Stdlib-only by construction** — `go/parser`,
...
requires. @status:impl/done

@fact:MINIMUM-EXTERNAL-TOOLING-MADE-STRUCTURAL This is the owner's "minimum external tooling" ideal made
structural: the language parses itself, one file, zero dependencies. @status:impl/done

- @fact:DELIVERY-EMBEDDED-AND-CONTENT-ADDRESSED **Delivery:** embedded in the Rust bridge crate (`include_str!`),
  ...
  materialised, the source stays import-free of sibling tool files. @status:impl/done
- @fact:PROTOCOL-NDJSON-ON-STDIO **Protocol:** NDJSON on stdio — a `{proto, files: [...]}` request, one
  `{file, facts: [...], markers: [...]}` record per input, `PROTOCOL =
  1`, additive evolution, unparseable file → zero facts + a `degraded`
  note, never a crash (B5). @status:impl/done
- @fact:FACT-KINDS **Fact kinds** (the ts-extract vocabulary, Go-shaped): `item`
  ...
  directive stream: tag, uri, r, reason, attached symbol, line). @status:impl/done
- @fact:ONE-EXTRACTION-TWO-CONSUMERS **One extraction, two consumers:** the conform frontend consumes
...
@fact:FRONTEND-IMPLEMENTS-THE-ENGINE-TRAIT `go-ai-native-conform-frontend` implements the engine's `Frontend`
...
Vec<Fact>` that round-trips through the bridge. @status:impl/done

@fact:FACTS-ARE-CONTENT-ADDRESSED Facts are keyed
`(file content-hash, frontend id+version)` in the engine's
content-addressed store — a 1-file diff re-extracts 1 file (A2). @status:impl/done

## 4. Topology: the `[go]` policy section {#topology}

@fact:kind-line-topology `req r1` @status:impl/done

@fact:CONFORM-TOML-GAINS-A-GO-SECTION `conform.toml` gains a `[go]` section, written by `go-ai-native init`
...
this surface names which ones the Go rules read, not their values. @status:impl/done

@fact:THE-SARIF-CITATION-PATH-IS-FED-BY-FOREIGN-LINTERS-NOT-BY-GO-EXTRACT The
...
not feed. @status:impl/done

@fact:EVERY-PACKAGE-GATED-OR-EXEMPT The
every-package-gated-or-exempt invariant is enforced by the engine on
every check, exactly as for the sibling stacks. @status:impl/done

## 5. The honest note {#honesty}

@fact:GO-EXTRACT-IS-A-PARSER-NOT-A-TYPE-CHECKER The structural gate is only as good as its facts, and go-extract is a
PARSER, not a type checker: it sees syntax and directives, not
resolved types. @status:impl/done

@fact:TYPE-DEPENDENT-RULES-ARE-OUT-OF-SCOPE Rules that would need type information (e.g. "this
call's receiver is a seam type") are out of this tool's scope and
belong to the vet/staticcheck/exhaustive evidence tier or the oracle. @status:impl/done

@fact:THE-DIVISION-IS-THE-THREE-TIER-SPLIT The division is deliberate — the same three-tier split (T-lex/T-syn/
T-sem) ENGINE-CONFORM §1 defines; go-extract is the T-syn tier, and
Go's T-sem tier is the toolchain itself. @status:impl/done
