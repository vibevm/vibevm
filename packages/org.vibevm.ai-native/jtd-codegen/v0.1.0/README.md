...
##WHAT-IT-IS `jtd-codegen` generates strictly-typed, language-specific code
...
hand-maintained duplicates on either side of a contract. @impl/done

##THIS-PACKAGE-IS-THE-RECIPE-NOT-THE-BINARY **This package ships the
...
##UPSTREAM Upstream:
<https://github.com/jsontypedef/json-typedef-codegen>. @impl/done

## Pinned version {#pin}

##PINNED-VERSION **`jtd-codegen 0.4.1`** — the most recent stable release at
...
it does not enforce a particular generator build. @impl/done

## Install {#install}

##INSTALL-TARGET Drop the platform binary at
`tools/jtd-codegen/jtd-codegen` (or `jtd-codegen.exe` on Windows) inside
the consuming project, and keep that directory gitignored. @impl/done

### Windows {#install-windows}

##INSTALL-WINDOWS From the project root, in PowerShell or Git Bash: @impl/done
...
##WINDOWS-GNU-BUILD Upstream ships a `gnu` build, not `msvc` — the static
binary works on the Windows hosts the projects target. @impl/done

### macOS {#install-macos}

##INSTALL-MACOS Apple Silicon (Intel: swap in
`x86_64-apple-darwin.tar.gz`): @impl/done
...
##INSTALL-LINUX One command: @impl/done
...
##VERIFY-VERSION The installed binary answers with the pinned version: @impl/done
...
##CONSUMER-WIRES-ITS-OWN-TASK The consumer wires the binary into its own
...
actionably, pointing at this recipe, when both are missing. @impl/done
