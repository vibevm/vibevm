...
///
/// ```
/// use core_ai_native_mcp::JsonRpcMessage;
///
/// let m = core_ai_native_mcp::parse_line(r#"{"jsonrpc":"2.0","id":1,"method":"ping"}"#).unwrap();
/// assert!(matches!(m, JsonRpcMessage::Request(r) if r.method == "ping"));
/// let n = core_ai_native_mcp::parse_line(r#"{"jsonrpc":"2.0","method":"notifications/initialized"}"#)
///     .unwrap();
...
///
/// ```
/// use core_ai_native_mcp::JsonRpcResponse;
///
/// let ok = JsonRpcResponse::ok(1.into(), serde_json::json!({"pong": true}));
/// assert!(ok.render().unwrap().contains("\"pong\":true"));
/// let err = JsonRpcResponse::error(2.into(), core_ai_native_mcp::JsonRpcError::method_not_found("x/y"));
...
///
/// ```
/// let e = core_ai_native_mcp::JsonRpcError::invalid_params("missing `name`");
