//! bin `go-ai-native-mcp` — launch the server on stdio for one project
//! root. Agent hosts run this straight from the slot (the
...
#[command(
    ...
    about = "The AI-Native Go discipline + tcg type oracle over MCP (PROP-027)"
)]
struct Cli {
...
fn main() -> Result<()> {
    let cli = Cli::parse();
    // The verbatim-free absolutisation: the same many-times-learned
    // lesson applies to every child this server spawns (gopls, go) —
    // no `\\?\` paths downstream.
    let root = go_ai_native_tcg_bridge::verbatim_free(
        &cli.path.canonicalize().unwrap_or_else(|_| cli.path.clone()),
    );
    eprintln!(
        "go-ai-native-mcp: serving `{}` on stdio ({} tools; protocol {})",
        root.display(),
        go_ai_native_mcp::TOOL_NAMES.len(),
        mcp_core::PROTOCOL_VERSION,
    );
    let tools = go_ai_native_mcp::tool_set(&root);
