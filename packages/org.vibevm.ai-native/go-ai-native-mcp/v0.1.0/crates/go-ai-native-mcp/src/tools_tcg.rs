...
//! harness, over ONE persistent gopls session shared by all five tools
//! (lazy spawn on first use, respawn-once on a crashed session — the
//! posture the serve relay and the sibling servers carry). Enrichment
//! goes through the SAME lib fns the `go-ai-native-tcg` one-shots call
//! (`go_ai_native_tcg::enrich_validate` over the gate's own rules) —
//! one engine, one truth, and the policy reloads per call so a
//! mid-session freeze is honoured immediately.

specmark::scope!("spec://go-ai-native-mcp/tools/discipline-mcp-go#tcg-tools");
...
type Oracle = GoOracle<ChildTransport>;
...
fn tcg_scope(session: &mut TcgSession, args: &Value) -> ToolOutput {
    ...
    match session.with_oracle(|o| o.complete(&file, pos, content.clone())) {
        Ok(entries) => {
            let symbols = finalise_completions(&policy.config, entries, &file, None, 200);
            let cell = cell_of(&policy.config, &file);
            let seam = seam_file_for(&policy.config, &file);
            let mut branded = Vec::new();
            let mut sources: Vec<String> = vec![file.clone()];
            if let Ok(dir) = std::fs::read_dir(policy.root.join(&seam)) {
                for entry in dir.filter_map(std::result::Result::ok) {
                    if entry.path().extension().and_then(|e| e.to_str()) == Some("go") {
                        sources.push(format!("{seam}/{}", entry.file_name().to_string_lossy()));
                    }
                }
            }
            sources.sort();
...
fn tcg_complete(session: &mut TcgSession, args: &Value) -> ToolOutput {
    ...
    match session.with_oracle(|o| o.complete(&file, pos, content.clone())) {
        Ok(entries) => ToolOutput::ok(pretty(&json!({
            "entries": finalise_completions(&policy.config, entries, &file, prefix.as_deref(), max),
        }))),
        Err(e) => bridge_failure(&e),
    }
}
...
fn tcg_bench(session: &mut TcgSession, args: &Value) -> ToolOutput {
    ...
    match mcp_core::capture(|| go_ai_native_tcg::bench::run_bench(&root, &corpus, &report)) {
...
pub fn tcg_tools(root: &Path) -> Vec<Box<dyn Tool>> {
    ...
    vec![
        t(
            "tcg_validate",
            "Check a file (or a hypothetical overlay) BEFORE writing it: gopls \
             diagnostics + the conform gate's own findings flagged against the frozen \
             ratchet + the //spec: marker stream + REQ-citing advice. isError mirrors \
             the one-shot exit contract: an error diagnostic OR a non-baselined \
             finding. = `go-ai-native-tcg validate`.",
            json!({"file": file, "content": content, "language": lang}),
            tcg_validate,
        ),
        t(
            "tcg_scope",
            "What is in scope here: completion-sweep symbols, the package cell, the \
             seams package, and defined-type brands (heuristic, labelled). \
             = `go-ai-native-tcg scope`.",
            json!({"file": file, "position": position, "content": content, "language": lang}),
            tcg_scope,
        ),
        t(
            "tcg_complete",
            "Type-valid continuations at a position, with the §2-ban flag on ambient \
             defaults inside cells. = `go-ai-native-tcg complete`.",
            json!({
        ...
        t(
            ...
            "The type at a position (hover): display text + documentation. \
             = `go-ai-native-tcg type`.",
            json!({"file": file, "position": position, "content": content, "language": lang}),
            tcg_type,
        ),
        t(
            "tcg_bench",
            "The differential corpus harness: oracle-vs-go-build agreement + latency \
             report. = `go-ai-native-tcg bench`. Heavy: materialises scratch copies and \
             runs go build per case — expect minutes.",
            json!({
                "corpus": {"type": "string", "description": "root-relative corpus JSON (required)"},
