# Tool Spec: `vibe-agentic-tcg-ts` — the Agentic Type Oracle for TypeScript {#root}
...
@fact:COMMISSIONED-AS-THE-AGENTIC-DELIVERY *Commissioned 2026-07-07 as the AGENTIC delivery of the tcg line —
the sibling of `typescript-ai-native-tcg.md` (token-level, VERY-FAR-future) — and
implemented by AGENTIC-TCG-TS-PLAN v0.1.* @status:impl/done
...
MCP seam is vibevm's PROP-026.* @status:impl/done

## 1. What problem it solves {#problem}

@fact:TCD-MASKS-LOGITS-INSIDE-THE-SAMPLER True type-constrained decoding masks logits inside the sampler. @status:spec/done

@fact:HOSTED-AGENTS-NEVER-EXPOSE-LOGITS A hosted
coding agent (Claude Code, opencode, Codex over their APIs) never exposes
logits — the harness receives sampled text. @status:spec/done

@fact:THE-GUARANTEE-IS-UNREACHABLE-IN-AGENTIC-MODE So the by-construction
...
line's value indefinitely. @status:spec/done

@fact:decompose-what-the-mask-buys-lead Decompose what the mask actually buys, and most of it is not the mask: @status:spec/done

1. @fact:VALUE-GUARANTEE **Guarantee** — a type-invalid continuation cannot be emitted. Decode-
   loop-only. NOT deliverable agentically; stays with the token-level
   sibling. @status:spec/done
2. @fact:VALUE-INFORMATION **Information** — "what is in scope here, with what type; does this
   fragment type-check; what continuations are type-valid at this
   position." Pure data. Deliverable TODAY as tool responses. @status:impl/done
3. @fact:VALUE-FEEDBACK-LATENCY **Feedback latency** — a mask rejects instantly; an agent today learns
   ...
   fact on ts-demo) without touching disk. @status:impl/done
4. @fact:VALUE-DISCIPLINE-AT-GENERATION-TIME **Discipline at generation time** — the §8 unsafe-set bans, branding
   ...
   conform engine the gate runs. @status:impl/done

@fact:AGENTIC-TCG-SHIPS-VALUES-TWO-THROUGH-FOUR `vibe-agentic-tcg-ts` ships 2–4: a long-lived language-service oracle the
agent CONSULTS, instead of a mask the sampler OBEYS. @status:impl/done

@fact:GUARANTEE-STAYS-WITH-THE-FLOOR The guarantee (1)
stays where it already lives — the floor. @status:impl/done

@fact:ORACLE-MAKES-RED-FLOOR-ITERATIONS-RARE The oracle's job is to make
red floor iterations rare; the floor's job is unchanged: to be the truth. @status:impl/done

## 2. Design stance (consequences of what we know) {#design-stance}

- @fact:STANCE-STAND-ON-THE-REAL-COMPILER **Stand on the real compiler, not a model of it.** The oracle is an
  ...
  trivially: their code is not needed and not read. @status:impl/done
- @fact:STANCE-OVERLAYS-NEVER-DISK **Overlays, never disk.** Validation of a HYPOTHETICAL file state —
  ...
  the write happens once, already-typed. @status:impl/done
- @fact:STANCE-ONE-EXTRACTION-TWO-CONSUMERS **One extraction, two consumers.** The oracle's `validate` returns the
  ...
  conform" in one roundtrip. @status:impl/done
- @fact:STANCE-POLICY-INTERPRETATION-LIVES-IN-RUST **Policy interpretation lives in Rust, once.** The node side emits
  ...
  when there is one engine. @status:impl/done
- @fact:STANCE-DUAL-TRANSPORT **Dual transport, agent's choice** (the PROP-018 §2.8 idiom): a
  ...
  or for scripts. Same protocol shape on both. @status:impl/done
- @fact:STANCE-CAPABILITY-ROUTING-IS-FREE **Capability routing is free here.** DR1-015's over-constraint risk
  ...
  (gpt-oss-20b) because that is where the lift is claimed. @status:impl/done

## 3. Component shape (how it fits vibevm) {#component-shape}

@fact:three-processes-lead Three processes, two thin NDJSON hops, each layer doing the one thing
the layer below cannot: @status:impl/done
...
- @fact:COMPONENT-THE-ORACLE **The oracle** (`tools/ts-oracle/oracle.ts`, this package): self-
  ...
  `type`, `update`, `shutdown` (TCG-PROTOCOL-v0.1). @status:impl/done
- @fact:COMPONENT-THE-BRIDGE **The bridge** (`crates/typescript-ai-native-tcg-bridge`): persistent child client —
  correlation ids, timeouts, kill-on-drop, five-way typed error taxonomy,
  replay-tested without node. @status:impl/done
- @fact:COMPONENT-THE-CLI **The CLI** (`crates/typescript-ai-native-tcg`, bin **`typescript-ai-native-tcg`**,
...
- @fact:COMPONENT-THE-PRODUCT-SEAM **The product seam** (vibevm, PROP-027; PROP-026 §2 keeps the
  ...
  has zero vibe-mcp imports. @status:impl/done
- @fact:COMPONENT-DETERMINISM-AND-AUDITABILITY **Determinism and auditability:** given (project state, overlay set,
  ...
  `spec://` REQs Class-F style; nothing here samples anything. @status:impl/done

## 4. Staged ambition {#staged-ambition}

- @fact:STAGE-A-CONSULTATION-ORACLE **Stage A — the consultation oracle (THIS brief, shipped by the plan):**
  validate/scope/complete/type over overlays, discipline-enriched,
  MCP + one-shot delivery, measured by the two-arm battery. @status:impl/done
- @fact:STAGE-B-RICHER-DISCIPLINE-ADVICE **Stage B — richer discipline advice:** brand-constructor suggestions,
  ...
  protocol already carries fields for. @status:spec/done
- @fact:STAGE-C-THE-RUST-TWIN-AGENTICALLY **Stage C — the Rust twin agentically** (`rust-ai-native-tcg` over rust-analyzer,
  a separate commissioning): the language parameter and PROP-026 are cut
  to admit it without new surface. @status:spec/done
- @fact:STAGE-D-TOKEN-LEVEL-TCG **Stage D — token-level TCG (the sibling brief, VERY-FAR-future,
  ...
  answers changes. The clean-room rule re-binds there. @status:spec/done

## 5. Licensing posture {#licensing}

- @fact:LICENSING-TS-COMPILER-API-AND-LANGUAGE-SERVICE TypeScript Compiler API / LanguageService: Apache-2.0 — clean, and the
  consumer brings their own install (we link nothing). @status:spec/done
- @fact:LICENSING-NODE-NDJSON-AND-OUR-ENGINES node, NDJSON, our own engines: in-house or std. @status:spec/done
- @fact:LICENSING-PLDI-PACKAGE-IS-NOT-A-DEPENDENCY The PLDI'25 reproduction package: **not a dependency, not a reference,
  not opened** for the agentic line (inspiration-only rule,
  `spec/boot/90-user.md`). No third-party grammar/CFG tooling is needed
  at all in agentic mode. @status:impl/done
...
- @fact:RISK-NO-GUARANTEE-BY-CONSTRUCTION **No guarantee by construction.** The agent may ignore the oracle.
  ...
  against Stage B investment, recorded, not hidden). @status:spec/done
- @fact:RISK-LATENCY-UNDER-SCALE **Latency under scale.** The Phase-0 spike says 22 ms warm validate on
  ...
  posted targets move WITH a recorded reason. Never a silent miss. @status:spec/done
- @fact:RISK-LS-VS-TSC-DIVERGENCE **LS-vs-tsc divergence.** Same engine underneath but options are
  ...
  same API tsc uses from the start. @status:spec/done
- @fact:RISK-WINDOWS-CHILD-LIFECYCLE **Windows child lifecycle.** Junction/verbatim/PATH quirks are known
  house lessons; kill-on-drop + shutdown op + no-zombie assertions are
  Phase-0-proven and re-asserted in the hermetic test. @status:impl/done
- @fact:RISK-STRONG-MODEL-INDIFFERENCE **Strong-model indifference.** Plausible (DR1-015 inverted); the
  battery deliberately measures the weak population where the claim
  lives. A strong-agent arm is a later, optional measurement. @status:spec/done
- @fact:RISK-SCOPE-CREEP-TOWARD-AN-LSP-RELAY **Scope creep toward an LSP relay.** The surface is the four queries +
  lifecycle, full stop; rename/code-actions/references are named
  non-goals (PROP-026 carries the line). @status:impl/done

## 7. One-line summary {#summary}

@fact:ONE-LINE-SUMMARY `vibe-agentic-tcg-ts` gives a coding agent the type oracle's INFORMATION
...
an inference substrate on its own very-far-future track. @status:impl/done
