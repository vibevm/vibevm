...
- **The bridge** (`crates/typescript-ai-native-tcg-bridge`): persistent child client —
  correlation ids, timeouts, kill-on-drop, five-way typed error taxonomy,
  replay-tested without node.
- **The CLI** (`crates/typescript-ai-native-tcg`, bin **`typescript-ai-native-tcg`**,
...
- **Stage C — the Rust twin agentically** (`rust-ai-native-tcg` over rust-analyzer,
