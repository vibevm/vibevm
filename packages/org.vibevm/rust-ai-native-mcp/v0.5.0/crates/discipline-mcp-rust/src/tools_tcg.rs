...
//! fns the `tcg-rust` one-shots call (`tcg_cli_rust::enrich_validate`
...
specmark::scope!("spec://rust-ai-native-mcp/tools/discipline-mcp-rust#tcg-tools");
...
fn tcg_bench(session: &mut TcgSession, args: &Value) -> ToolOutput {
    ...
    match mcp_core::capture(|| {
        tcg_cli_rust::bench::run_bench(&root.join(corpus), &root.join(report), &root)
    }) {
    ...
    vec![
        t(
            ...
            "Check a file (or a hypothetical overlay) BEFORE writing it: rust-analyzer \
             ...
             an error diagnostic OR a non-baselined finding. = `tcg-rust validate`.",
        ...
        t(
            ...
            "Type-valid continuations at a position, with the §6-ban flag on unsafe \
             forms (unwrap/expect in domain code). = `tcg-rust complete`.",
            json!({
        ...
        t(
            ...
            "The differential corpus harness: oracle-vs-cargo agreement + latency \
             report. = `tcg-rust bench`. Heavy: materialises a scratch workspace and \
