//! bin `discipline-mcp-rust` — launch the server on stdio for one
...
fn main() -> Result<()> {
    ...
    eprintln!(
        "discipline-mcp-rust: serving `{}` on stdio ({} tools; protocol {})",
        root.display(),
        discipline_mcp_rust::TOOL_NAMES.len(),
        mcp_core::PROTOCOL_VERSION,
    );
    let tools = discipline_mcp_rust::tool_set(&root);
    let mut server = mcp_core::Server::new(
        discipline_mcp_rust::SERVER_NAME,
