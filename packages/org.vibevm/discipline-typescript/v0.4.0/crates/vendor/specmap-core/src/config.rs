...
pub struct Config {
    ...
    /// `specmap.json`. Typically generated by `discipline-rust init` from
...
///
/// ```
/// let e: specmap_core::config::ExternalSpec = toml::from_str(
///     "namespace = \"discipline-core\"\nroot = \"vibedeps/flow-discipline-core/0.3.0/spec\"\n",
/// )
/// .unwrap();
/// assert_eq!(e.namespace, "discipline-core");
/// ```
#[derive(Debug, Clone, Deserialize)]
        ...
        if cfg.namespace.is_empty() {
            anyhow::bail!(
                "{}: `namespace` is required — the spec://<namespace>/… segment minted \
                 for this project's units (run `discipline-rust init`, or add e.g. \
                 `namespace = \"myproject\"`)",
                path.display()
            );
        }
...
mod tests {
    ...
    fn external_specs_parse() {
        ...
        assert_eq!(cfg.external_specs[0].namespace, "discipline-core");
    }
