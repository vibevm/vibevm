...
specmark::scope!("spec://discipline-typescript/tools/discipline-mcp-typescript#discipline-tools");
...
pub(crate) fn language_mismatch(args: &Value) -> Option<ToolOutput> {
    ...
    Some(ToolOutput::failed(format!(
        "this server serves language `typescript`; asked for `{asked}` — mount \
         that language's own discipline server (mcp:org.vibevm/discipline-{asked}) \
...
mod tests {
    ...
    fn language_mismatch_refuses_with_the_recipe() {
        ...
        assert!(out.is_error);
        assert!(out.report.contains("discipline-rust"), "{}", out.report);
