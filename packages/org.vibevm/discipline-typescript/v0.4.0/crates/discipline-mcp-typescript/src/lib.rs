//! `discipline-mcp-typescript` — the AI-Native TypeScript discipline
...
//! `specmap-typescript` CLIs call, each wrapped in the stderr-capture
...
specmark::scope!("spec://discipline-typescript/tools/discipline-mcp-typescript#root");
...
pub const SERVER_NAME: &str = "discipline-typescript";

/// Assemble the full tool set for a project root: 12 discipline + 5
/// tcg = 17 tools, the whole `discipline-typescript` + `tcg-typescript`
/// command surface (the TS umbrella has no ledger).
///
/// ```
/// let set = discipline_mcp_typescript::tool_set(std::path::Path::new("."));
