//! `discipline-typescript fast-loop` — the Class-E
//! `cell-fast-loop-present` checker, TS shape (scaffold-e-fast-loop):
//! every cell's tests run in isolation inside the per-cell budget,
//! measured wall-clock to the verdict via node's TAP reporter (the
//! same parser the test-gate uses, so the two gates cannot disagree on
//! what a test result is).
//!
//! A deliberate asymmetry vs the Rust twin, recorded here: the Rust
//! fast loop builds AND tests each cell in isolation (`nextest -p`);
//! `tsc` has no per-cell project mode without project references, so
//! type-checking stays a whole-tree floor step and the per-cell loop
//! measures the test half. A cell without tests fails the check — the
//! card requires the loop to EXIST.

use std::path::Path;
use std::time::Instant;

use anyhow::{Result, bail};
use walkdir_shim::has_tests;

mod walkdir_shim {
    use std::path::Path;

    /// Whether a cell directory carries any `*.test.ts` / `*.spec.ts`.
    pub fn has_tests(dir: &Path) -> bool {
...
pub fn run_fast_loop(
    ...
    let Some(cells_dir) = config.typescript.cells_dir.clone() else {
        bail!(
            "fast-loop: no `cells_dir` in conform.toml [typescript] — the cell \
             layout is what the per-cell loop measures (set it, e.g. \
             cells_dir = \"src/cells\")"
        );
    };
