//! `discipline-typescript trace …` — traceability queries over the
//! specmap (PROP-014 §2.6), built fresh through the ts-tsc scanner so
//! explain answers for the tree as it is, never a stale artefact. The
//! render half is the neutral engine's — text, JSON, and the
//! ledger-cached prose are byte-identical in shape to the Rust twin.
