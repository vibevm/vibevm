...
fn init_then_gates_catch_violations_and_the_tagged_tree_passes() {
    ...
    let err =
        conform_cli_typescript::run_check(root, conform_cli_typescript::DEFAULT_TS_BASELINE, None)
    ...
    specmap_cli_typescript::run_specmap_typescript(root, false).expect("mint warns only");
    let err = specmap_cli_typescript::run_specmap_typescript(root, true)
    ...
    specmap_cli_typescript::run_specmap_typescript(root, false).expect("mint");
    specmap_cli_typescript::run_specmap_typescript(root, true).expect("check clean");
}
