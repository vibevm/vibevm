...
**Home.** `flow:org.vibevm/core-ai-native`, `spec/mechanisms/` — this mechanism ships with the Discipline (URIs `spec://core-ai-native/mechanisms/PROP-014#…`); its Rust implementation ships in `stack:org.vibevm/rust-ai-native-lang` (`specmap-core` + the `specmap-rust` binary). The tag *syntax* shown throughout (`#[spec]`, `scope!`) is the Rust projection; other language stacks ship their own projection of the same model.
