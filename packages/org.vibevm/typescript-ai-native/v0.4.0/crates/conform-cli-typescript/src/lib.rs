//! The `conform-typescript` gate driver (the TS twin of `conform-cli`):
...
//! The baseline FILE is separate (`conform-typescript-baseline.json`)
...
use conform_frontend_typescript::TsTscFrontend;

/// The default TS baseline path, root-relative.
pub const DEFAULT_TS_BASELINE: &str = "conform-typescript-baseline.json";

fn load_config(root: &Path) -> Result<Config> {
    let (cfg, origin) = Config::load_or_default(root)?;
    match origin {
        conform_core::ConfigOrigin::Loaded => {
            eprintln!("conform-typescript: policy conform.toml (loaded).");
        }
        conform_core::ConfigOrigin::Defaulted => eprintln!(
            "conform-typescript: NO conform.toml — topology default in force \
             (roots = [\"src\"], no cells gate); run `discipline-typescript init` \
...
fn extract(root: &Path, config: &Config) -> Result<Vec<conform_core::SourceFacts>> {
    ...
    eprintln!(
        "conform-typescript: extracted {} file(s), {} cached (producer ts-tsc-1).",
        log.extracted.len(),
        log.cached,
    );
    Ok(facts)
...
pub fn run_check(root: &Path, baseline_rel: &str, scope: Option<&str>) -> Result<()> {
    ...
    for f in &new {
        eprintln!(
            "  conform-typescript: NEW {} {}:{} — {}",
            f.rule, f.file, f.line, f.message
        );
    }
    for fp in &stale {
        eprintln!("  conform-typescript: baseline entry no longer fires — prune it: {fp}");
    }
    let counts = count_by_rule(&findings);
    eprintln!(
        "conform-typescript check: {} finding(s) in scope {} ({:?}), {} frozen in baseline, {} new; SARIF at {}.",
        findings.len(),
    ...
    if !new.is_empty() {
        bail!(
            "conform-typescript: {} new finding(s) against the baseline",
            new.len()
        );
    }
    Ok(())
...
pub fn run_freeze(root: &Path, baseline_rel: &str) -> Result<()> {
    ...
    eprintln!(
        "conform-typescript freeze: {} fingerprint(s) frozen ({:?}) at {}.",
        fps.len(),
        counts,
        baseline_rel
    );
