//! `discipline-typescript health` — the sweep fact-collector, TS shape
//! (`spec://discipline-core/04-SWEEP-PLAYBOOK#collector`): the
...
pub fn run_health(root: &Path, out_rel: &str) -> Result<()> {
    ...
    let records = ts_extract_bridge::extract_tree(root, &extractor, None)
    ...
    let ts_roots: Vec<String> = config
        ...
        .collect();
    let in_scope: Vec<&ts_extract_bridge::FileRecord> = records
        .iter()
    ...
    for record in &in_scope {
        for fact in &record.facts {
            match fact {
                ts_extract_bridge::RawFact::FileMetrics { lines } => {
                    if *lines > BUDGET {
                ...
                ts_extract_bridge::RawFact::TsUnsafe { reason, .. } => {
                    if reason.is_some() {
                        unsafe_reasoned += 1;
                    } else {
                        unsafe_unreasoned += 1;
                ...
                ts_extract_bridge::RawFact::Item {
                    is_exported,
                    has_doc_example,
                    ..
                } => {
                ...
                ts_extract_bridge::RawFact::Import { .. } => {}
            }
        }
    }
    ...
    let orphan_backlog = match specmap_core::config::Config::load(root)? {
        Some(cfg) => specmap_scan_typescript::orphans(&records, &cfg)
            .into_iter()
    ...
    let snapshot = serde_json::json!({
        "schema": 1,
        "collector": "discipline-typescript health",
