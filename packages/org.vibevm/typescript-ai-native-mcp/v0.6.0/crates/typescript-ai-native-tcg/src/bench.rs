//! `typescript-ai-native-tcg bench` — the corpus harness behind the battery's
...
use typescript_ai_native_tcg_bridge::{OracleTransport, SystemOracle};
...
pub fn run_bench(root: &Path, corpus: &Path, report_path: &Path) -> Result<i32> {
    ...
    println!(
        "typescript-ai-native-tcg bench: {} case(s), agreement {:.1}%, cold init {:.0} ms, \
