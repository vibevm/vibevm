...
//! `typescript-ai-native-specmap` CLIs call — parity by construction, pinned by
    ...
    vec![
        t(
            ...
            "Bootstrap the discipline surface (conform.toml, specmap.toml, registries, \
             external specs). = `typescript-ai-native init`. Never overwrites without \
             `force`.",
            properties! {
        ...
        t(
            ...
            "The seven-step verification floor: prettier → tsc → tests → eslint → \
             ...
             `typescript-ai-native floor`. Runs the project's whole toolchain — \
            ...
            |root, args| {
                typescript_ai_native_cli::run_floor(
                    root,
                    &typescript_ai_native_cli::FloorOptions {
        ...
        t(
            ...
            "The ts-tsc conform gate: extract facts through the project's own \
             ...
             frozen ratchet. = `typescript-ai-native-conform check`.",
            properties! {
                "scope": {"type": "string", "description": "limit to one root by name"},
                "baseline": {"type": "string", "description": "ratchet file, root-relative (default typescript-ai-native-conform-baseline.json)"},
            },
            |root, args| {
        ...
        t(
            ...
            "Rewrite the TS conform ratchet to the current finding set. = \
             `typescript-ai-native-conform freeze`.",
            properties! {
                "baseline": {"type": "string", "description": "ratchet file, root-relative"},
            },
            |root, args| {
                typescript_ai_native_conform::run_freeze(
                    root,
                    str_arg(
                        args,
                        "baseline",
        ...
        t(
            ...
            properties! {},
            |root, _| typescript_ai_native_specmap::run_specmap_typescript(root, true),
        ),
        t(
            "specmap_write",
            "Regenerate and write specmap.json; the orphan gate reports non-blocking. \
             = `typescript-ai-native-specmap`.",
            properties! {},
            |root, _| typescript_ai_native_specmap::run_specmap_typescript(root, false),
        ),
        t(
            "trace_explain",
            "Explain one symbol or spec unit through the index. = \
             `typescript-ai-native trace <target>`.",
            properties! {
            ...
            |root, args| {
                ...
                typescript_ai_native_cli::run_trace_explain(
                    root,
                    target,
                    bool_arg(args, "json"),
                    bool_arg(args, "prose"),
        ...
        t(
            ...
            "The xfail-strict test gate over node's TAP output vs the tests baseline. \
             = `typescript-ai-native test-gate`. Runs the whole suite — expect \
             minutes.",
            properties! {
                "baseline": {"type": "string", "description": "root-relative baseline (default discipline/registry/tests-baseline.json)"},
            },
            |root, args| {
                typescript_ai_native_cli::run_test_gate(
                    root,
                    str_arg(
                        args,
                        "baseline",
                        typescript_ai_native_cli::DEFAULT_TESTS_BASELINE,
                    ),
                )
            },
        ),
        ...
        t(
            "health",
            "The sweep's fact collector: one JSON snapshot. = `typescript-ai-native \
             health`.",
            properties! {
                "out": {"type": "string", "description": "root-relative output (default discipline/health/latest-typescript.json)"},
            },
            |root, args| {
                typescript_ai_native_cli::run_health(
                    root,
                    str_arg(args, "out", typescript_ai_native_cli::DEFAULT_HEALTH_OUT),
                )
            },
        ),
        t(
            ...
            "Per-cell first-signal budget: typecheck+test each cell in isolation. = \
             `typescript-ai-native fast-loop`. Expensive.",
            properties! {
            ...
            |root, args| {
                typescript_ai_native_cli::run_fast_loop(
                    root,
        ...
        t(
            ...
            "Scaffold one cell (dir + index seam + spec marker; the cells dir and seam \
             ...
             = `typescript-ai-native codemod add-cell <cell> <spec-uri>`.",
            properties! {
            ...
            |root, args| {
                ...
                typescript_ai_native_cli::run_codemod_add_cell(
                    root,
                    need("cell")?,
                    need("spec_uri")?,
                )
