...
impl TsTscFrontend {
    ...
    pub fn new(project_root: &Path) -> Result<TsTscFrontend> {
        let path = typescript_ai_native_extract_bridge::materialise_extractor(project_root)
            .with_context(|| {
    ...
    fn warm_batch(&self, files: Option<&[String]>) {
        let records = match typescript_ai_native_extract_bridge::extract_tree(
            &self.project_root,
            &self.extractor,
            files,
        ) {
        ...
        for record in &records {
            warmed.insert(
                record.file.clone(),
                typescript_ai_native_extract_bridge::conform_facts(record),
            );
        }
    }
    ...
    pub fn probe(
        &self,
    ) -> std::result::Result<(), typescript_ai_native_extract_bridge::BridgeError> {
        typescript_ai_native_extract_bridge::extract_tree(
            &self.project_root,
            &self.extractor,
            Some(&[]),
        )
...
mod tests {
    ...
    fn extractor_materialises_content_addressed_and_idempotent() {
        ...
        assert_eq!(body, typescript_ai_native_extract_bridge::EXTRACTOR_SOURCE);
    }
}
