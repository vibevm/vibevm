...
- **Enrichment is in-process — one engine, zero extra hops.** The gate's
  fact extractor (`conform-frontend-rust`'s `RustFrontend::extract`) is
  a pure function over source text, so the relay runs the REAL conform
  rules (`conform_cli_rust::build_rules` → `conform_core::check`) on the
...
- **Dual transport, agent's choice** (PROP-018 §2.8): a persistent
  ...
  (`vibe bin exec tcg-rust -- validate …`) for agents without MCP.
...
agent ──MCP (tcg_validate/tcg_scope/tcg_complete/tcg_type,
   ...
   └─or one-shot─▶ vibe bin exec tcg-rust -- <op> ──▶ tcg-rust
...
- **The bridge** (`crates/tcg-oracle-bridge-rust`, this package): the
...
- **The CLI** (`crates/tcg-cli-rust`, bin **`tcg-rust`**, the package's
