...
fn the_frozen_baseline_downgrades_to_baselined() {
    ...
    let frontend = conform_frontend_rust::RustFrontend;
    ...
    let config = conform_cli_rust::load_config(dir.path()).expect("config");
...
fn exit_contract_covers_error_diagnostics() {
    ...
    let outcome = ValidateOutcome {
        diagnostics: vec![tcg_oracle_bridge_rust::Diagnostic {
