//! `tcg-rust serve` — the persistent enriching relay
//! (TCG-PROTOCOL-RUST §1–§3): outer frames in on stdin, frames out on
//! stdout; `validate` answers are widened with the gate's findings +
//! advice; everything else passes through the oracle. The relay OWNS
//! session init (a host's first frame may be any op); client `init`
//! frames respawn the analyzer.

specmark::scope!("spec://rust-ai-native/mechanisms/TCG-PROTOCOL-RUST-v0.1#parity");
...
use tcg_oracle_bridge_rust::client::ChildTransport;
use tcg_oracle_bridge_rust::{RustOracle, TcgBridgeError};

use crate::{
    Policy, QUIESCENCE_BUDGET, ScopeAnswer, derive_crate_module, detect_newtypes, enrich_validate,
    finalise_completions, parse_position, seam_file_for,
};

/// The wire protocol both hops share (TCG-PROTOCOL-RUST §1).
...
fn position_param(
    params: &serde_json::Value,
) -> Option<tcg_oracle_bridge_rust::position::OuterPosition> {
    let p = params.get("position")?;
    if let Some(s) = p.as_str() {
        return parse_position(s).ok();
    }
    Some(tcg_oracle_bridge_rust::position::OuterPosition {
...
fn init_result(oracle: &RustOracle<ChildTransport>) -> serde_json::Value {
    serde_json::json!({
        ...
        "position_encoding": match oracle.capabilities().position_encoding {
            tcg_oracle_bridge_rust::position::PositionEncoding::Utf8 => "utf-8",
            tcg_oracle_bridge_rust::position::PositionEncoding::Utf16 => "utf-16",
        },
...
fn handle_op(
    policy: &Policy,
    oracle: &mut RustOracle<ChildTransport>,
    id: Option<u64>,
    op: &str,
    params: &serde_json::Value,
        ...
        "scope" => match &file {
            Some(f) => {
                let pos = position_param(params).unwrap_or(
                    tcg_oracle_bridge_rust::position::OuterPosition {
                        line: 1,
                        character: 0,
                    },
                );
                oracle.complete(f, pos, content.clone()).map(|entries| {
                    ...
                    let seam = seam_file_for(&policy.root, f);
                    let mut branded = text
                        .as_deref()
                        .map(|t| detect_newtypes(t, f))
                        .unwrap_or_default();
                    if seam != *f
                        && let Ok(seam_text) = std::fs::read_to_string(policy.root.join(&seam))
                    {
        ...
        "complete" => match (&file, position_param(params)) {
            (Some(f), Some(pos)) => oracle.complete(f, pos, content.clone()).map(|entries| {
                ...
                let max = params
                    ...
                    .max(1) as usize;
                serde_json::json!({
                    "entries": finalise_completions(entries, f, prefix.as_deref(), max),
                })
            }),
...
pub fn run_serve(root: &Path) -> Result<i32> {
    ...
    // The relay owns the session: boot the analyzer up front so the
    // host's FIRST frame can be validate/scope/… (client init frames
    // remain re-init).
    let mut oracle = match RustOracle::spawn(&root, QUIESCENCE_BUDGET) {
        Ok(o) => o,
        Err(e) => {
    ...
    eprintln!(
        "tcg-rust serve: oracle up — rust-analyzer {}, quiescent={}, {}",
    ...
    for line in stdin.lock().lines() {
        ...
        if op == "init" {
            ...
            match RustOracle::spawn(&root, QUIESCENCE_BUDGET) {
                Ok(fresh) => {
