# mcp:org.vibevm/discipline-rust

The AI-Native Rust discipline served over MCP: one stdio binary
(`discipline-mcp-rust`), eighteen tools — the whole `discipline-rust`
...
- **Install:** `vibe install mcp:org.vibevm/discipline-rust` — the
  exact `=` pin pulls the matching `stack:org.vibevm/rust-ai-native`.
- **Register:** `vibe mcp install` writes the server into your agent's
  ...
  artifact — `vibe bin build discipline-mcp-rust` compiles it).
