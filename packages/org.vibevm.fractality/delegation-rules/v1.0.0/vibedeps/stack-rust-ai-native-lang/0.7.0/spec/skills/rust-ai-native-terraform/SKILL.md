...
Toolchain: `rust-ai-native` (install once:
...
2. `rust-ai-native init` — policies + empty registries. Every crate
...
6. **Characterize** currently-passing observable behavior (golden
   ...
   a landmine.
7. `rust-ai-native specmap` — mint the (initially small) index; commit
...
1. **Drain** the crate: unwraps out of domain code (restructure beats
   ...
   `rust-ai-native fast-loop --cell <crate>` keeps the loop under budget.
...
4. Each phase closes with the floor green (`rust-ai-native floor`) and
...
`rust-ai-native floor` green with every crate either gated or
...
held by the recurring sweep: /rust-ai-native-sweep.
