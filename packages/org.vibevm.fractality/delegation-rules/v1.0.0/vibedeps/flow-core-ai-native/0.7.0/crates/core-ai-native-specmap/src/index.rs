...
//! `rust-ai-native-specmap` (or the project's wrapper), gated by `--check` the way
...
pub fn check_with_scanner(
    ...
    let committed = std::fs::read_to_string(&path).with_context(|| {
        format!(
            "reading {} — run `rust-ai-native-specmap` (or your project's wrapper) first",
            path.display()
        )
    })?;
    if fresh == committed {
...
///
/// ```
/// use core_ai_native_specmap::index::{Summary, vacuity_warning};
///
