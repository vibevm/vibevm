...
|---|---|---|---|---|
| [PP-001](postponed/PP-001-rule-rp5-fire-mt-c2-05-rerun.md) | Rule RP5 — authorize and fire the MT-C2-05 re-run | 2026-07-10 | **DONE 2026-07-12** — fired (3+3 GLM) + scored. REPLICATES Ф6: A′ = 11.1% ≈ old A (within noise), B′ = 0% = old B; PR1 falsified (noise), PR2 not-confirmed (F23 `-p` re-fire), PR3 falsified. Initiative hooks don't move a cold `-p` boss; the RLM gate (44.4%) does — validates the C3 pivot. Open lever: move the nudge to a PostToolUse path | drained (F23 `-p`-nudge follow-up noted in MT-C2-05) |
...
| [PP-003](postponed/PP-003-option-c-advisor-slice.md) | Option C — the advisor slice (V4) | 2026-07-11 | **DONE 2026-07-12 (Stage C executed)** — core (`advice` + RD-10 bar) + `advise` verb + ladder-as-routing-data + help/hurt trial MT-C3-02 fired&scored + the uncertainty-trigger protocol (ADVISOR-PLAN §6). Trial finding: null mechanism — a weak caller won't consult from a preamble; the consult must be DRIVEN (§6 delivery options). Re-run follow-ups: mandated/forced consult, pin task-1 name, a 3rd tier for the full RD-10 inversion | drained (re-run to measure thresholds when commissioned) |
| [PP-004](postponed/PP-004-next-trial-improvements.md) | Next-trial improvements (worker caps, schema+Silo menu tasks, `decisions` verb) | 2026-07-12 | **DONE 2026-07-12** — all four items landed + the gated re-run fired & scored (MT-C3-03: P-C3-a CONFIRMED as a hard count, P-C3-b SUPPORTED, P-C3-d CONFIRMED). Re-fire follow-up → **PP-005 FU-3** | drained |
| [PP-005](postponed/PP-005-trial-followups.md) | The three trial follow-ups — FU-1 advisor forced-consult re-run (MT-C3-02), FU-2 PP-001 PostToolUse nudge (MT-C2-05), FU-3 PP-004 clean-N=3 re-fire (MT-C3-03) | 2026-07-12 | POSTPONED — each is a re-run of a frozen protocol with one small change; none blocks anything (the five-task goal is done, the machinery is floor-green) | owner commissions any of the three (paid GLM runs) |
