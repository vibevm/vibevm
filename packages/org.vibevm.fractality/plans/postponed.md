...
| [PP-002](postponed/PP-002-def-c2-2b-worker-credibility.md) | DEF-C2-2b-full — acceptance-backed worker credibility on the boss surface | 2026-07-10 | **CORE LANDED 2026-07-12** — the credibility QUERY (`core::worker_credibility` → `CredibilityFact`: most recent completed run with full-green acceptance, D7-factual, dated), tested + floor-green. Acceptance data already rode `RunRecord.collected`. REMAINING: surface wiring (scoreboard / cold board / mid-work nudge cite the fact with its age) | wire the fact onto a boss surface |
