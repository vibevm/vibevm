...
@fact:companion-line **Companion to:** BACKLOG.md B-041 (closed by `1f048058`) (the commissioning entry) and the backlog's own [`#map` section](BACKLOG.md#map); the atoms this map arranges are the backlog's build entries B-001…B-043 and the campaign's «Specified, not built» annotations. Genre-wise this is a design-rationale document living beside the backlog it arranges — the [`spec/design/` index](spec/design/README.xml) carries its row. [`ROADMAP.md`](ROADMAP.md) is a different document: the **product** milestone roadmap (M0/M1.x…); this map covers the **discipline tooling**, and the two do not compete.
...
- @fact:WAVE-G **Волна Г — хост догоняет собственную дисциплину: ЗАКРЫТА ЦЕЛИКОМ 2026-08-05.** B-005 (ancestry gate, построено 2026-08-04) → B-010 (check-verb fix, коммит `4685c8cc`; строка бэклога держала `open` сутки после) → F-132 (долг схем — закрыт правкой ложной клаузулы, а не постройкой: назначенная единица разметки не размечена и прочитана быть не может, B-060 (closed by `0f12992e`)) → B-040 (обзор швов — четыре посадки построены, пятая отклонена измерением, sealed traits записаны как сознательный отказ; дизайн `spec/design/typed-seams.xml`). Волна попутно родила три находки — B-060, B-061 (closed by `572f3c1a`), B-062 (closed by `ff2079e1`) — и последняя есть развилка владельца.
