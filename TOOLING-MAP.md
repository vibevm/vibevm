...
@fact:companion-line **Companion to:** BACKLOG.md B-041 (closed by `1f048058`) (the commissioning entry) and the backlog's own [`#map` section](BACKLOG.md#map); the atoms this map arranges are the backlog's build entries B-001…B-043 and the campaign's «Specified, not built» annotations. Genre-wise this is a design-rationale document living beside the backlog it arranges — the [`vibevm/vibespecs/design/` index](spec/design/README.xml) carries its row. [`ROADMAP.md`](ROADMAP.md) is a different document: the **product** milestone roadmap (M0/M1.x…); this map covers the **discipline tooling**, and the two do not compete.
...
@fact:frame-line **The frame (owner, 2026-08-02, same sitting):** «мы сейчас находимся в процессе более большого рефакторинга… нам надо действовать в рамках этого процесса, а то чего не хватает — отложить на потом». The running process is the **PROP-043 Progress-Control programme, wave 2** — the packages-actualization campaign ([`vibevm/vibespecs/terraforms/PACKAGES-ACTUALIZATION-CAMPAIGN-v0.1.xml`](vibevm/vibespecs/terraforms/PACKAGES-ACTUALIZATION-CAMPAIGN-v0.1.xml)), Phase D at its exit ramp, Phases E/T/F/G designed ahead. **This map does not start work; the campaign's phases do.** Phase E's mandate drains the recorded builds it inherits; what a phase mandate does not cover waits for the next owner mandate. The waves below are the *shape* of that drainage, not a parallel programme.
...
@fact:map-refreshed-2026-08-06-second-sitting **Refreshed 2026-08-06, second
sitting — and the fact above is kept as it was rather than rewritten**, because
it is a dated reading and rewriting one keeps its conclusion while erasing the
reasoning that produced it. What the refresh changed, in full: the map's **search**
gap is closed at BOTH levels (`vibe query` the filters, `vibe select` the
traversal), so fork 6 below is taken rather than open; the index's four
capabilities named in the line above are built; and three new atoms arrived that
no wave arranges yet — B-075 (the panel's intermittent red), B-076 (the
traceability gate does not check that a cited anchor resolves) and B-077 (the
TUI's duplicated persist path, the one item standing between that subsystem and
«finished»). What still has NOT moved is the ordering logic. **The instruction
that produced this refresh stands for the next reader too: refresh before
...
@fact:agent-gaps-re-measured-2026-08-06 **Re-measured 2026-08-06, and two of
those four gaps are closed** — the reading above is kept dated rather than
rewritten. vibe's own MCP now carries **three** map tools: `explain` (one
target), `query` (the filter set) and `select` (the traversal), each a thin
surface over the same `vibe-trace` capability the CLI uses. «Объясни чужой
пакет» has its data path: the reader assembles a second, never-persisted
resolver in memory from the maps installed packages carry, which is what left
the committed map byte-identical. **The two that remain are the two this line
should now be read for:** no LLM prose producer, and no fetch-by-hash for
fragments. @status:doc/work
...
- @fact:WAVE-V **Волна В — карта и её потребители:** B-013 → the one format change (B-019а + B-016.1 + B-017; B-024 decided alongside) → B-018.1/.2 → B-018.4 + B-016.2 → B-020 + B-021; B-014 decided here. Feeder: B-022 (ledger mechanisms). Exit: M-ASK + M-DRIFT. **Status 2026-08-06: B-013, B-019а, B-018.1, B-018.2 and B-018.4 are built** — the point lookup, both search levels and the foreign-package resolver. What the wave still owes: B-016.2 (fragments by hash), B-020 (the LLM prose producer), B-021 (threshold warnings), B-017 (privacy profiles) and B-024's vocabulary merge; B-019(б) is two slices in and B-019(в) waits on the consumer-boundary question B-019 records.
- @fact:WAVE-G **Волна Г — хост догоняет собственную дисциплину: ЗАКРЫТА ЦЕЛИКОМ 2026-08-05.** B-005 (ancestry gate, построено 2026-08-04) → B-010 (check-verb fix, коммит `4685c8cc`; строка бэклога держала `open` сутки после) → F-132 (долг схем — закрыт правкой ложной клаузулы, а не постройкой: назначенная единица разметки не размечена и прочитана быть не может, B-060 (closed by `0f12992e`)) → B-040 (обзор швов — четыре посадки построены, пятая отклонена измерением, sealed traits записаны как сознательный отказ; дизайн `vibevm/vibespecs/design/typed-seams.xml`). Волна попутно родила три находки — B-060, B-061 (closed by `572f3c1a`), B-062 (closed by `ff2079e1`) — и последняя есть развилка владельца.
...
6. @fact:FORK-QUERY-LANGUAGE The map query language v0 (exact URI + symbol + type filter + response ceiling is the placeholder) (B-018.2) — **TAKEN 2026-08-06, and taken as BOTH shapes rather than one.** The owner's ruling was that the two weighed shapes are two LEVELS, not two candidates: the filter level (three predicates joined by AND under a hard ceiling) is **permanent** and shipped as `vibe query` (`471e3b1b`); the traversal level — depth plus «has no edge of this kind», the one that answers «which rules does nobody verify» — shipped as `vibe select` (`fc7495f2`) and stands ON it, in its own module behind its own entry point so a broken grammar cannot take the filters down. The grammar's versioning cost was named before the work was authorised and is paid by a version reported in every answer rather than demanded in every query. One thing the fork list could not have known and the build measured: the traversal level's own canonical question is **unanswerable without a document scope** — `lacks:verifies` alone returns 5 742 of 5 825 units, and neither shipped filter can narrow it, because `kind` is carried by zero units in this tree and `uri` is exact. Contract: `PROP-015 §2.2.2`; reasoning: [`vibevm/vibespecs/design/map-query-language.xml`](vibevm/vibespecs/design/map-query-language.xml).
