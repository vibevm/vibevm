...
- ##WAVE-G **Волна Г — хост догоняет собственную дисциплину: ЗАКРЫТА ЦЕЛИКОМ 2026-08-05.** B-005 (ancestry gate, построено 2026-08-04) → B-010 (check-verb fix, коммит `4685c8cc`; строка бэклога держала `open` сутки после) → F-132 (долг схем — закрыт правкой ложной клаузулы, а не постройкой: назначенная единица разметки не размечена и прочитана быть не может, [B-060](BACKLOG.md#b-060)) → B-040 (обзор швов — четыре посадки построены, пятая отклонена измерением, sealed traits записаны как сознательный отказ; дизайн `spec/design/typed-seams.md`). Волна попутно родила три находки — B-060, [B-061](BACKLOG.md#b-061), [B-062](BACKLOG.md#b-062) — и последняя есть развилка владельца. @doc/done
...
6. ##FORK-QUERY-LANGUAGE The map query language v0 (exact URI + symbol + type filter + response ceiling is the placeholder) (B-018.2) — **NOT taken; deferred by the owner 2026-08-04 to the backlog at medium priority.** Part 1 of B-018 shipped the same day, so the host answers the canonical query for an exact address; what is deferred is SEARCH. The two shapes weighed are recorded in the entry so they are not re-derived — three independent filters combined by AND with a hard result ceiling (nothing to parse, nothing to version), or that plus graph traversal, which answers «which rules does nobody verify» at the cost of a grammar. @doc/done
